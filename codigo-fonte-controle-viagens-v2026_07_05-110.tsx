...
const APP_VERSION = "2026.07.05-110";
...
const CAMINHOES_HEADER = ["id", "placa", "semAlertaOleo", "intervaloOleoKm", "tipo", "modelo", "foto", "renavam", "chassi", "anoFabricacao", "anoModelo", "cor", "combustivel", "categoria", "municipio", "uf", "vencimentoDocumento", "documentosPorAnoJSON", "crv", "ipvaPagoPorAnoJSON"];
...
const BOLETOS_HEADER = ["id", "empresa", "descricao", "notaFiscal", "valor", "dataVencimento", "contaBancaria", "dataPagamento", "observacao", "desconto", "descontoDescricao", "juros", "baixasJSON", "codigoBarras"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Caminhoes!A1:T1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Caminhoes!A1:T1", values: [CAMINHOES_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/Boletos!A1:N1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Boletos!A1:N1", values: [BOLETOS_HEADER] }),
  });
...
function boletoToRow(b) {
  return [b.id, b.empresa, b.descricao || "", b.notaFiscal, b.valor, b.dataVencimento, b.contaBancaria, b.dataPagamento, b.observacao || "", b.desconto || "", b.descontoDescricao || "", b.juros || "", JSON.stringify(b.baixas || []), b.codigoBarras || ""];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    sheetsFetch(`/values/Caminhoes!A2:T`, token),
    sheetsFetch(`/values/Viagens!A2:X`, token),
    sheetsFetch(`/values/Vales!A2:H`, token),
    sheetsFetch(`/values/Boletos!A2:N`, token),
...
const SHEET_DEFS = {
  trucks: { nome: "Caminhoes", header: CAMINHOES_HEADER, colunas: "T", toRow: truckToRow },
  ...
  boletos: { nome: "Boletos", header: BOLETOS_HEADER, colunas: "N", toRow: boletoToRow },
...
function decodificarCodigoBarrasBoleto(codigo) {
  ...
  const dataBase = new Date(Date.UTC(1997, 9, 7));
  ...
  return { valor, dataVencimento: iso, codigoBarras: limpo };
}
...
function carregarZXing() {
  ...
  zxingCarregando = carregarScript("https://unpkg.com/@zxing/library@0.20.0/umd/index.min.js")
    .then(() => carregarScript("https://unpkg.com/@zxing/browser@0.1.5"))
    .then(() => ({ ZXing: window.ZXing, ZXingBrowser: window.ZXingBrowser }));
        ...
        leitor.decodeFromConstraints(
          ...
          (resultado, erro, controle) => {
            ...
            if (resultado) {
              ...
              if (texto && texto.replace(/\D/g, "").length === 44) {
                controle.stop();
                onDetectado(texto.replace(/\D/g, ""));
              }
            }
          }
        );
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 60, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "min(420px, 92vw)", background: "#1B2430", borderRadius: 14, padding: 16, textAlign: "center" }}>
        ...
        <button
          ...
          style={{ marginTop: 14, background: "#fff", color: "#1B2430", border: "none", borderRadius: 10, padding: "10px 18px", fontWeight: 700, cursor: "pointer" }}
        >
          Cancelar
        </button>
      </div>
...
function NovoMotoristaForm({ visible, onCancel, onConfirm, inputStyle }) {
  ...
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 12, marginBottom: 16 }}>
      ...
      <Field label="Data">
        <input type="date" style={inputStyle} value={data} onChange={(e) => setData(corrigirAnoDigitado(e.target.value))} />
      </Field>
      <Field label="Valor (R$)">
      ...
      {aberto && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Field label="De">
              <input type="date" style={inputStyle} value={periodStart} onChange={(e) => onChangeStart(corrigirAnoDigitado(e.target.value))} />
            </Field>
            <Field label="Até">
              <input type="date" style={inputStyle} value={periodEnd} onChange={(e) => onChangeEnd(corrigirAnoDigitado(e.target.value))} />
            </Field>
          </div>
        </div>
      )}
    ...
    <div style={{ padding: "24px 28px" }}>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(480px, 100vw)", background: "#fff", zIndex: 21, overflowY: "auto", boxShadow: "-8px 0 24px rgba(0,0,0,0.15)", padding: "24px 24px 100px" }}>
            ...
            {!boletoTravado && (
              <button
                ...
                style={{ background: "#2451A6", color: "#fff", border: "none", borderRadius: 10, padding: "10px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer", marginBottom: 16, width: "100%" }}
              >
            ...
            {leitorAberto && (
              <LeitorCodigoBarras
                ...
                onDetectado={(codigo) => {
                  ...
                  setEditing((prev) => ({ ...prev, valor: decodificado.valor, dataVencimento: decodificado.dataVencimento, codigoBarras: decodificado.codigoBarras }));
                }}
              />
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
              ...
              <Field label="Data de vencimento">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.dataVencimento} onChange={(e) => setEditing({ ...editing, dataVencimento: corrigirAnoDigitado(e.target.value) })} />
              </Field>
              <Field label="Data de pagamento (baixa)">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.dataPagamento} onChange={(e) => setEditing({ ...editing, dataPagamento: corrigirAnoDigitado(e.target.value) })} />
              </Field>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {!boletoTravado && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={novaBaixaData} onChange={(e) => setNovaBaixaData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label="Valor (R$)">
              ...
              {(editing.baixas || []).length > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, marginTop: 10, background: "#1B2430", color: "#fff", borderRadius: 10, padding: "8px 12px" }}>
              ...
              <LeitorCodigoBarras
                ...
                onDetectado={(codigo) => {
                  ...
                  setNovoParcelas((prev) => prev.map((p) => {
                    ...
                    if (!decodificado) {
                      alert("Código lido, mas não consegui identificar o valor/vencimento automaticamente (deve ser boleto de concessionária/convênio). Preenche o valor e vencimento na mão, por favor.");
                      return p;
                    }
                    return { ...p, valor: decodificado.valor, dataVencimento: decodificado.dataVencimento };
                  }));
                }}
              />
            )}
              ...
              <>
                ...
                <RepeatingSection
                  ...
                  <Field label="Vencimento">
                    <input type="date" style={inputStyle} value={item.dataVencimento} onChange={(e) => update({ ...item, dataVencimento: corrigirAnoDigitado(e.target.value) })} />
                  </Field>
                  ...
                  <Field label="Data pagamento (baixa)">
                    <input type="date" style={inputStyle} value={item.dataPagamento} onChange={(e) => update({ ...item, dataPagamento: corrigirAnoDigitado(e.target.value) })} />
                  </Field>
                  <Field label="Observação">
                ...
                <>
                  ...
                  {parcelasPreview.map((p, i) => (
                    <div key={p.id} style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", borderRadius: 12, padding: 10, marginBottom: 8 }}>
                      ...
                      <Field label="Vencimento">
                        <input type="date" style={inputStyle} value={p.dataVencimento} onChange={(e) => updateParcelaPreview(p.id, "dataVencimento", corrigirAnoDigitado(e.target.value))} />
                      </Field>
                      <Field label="Descrição">
      ...
      {reportOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ width: "min(760px, 94vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="De">
                <input type="date" style={inputStyle} value={periodStart} onChange={(e) => setPeriodStart(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Até">
                <input type="date" style={inputStyle} value={periodEnd} onChange={(e) => setPeriodEnd(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Status">
...
export default function App() {
  ...
  const totalOutrosAlertas = viagensSemComissaoPreenchida.length + viagensSemValorFrete.length + caminhoesDocumentoAtrasado.length + cavalosIpvaNaoPago.length + carretasLicenciamentoNaoPago.length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="nav-group" style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {[
            { id: "dashboard", icone: "📊", rotulo: "Dashboard" },
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    ...
                    {caminhoesDocumentoAtrasado.length > 0 && (
                      <div style={{ background: "#FBEBE8", borderRadius: 10, padding: 10, fontSize: 12, color: "#7A2A1D" }}>
                        ...
                        {caminhoesDocumentoAtrasado.map((x) => x.placa).join(" · ")}
                      </div>
                    )}
                    {cavalosIpvaNaoPago.length > 0 && (
                      <div style={{ background: "#FFF6E2", borderRadius: 10, padding: 10, fontSize: 12, color: "#8A5A00" }}>
                        ...
                        {cavalosIpvaNaoPago.map((x) => x.placa).join(" · ")}
                      </div>
                    )}
                    {carretasLicenciamentoNaoPago.length > 0 && (
                      <div style={{ background: "#FFF6E2", borderRadius: 10, padding: 10, fontSize: 12, color: "#8A5A00" }}>
                        ...
                        {carretasLicenciamentoNaoPago.map((x) => x.placa).join(" · ")}
                      </div>
                    )}
                  </div>
                )}
      ...
      {view === "dashboard" && (
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 28px" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
            <div style={{ flex: "1 1 0", minWidth: 160 }}><MileSign label="Viagens" value={monthlyReport.totals.viagens} tone="blue" icon="🚚" full /></div>
            <div style={{ flex: "1 1 0", minWidth: 160 }}><MileSign label="Receita" value={BRL(monthlyReport.totals.receita)} tone="green" icon="💵" full /></div>
            <div style={{ flex: "1 1 0", minWidth: 160 }}><MileSign label="Abastecimento" value={BRL(monthlyReport.totals.abastecimentoTotal)} tone="amber" icon="⛽" full /></div>
            <div style={{ flex: "1 1 0", minWidth: 160 }}>
              <MileSign
                label="Rendimento"
                value={BRL(monthlyReport.totals.liquido)}
                tone={monthlyReport.totals.liquido >= 0 ? "green" : "red"}
                icon={monthlyReport.totals.liquido >= 0 ? "📈" : "📉"}
                full
              />
            </div>
          </div>
          ...
          {(() => {
            const rowsComViagem = monthlyReport.rows.filter((r) => r.viagens > 0);
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
            {monthlyReport.rows.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  onClick={() => { setFilterTruck(r.id); setView("viagens"); }}
                  ...
                  <div key={g.motorista}>
                    ...
                    {fechandoMotorista === g.motorista && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 12, marginBottom: 10 }}>
                        <Field label="Data do pagamento/acerto">
                          <input type="date" style={inputStyle} value={fechamentoDataEditavel} onChange={(e) => setFechamentoDataEditavel(corrigirAnoDigitado(e.target.value))} />
                        </Field>
                    ...
                    {addingValeFor === g.motorista ? (
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 12 }}>
                        ...
                        <Field label="Data">
                          <input type="date" style={inputStyle} value={valeData} onChange={(e) => setValeData(corrigirAnoDigitado(e.target.value))} />
                        </Field>
                        <Field label="Valor (R$)">
                                ...
                                {editandoFechamentoId === f.id && (
                                  <div style={{ display: "flex", gap: 8, alignItems: "flex-end", marginBottom: 8 }}>
                                    <Field label="Nova data do pagamento">
                                      <input type="date" style={inputStyle} value={editandoFechamentoData} onChange={(e) => setEditandoFechamentoData(corrigirAnoDigitado(e.target.value))} />
                                    </Field>
          ...
          >
            ...
            <div style={{ marginBottom: 14 }}>
              {addingDespesaFor !== null ? (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 12 }}>
                  ...
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={despesaData} onChange={(e) => setDespesaData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label="Descrição">
            ...
            <div style={{ marginBottom: 18, background: "#FFF6E2", borderRadius: 12, padding: 12 }}>
              ...
              {addingTaxa ? (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={taxaData} onChange={(e) => setTaxaData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label="Valor (R$)">
      ...
      {relatorioRecebimentoOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="De">
                <input type="date" style={inputStyle} value={recebimentoPeriodStart} onChange={(e) => setRecebimentoPeriodStart(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Até">
                <input type="date" style={inputStyle} value={recebimentoPeriodEnd} onChange={(e) => setRecebimentoPeriodEnd(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Caminhão">
      ...
      {view === "trocaoleo" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={trocaOleoData} onChange={(e) => setTrocaOleoData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="De">
                <input type="date" style={inputStyle} value={oleoPeriodStart} onChange={(e) => setOleoPeriodStart(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Até">
                <input type="date" style={inputStyle} value={oleoPeriodEnd} onChange={(e) => setOleoPeriodEnd(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Placa">
                ...
                <tbody>
                  {trocaOleoReport.items.map((t) => (
                    <tr key={t.id} style={{ borderBottom: "1px solid #EEF0F2" }}>
                      <td style={{ padding: "8px 10px", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>{truckLabel(t.caminhaoId)}</td>
          ...
          <div style={{ background: "#EEF0FF", border: "1px solid #C7CFFA", borderRadius: 12, padding: 16, marginTop: 24 }}>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                      ...
                      <Field label="Data">
                        <input type="date" style={inputStyle} value={servicoData} onChange={(e) => setServicoData(corrigirAnoDigitado(e.target.value))} />
                      </Field>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="De">
                <input type="date" style={inputStyle} value={servicoPeriodStart} onChange={(e) => setServicoPeriodStart(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Até">
                <input type="date" style={inputStyle} value={servicoPeriodEnd} onChange={(e) => setServicoPeriodEnd(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Placa">
      ...
      {view === "semparar" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ background: "#FFF6E2", border: "1px solid #D9A419", borderRadius: 12, padding: 14, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
              <Field label="Data">
                <input type="date" style={inputStyle} value={outroSemPararData} onChange={(e) => setOutroSemPararData(corrigirAnoDigitado(e.target.value))} />
              </Field>
              <Field label="Valor (R$)">
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <Field label="Data de vencimento">
                    <input type="date" style={inputStyle} value={semPararData} onChange={(e) => setSemPararData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
      ...
      {view === "seguro" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          {trucksCarretas.length > 0 && (
            <div style={{ background: "#fff", borderRadius: 12, padding: 12, marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {trucksCarretas.map((tr) => (
                  <div key={tr.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "#F7F8F9", borderRadius: 20, padding: "6px 12px", fontSize: 12 }}>
                    <span style={{ fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>{tr.placa}</span>
                    <button
                    ...
                    <div style={{ padding: "0 10px 12px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div>
                        <Field label="Vencimento do licenciamento/documento (opcional, só referência)">
                          <input type="date" value={tr.vencimentoDocumento || ""} onChange={(e) => updateTruckInfo(tr.id, { vencimentoDocumento: corrigirAnoDigitado(e.target.value) })} style={{ ...inputStyle, fontSize: 12, padding: "6px 8px" }} />
                        </Field>
                      </div>
        ...
        <>
          ...
          >
            ...
            <Section title="Viagem">
              ...
              <Field label="Data">
                <input type="date" style={inputStyle} value={editing.data} onChange={(e) => setEditing({ ...editing, data: corrigirAnoDigitado(e.target.value) })} />
              </Field>
              <Field label="Origem">
              ...
              <Field label="Data fim da viagem">
                <input type="date" style={inputStyle} value={editing.dataFim} onChange={(e) => setEditing({ ...editing, dataFim: corrigirAnoDigitado(e.target.value) })} />
              </Field>
            ...
            <Section title="Financeiro">
              ...
              <Field label="Data recebimento adiantamento">
                <input type="date" style={inputStyle} value={editing.dataRecebAdiantamento} onChange={(e) => setEditing({ ...editing, dataRecebAdiantamento: corrigirAnoDigitado(e.target.value) })} />
              </Field>
              ...
              <Field label="Data pagamento do saldo">
                <input type="date" style={inputStyle} value={editing.dataPagamentoSaldo} onChange={(e) => setEditing({ ...editing, dataPagamentoSaldo: corrigirAnoDigitado(e.target.value) })} />
              </Field>
            ...
            <RepeatingSection
              ...
              renderItem={(item, update) => (
                <>
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={item.data} onChange={(e) => update({ ...item, data: corrigirAnoDigitado(e.target.value) })} />
                  </Field>
                  <Field label="Tipo">
    ...
    {view === "trocaoleo" && (
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        ...
        <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 20, marginBottom: 4 }}>
          Outros Serviços Importantes
        </div>
        <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 16 }}>
