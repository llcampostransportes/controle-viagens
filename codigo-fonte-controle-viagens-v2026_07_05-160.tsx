...
const APP_VERSION = "2026.07.05-160";
...
const emptyGasto = () => ({ id: uid(), data: "", valor: "", descricao: "", categoria: "Despesas Estrada", setor: "", posto: "", numeroCupom: "", paraComissao: false });
  ...
  const visibleBoletos = useMemo(() => {
    return report.items
      .filter((b) => {
        ...
        const status = b.dataPagamento ? "pago" : vencimentoEfetivo(b) < hoje ? "vencido" : "pendente";
  ...
  const statsCards = useMemo(() => {
    ...
    return base.reduce(
      (acc, b) => {
        ...
        else if (vencimentoEfetivo(b) < hoje) acc.vencido += valor;
            ...
            return (
              ...
              >
                <div style={{ minWidth: 90, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 13, color: "#5A6472" }}>
                  {fmtDate(vencimentoEfetivo(b))}
                  {b.novoVencimento && <div style={{ fontSize: 10, color: "#9AA5B1" }}>orig. {fmtDate(b.dataVencimento)}</div>}
                </div>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(480px, 100vw)", background: "#fff", zIndex: 21, overflowY: "auto", boxShadow: "-8px 0 24px rgba(0,0,0,0.15)", padding: "24px 24px 100px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
              ...
              <Field label="Data de pagamento (baixa)">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.dataPagamento} onChange={(e) => setEditing({ ...editing, dataPagamento: e.target.value })} onBlur={(e) => setEditing({ ...editing, dataPagamento: corrigirAnoDigitado(e.target.value) })} />
              </Field>
              <Field label="Novo vencimento (opcional, se renegociar saldo)">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.novoVencimento || ""} onChange={(e) => setEditing({ ...editing, novoVencimento: e.target.value })} onBlur={(e) => setEditing({ ...editing, novoVencimento: corrigirAnoDigitado(e.target.value) })} />
              </Field>
...
export default function App() {
  ...
  // líquido geral. Só conta boleto que já foi efetivamente pago no período
  // (senão o "líquido" ficaria contando dinheiro que ainda não saiu)
  const dashboardGeralReport = useMemo(() => {
    ...
    const boletosPagos = boletos.filter((b) => {
      if (!b.dataPagamento) return false;
      if (b.dataPagamento < dashboardGeralPeriodStart || b.dataPagamento > dashboardGeralPeriodEnd) return false;
      const categoriaEmpresa = empresasPorCategoria[b.empresa];
      return categoriaEmpresa && categoriasIncluidasDashboardGeral.includes(categoriaEmpresa);
    });
    const boletosDespesa = boletosPagos.reduce((s, b) => {
      const base = (Number(b.valor) || 0) - (Number(b.desconto) || 0) + (Number(b.juros) || 0);
      const totalBaixas = (b.baixas || []).reduce((s2, bx) => s2 + (Number(bx.valor) || 0), 0);
      return s + (base - totalBaixas);
    }, 0);
    const taxaViagem = despesasVeiculo
    ...
    return { receitaBruta, comissoes, gastosExtrasEstrada, boletosDespesa, boletosPagos, taxaViagem, rendimentoLiquidoGeral };
  ...
  const boletosReport = useMemo(() => {
    const hoje = todayISO();
    const statusDe = (b) => (b.dataPagamento ? "pago" : vencimentoEfetivo(b) < hoje ? "vencido" : "pendente");
  ...
  const exportBoletosCSV = () => {
    ...
    boletosReport.items.forEach((b) => {
      const status = b.dataPagamento ? "Pago" : vencimentoEfetivo(b) < hoje ? "Vencido" : "Pendente";
      lines.push(
        [b.empresa, b.descricao || "", b.notaFiscal, (Number(b.valor) || 0).toFixed(2), valorFinalBoleto(b).toFixed(2), fmtDate(vencimentoEfetivo(b)), b.contaBancaria, b.dataPagamento ? fmtDate(b.dataPagamento) : "", status, b.observacao || ""]
          .join(";")
      );
    });
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    ...
    >
      ...
      {view === "dashboardGeral" && (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 28px" }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: "#5A6472", marginTop: 12, marginBottom: 4 }}>Detalhamento</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: "12px 16px" }}>
              <span style={{ fontSize: 14, color: "#5A6472" }}>💵 Receita bruta</span>
              <span style={{ fontSize: 17, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{BRL(dashboardGeralReport.receitaBruta)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: "12px 16px" }}>
              <span style={{ fontSize: 14, color: "#5A6472" }}>− 👤 Comissões</span>
              <span style={{ fontSize: 17, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "#B0402E" }}>{BRL(dashboardGeralReport.comissoes)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: "12px 16px" }}>
              <span style={{ fontSize: 14, color: "#5A6472" }}>− 🧾 Gastos extras (Despesas Estrada)</span>
              <span style={{ fontSize: 17, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "#B0402E" }}>{BRL(dashboardGeralReport.gastosExtrasEstrada)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: "12px 16px" }}>
              <span style={{ fontSize: 14, color: "#5A6472" }}>− 🧾 Boletos pagos ({dashboardGeralReport.boletosPagos.length})</span>
              <span style={{ fontSize: 17, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "#B0402E" }}>{BRL(dashboardGeralReport.boletosDespesa)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: "12px 16px" }}>
              <span style={{ fontSize: 14, color: "#5A6472" }}>− 🤝 Taxa de viagem (rateio)</span>
              <span style={{ fontSize: 17, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "#B0402E" }}>{BRL(dashboardGeralReport.taxaViagem)}</span>
            </div>
            <div
              style={{
                display: "flex", justifyContent: "space-between", alignItems: "center",
                background: dashboardGeralReport.rendimentoLiquidoGeral >= 0 ? "linear-gradient(160deg, #fff 0%, #E9F5F1 100%)" : "linear-gradient(160deg, #fff 0%, #FBEBE8 100%)",
                border: `1px solid ${dashboardGeralReport.rendimentoLiquidoGeral >= 0 ? "#1F6F5C" : "#B0402E"}`,
                borderRadius: 12, padding: "16px 18px", marginTop: 8,
              }}
            >
              <span style={{ fontSize: 15, fontWeight: 700 }}>{dashboardGeralReport.rendimentoLiquidoGeral >= 0 ? "📈" : "📉"} Rendimento líquido geral</span>
              <span style={{ fontSize: 22, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: dashboardGeralReport.rendimentoLiquidoGeral >= 0 ? "#12503F" : "#B0402E" }}>
                {BRL(dashboardGeralReport.rendimentoLiquidoGeral)}
              </span>
            </div>
          </div>

          {dashboardGeralReport.boletosPagos.length > 0 && (
            <div style={{ marginTop: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#5A6472", marginBottom: 8 }}>Boletos considerados nesse período</div>
              <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {dashboardGeralReport.boletosPagos.map((b, i) => {
                      ...
                      return (
                        <tr key={b.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                          ...
                          <td style={{ padding: "8px 12px", borderTop: "1px solid #EEF0F2", color: "#5A6472" }}>{b.descricao || ""}</td>
                          <td style={{ padding: "8px 12px", borderTop: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(b.novoVencimento || b.dataVencimento)}</td>
                          <td style={{ padding: "8px 12px", borderTop: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(b.dataPagamento)}</td>
                          <td style={{ padding: "8px 12px", borderTop: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700 }}>{BRL(Number(b.valor) || 0)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      {relatorioGastosOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ width: "min(980px, 96vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 22 }}>
