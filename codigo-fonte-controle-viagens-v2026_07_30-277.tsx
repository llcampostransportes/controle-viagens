...
const APP_VERSION = "2026.07.30-277";
...
const CAMINHOES_HEADER = ["id", "placa", "semAlertaOleo", "intervaloOleoKm", "tipo", "modelo", "foto", "renavam", "chassi", "anoFabricacao", "anoModelo", "cor", "combustivel", "categoria", "municipio", "uf", "vencimentoDocumento", "documentosPorAnoJSON", "crv", "ipvaPagoPorAnoJSON", "carretaVinculadaId", "eixos"];
...
const BOLETOS_HEADER = ["id", "empresa", "descricao", "notaFiscal", "valor", "dataVencimento", "contaBancaria", "dataPagamento", "observacao", "desconto", "descontoDescricao", "juros", "baixasJSON", "codigoBarras", "linhaDigitavel", "novoVencimento", "notaFiscalData"];
...
const TROCAS_OLEO_HEADER = ["id", "caminhaoId", "data", "km", "filtroTrocado", "observacao", "valor", "empresa", "pecas"];
const SERVICOS_VEICULO_HEADER = ["id", "caminhaoId", "data", "km", "tipoServico", "observacao", "empresa", "itens", "pecas"];
...
const MULTAS_HEADER = ["id", "caminhaoId", "motorista", "data", "descricao", "valor", "dataVencimento", "dataPagamento", "descontadoMotorista", "observacao"];
const FINANCEIRO_PESSOAL_HEADER = ["id", "tipo", "descricao", "pessoa", "valor", "dataVencimento", "dataPagamento", "observacao", "titular", "ondeComprou", "notaFiscalNumero", "notaFiscalData"];
...
async function ensureSheetsExist(token) {
  ...
  const missing = ["Caminhoes", "Viagens", "Vales", "Boletos", "Empresas", "Fechamentos", "DespesasVeiculo", "TaxasPool", "Motoristas", "Contas", "TrocasOleo", "ServicosVeiculo", "SemParar", "Seguro", "SemPararOutros", "FinanceiroPessoal", "Multas", "Config", "Backups"].filter((t) => !titles.includes(t));
  if (missing.length > 0) {
  ...
  await sheetsFetch(`/values/Caminhoes!A1:V1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Caminhoes!A1:V1", values: [CAMINHOES_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/Boletos!A1:Q1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Boletos!A1:Q1", values: [BOLETOS_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/TrocasOleo!A1:I1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "TrocasOleo!A1:I1", values: [TROCAS_OLEO_HEADER] }),
  });
  await sheetsFetch(`/values/ServicosVeiculo!A1:I1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "ServicosVeiculo!A1:I1", values: [SERVICOS_VEICULO_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/FinanceiroPessoal!A1:L1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "FinanceiroPessoal!A1:L1", values: [FINANCEIRO_PESSOAL_HEADER] }),
  });
  await sheetsFetch(`/values/Multas!A1:J1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Multas!A1:J1", values: [MULTAS_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/Backups!A1:C1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Backups!A1:C1", values: [["data", "criadoEm", "conteudo"]] }),
  });
}
...
function rowToTruck(row) {
  ...
  return {
    ...
    carretaVinculadaId: row[20] || "", eixos: row[21] || "5",
  };
}
function truckToRow(t) {
  return [
    ...
    t.carretaVinculadaId || "", t.eixos || "5",
  ];
}
...
// Financeiro Pessoal: contas a pagar/receber soltas, sem ligação nenhuma
// com viagens, boletos ou o dashboard — só um controle à parte
function rowToMulta(row) {
  return {
    id: row[0] || "",
    caminhaoId: row[1] || "",
    motorista: row[2] || "",
    ...
    descricao: row[4] || "",
    valor: row[5] || "",
    dataVencimento: row[6] || "",
    dataPagamento: row[7] || "",
    descontadoMotorista: row[8] === "sim",
    observacao: row[9] || "",
  };
}
function multaToRow(m) {
  return [m.id, m.caminhaoId || "", m.motorista || "", m.data, m.descricao || "", m.valor, m.dataVencimento || "", m.dataPagamento || "", m.descontadoMotorista ? "sim" : "", m.observacao || ""];
}

function rowToFinanceiroPessoal(row) {
  return {
    id: row[0] || "",
    tipo: row[1] || "pagar", // pagar | receber
    descricao: row[2] || "",
    pessoa: row[3] || "",
    valor: row[4] || "",
    ...
    titular: row[8] || "",
    ondeComprou: row[9] || "",
    notaFiscalNumero: row[10] || "",
    notaFiscalData: row[11] || "",
  };
}
function financeiroPessoalToRow(f) {
  return [f.id, f.tipo, f.descricao || "", f.pessoa || "", f.valor, f.dataVencimento || "", f.dataPagamento || "", f.observacao || "", f.titular || "", f.ondeComprou || "", f.notaFiscalNumero || "", f.notaFiscalData || ""];
}
...
function rowToTrocaOleo(row) {
  return {
    ...
    valor: row[6] || "",
    empresa: row[7] || "",
    pecas: safeParseJSON(row[8], []),
  };
}
function trocaOleoToRow(t) {
  return [t.id, t.caminhaoId, t.data, t.km, t.filtroTrocado ? "sim" : "", t.observacao || "", t.valor || "", t.empresa || "", JSON.stringify(t.pecas || [])];
}
...
function rowToServicoVeiculo(row) {
  return {
    ...
    observacao: row[5] || "",
    empresa: row[6] || "",
    itens: safeParseJSON(row[7], []),
    pecas: safeParseJSON(row[8], []),
  };
}
function servicoVeiculoToRow(s) {
  return [s.id, s.caminhaoId, s.data, s.km, s.tipoServico, s.observacao || "", s.empresa || "", JSON.stringify(s.itens || []), JSON.stringify(s.pecas || [])];
}
...
function rowToBoleto(row) {
  return {
    ...
    notaFiscalData: row[16] || "",
  };
}
function boletoToRow(b) {
  return [b.id, b.empresa, b.descricao || "", b.notaFiscal, b.valor, b.dataVencimento, b.contaBancaria, b.dataPagamento, b.observacao || "", b.desconto || "", b.descontoDescricao || "", b.juros || "", JSON.stringify(b.baixas || []), b.codigoBarras || "", b.linhaDigitavel || "", b.novoVencimento || "", b.notaFiscalData || ""];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, financeiroPessoalRes, multasRes, configRes] = await Promise.all([
    sheetsFetch(`/values/Caminhoes!A2:V`, token),
    sheetsFetch(`/values/Viagens!A2:X`, token),
    sheetsFetch(`/values/Vales!A2:H`, token),
    sheetsFetch(`/values/Boletos!A2:Q`, token),
    ...
    sheetsFetch(`/values/TrocasOleo!A2:I`, token),
    sheetsFetch(`/values/ServicosVeiculo!A2:I`, token),
    sheetsFetch(`/values/SemParar!A2:H`, token),
    sheetsFetch(`/values/Seguro!A2:L`, token),
    sheetsFetch(`/values/SemPararOutros!A2:E`, token),
    sheetsFetch(`/values/FinanceiroPessoal!A2:L`, token),
    sheetsFetch(`/values/Multas!A2:J`, token),
  ...
  const financeiroPessoal = (financeiroPessoalRes.values || [])
    .filter((row) => row[0])
    .map(rowToFinanceiroPessoal);
  const multas = (multasRes.values || [])
    .filter((row) => row[0])
    .map(rowToMulta);
  const config = (configRes.values || [])
    .filter((row) => row[0])
    .map(rowToConfig);
  return { trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, financeiroPessoal, multas, config };
}

// Cada aba tem: nome, cabeçalho, e a faixa de colunas usada (pra limpar sobras).
const SHEET_DEFS = {
  trucks: { nome: "Caminhoes", header: CAMINHOES_HEADER, colunas: "V", toRow: truckToRow },
  ...
  boletos: { nome: "Boletos", header: BOLETOS_HEADER, colunas: "Q", toRow: boletoToRow },
  ...
  trocasOleo: { nome: "TrocasOleo", header: TROCAS_OLEO_HEADER, colunas: "I", toRow: trocaOleoToRow },
  servicosVeiculo: { nome: "ServicosVeiculo", header: SERVICOS_VEICULO_HEADER, colunas: "I", toRow: servicoVeiculoToRow },
  ...
  financeiroPessoal: { nome: "FinanceiroPessoal", header: FINANCEIRO_PESSOAL_HEADER, colunas: "L", toRow: financeiroPessoalToRow },
  multas: { nome: "Multas", header: MULTAS_HEADER, colunas: "J", toRow: multaToRow },
...
const emptyGasto = () => ({ id: uid(), data: "", valor: "", descricao: "", categoria: "Despesas Estrada", setor: "", posto: "", numeroCupom: "", paraComissao: false, importante: false, empresa: "", estabelecimento: "", km: "", gerarBoleto: false, vencimentoBoleto: "" });
...
/* ---------- field ---------- */
// campo de texto com sugestões — feito na mão em vez de usar <datalist>,
// porque o datalist nativo não mostra as opções no Safari do iPad (funciona
// normal no computador, mas no iPad simplesmente não aparece nada)
function CampoComSugestoes({ value, onChange, onBlur, onKeyDown, opcoes, placeholder, style }) {
  const [aberto, setAberto] = useState(false);
  const valorAtual = value || "";
  const filtradas = (opcoes || [])
    .filter((o) => o.toLowerCase().includes(valorAtual.toLowerCase()))
    .slice(0, 8);
  return (
    <div style={{ position: "relative" }}>
      <input
        ...
        onBlur={(e) => { setAberto(false); if (onBlur) onBlur(e); }}
        onKeyDown={onKeyDown}
      ...
      {aberto && filtradas.length > 0 && (
        <div
          ...
          style={{
            ...
            boxShadow: "0 8px 24px rgba(27,36,48,0.14)", maxHeight: 190, overflowY: "auto",
          }}
        >
          {filtradas.map((op) => (
            <div
              ...
              style={{ padding: "9px 12px", fontSize: 13, cursor: "pointer", borderBottom: "1px solid #EEF0F2" }}
            >
              {op}
            </div>
          ))}
          ...
          {ordemCategorias.map((categoria) => {
            ...
            return (
              <div key={categoria} style={{ marginBottom: 4 }}>
                <div style={{ background: "#F7F8F9", borderRadius: 10, display: "flex", alignItems: "center", gap: 4, padding: "2px 2px 2px 8px" }}>
                  <button
                    onClick={() => setCategoriasFechadas((prev) => ({ ...prev, [categoria]: !prev[categoria] }))}
                    style={{ background: "none", border: "none", padding: "4px 0", cursor: "pointer", color: "#9AA5B1" }}
                  >
                    <span style={{ display: "inline-block", transform: fechada ? "rotate(-90deg)" : "none", transition: "transform 0.15s" }}>▾</span>
                  </button>
                  <button
                    onClick={() => {
                      const todasMarcadas = marcadasDaCategoria === nomesDaCategoria.length;
                      nomesDaCategoria.forEach((nome) => {
                        const marcada = empresasSelecionadas.includes(nome);
                        if (todasMarcadas && marcada) onToggle(nome);
                        if (!todasMarcadas && !marcada) onToggle(nome);
                      });
                    }}
                    title="Marcar/desmarcar todas dessa categoria"
                    style={{ flex: 1, background: "none", border: "none", padding: "6px 4px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 12, color: "#3A4351", textAlign: "left", textTransform: "uppercase", letterSpacing: 0.3 }}
                  >
                    {categoria}
...
function BoletosView({
  ...
  const [novoNotaFiscalData, setNovoNotaFiscalData] = useState("");
  ...
  const visibleBoletos = useMemo(() => {
    return report.items
      ...
      .filter((b) => statusFilter === "all" || (statusFilter === "aberto" ? (b.__status === "pendente" || b.__status === "vencido") : b.__status === statusFilter))
  ...
  const openEdit = (b) => {
    ...
    const editingInicial = { ...bLimpo, baixas: bLimpo.baixas || [] };
    setEditing(editingInicial);
    originalEditingRef.current = JSON.stringify(editingInicial);
  ...
  const confirmarSalvarPreview = () => {
    ...
    const novosBoletos = parcelasPreview.map((p) => ({
      ...
      notaFiscalData: novoNotaFiscalData,
      valor: p.valor,
  ...
  const fecharPainel = () => {
    if (editing && JSON.stringify(editing) !== originalEditingRef.current) {
      if (!window.confirm("Você tem alterações não salvas nesse boleto (ex: uma baixa/pagamento adicionado). Se sair agora, elas serão perdidas. Sair mesmo assim?")) {
        return;
      }
    }
    setPanelOpen(false);
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-end", flexWrap: "wrap", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: 14, marginBottom: 14 }}>
        ...
        <Field label="Status">
          <select style={inputStyle} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
            ...
            <option value="aberto">Pendente + Vencido</option>
      ...
      {visibleBoletos.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {visibleBoletos.map((b) => {
            ...
            const fundoTom = status === "pago" ? "#E9F5F1" : status === "vencido" ? "#FBEBE8" : "#FFF6E2";
            return (
              <div
                key={b.__key}
                onClick={() => openEdit(b)}
                style={{
                  background: `linear-gradient(160deg, #fff 0%, ${fundoTom} 100%)`, borderRadius: 12, padding: "12px 16px",
      ...
      {panelOpen && editing && (
        <>
          <div onClick={fecharPainel} style={{ position: "fixed", inset: 0, background: "rgba(27,36,48,0.45)", zIndex: 20 }} />
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(480px, 100vw)", background: "#fff", zIndex: 21, overflowY: "auto", boxShadow: "-8px 0 24px rgba(0,0,0,0.15)", padding: "24px 24px 100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button onClick={fecharPainel} style={{ background: "none", border: "none", fontSize: 26, cursor: "pointer", color: "#5A6472", padding: "10px", margin: "-10px", lineHeight: 1 }}>×</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
              ...
              <Field label="Data da nota">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.notaFiscalData || ""} onChange={(e) => setEditing({ ...editing, notaFiscalData: e.target.value })} onBlur={(e) => setEditing({ ...editing, notaFiscalData: corrigirAnoDigitado(e.target.value) })} />
              </Field>
            ...
            {(Number(editing.desconto) > 0 || Number(editing.juros) > 0 || totalBaixasBoleto(editing) > 0) && (
              <div style={{ background: "#F7F8F9", borderRadius: 12, padding: 10, marginBottom: 16, fontSize: 13 }}>
                ...
                {totalBaixasBoleto(editing) > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", color: "#8A5A00" }}>
                    <span>Baixas já pagas</span>
                    <span>−{BRL(totalBaixasBoleto(editing))}</span>
                  </div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, borderTop: "1px solid #D7DBE0", marginTop: 4, paddingTop: 4 }}>
                  <span>Saldo restante (falta pagar)</span>
        ...
        <>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(560px, 100vw)", background: "#fff", zIndex: 21, overflowY: "auto", boxShadow: "-8px 0 24px rgba(0,0,0,0.15)", padding: "24px 24px 100px" }}>
            ...
            {leitorParcelaId && (
              <LeitorCodigoBarras
                ...
                onDetectado={(codigo) => {
                  ...
                  if (parcelasPreview !== null && parcelasPreview.some((p) => p.id === idAlvo)) {
                    setParcelasPreview((prev) => prev.map(aplicar));
                  } else {
                    setNovoParcelas((prev) => prev.map(aplicar));
                  }
                }}
              />
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 12 }}>
              ...
              <Field label="Data da nota">
                <input type="date" style={inputStyle} value={novoNotaFiscalData} onChange={(e) => setNovoNotaFiscalData(e.target.value)} onBlur={(e) => setNovoNotaFiscalData(corrigirAnoDigitado(e.target.value))} />
              </Field>
            </div>
            {novoEmpresaCustom && (
              <button
              ...
              return (
                <>
                  ...
                  {parcelasPreview.map((p, i) => (
                    <div key={p.id} style={{ background: "#F7F8F9", borderRadius: 12, padding: 10, marginBottom: 8 }}>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
                        <button
                          onClick={() => setLeitorParcelaId(p.id)}
                          style={{ background: "#2451A6", color: "#fff", border: "none", borderRadius: 8, padding: "6px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}
                        >
                          📷 {p.codigoBarras ? "Ler de novo" : "Ler código de barras"}
                        </button>
                        {(p.linhaDigitavel || p.codigoBarras) && (
                          <div style={{ fontSize: 11, color: "#5A6472", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", wordBreak: "break-all" }}>
                            {p.linhaDigitavel || p.codigoBarras}
                          </div>
                        )}
                      </div>
      ...
      {reportOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="modal-relatorio" style={{ width: "min(1300px, 98vw)", maxHeight: "94vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            ...
            {report.items.length === 0 ? (
              ...
              <>
                <div style={{ overflowX: "auto", border: "1px solid #E4E7EB", borderRadius: 10 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {report.items.map((b, i) => {
                      ...
                      const corStatus = b.__kind === "pago" ? "#1F6F5C" : b.__status === "vencido" ? "#B0402E" : "#8A5A00";
                      const bgStatus = b.__kind === "pago" ? "#E9F5F1" : b.__status === "vencido" ? "#FBEBE8" : "#FFF6E2";
                      return (
                        <tr key={b.__key} style={{ borderBottom: "1px solid #EEF0F2", background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                          ...
                          <td style={{ padding: "8px 10px", textAlign: "center" }}>
                            <span style={{ background: bgStatus, color: corStatus, fontWeight: 700, fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>{status.toUpperCase()}</span>
                          </td>
                ...
                <div style={{ display: "flex", gap: 16, fontSize: 11, color: "#5A6472", margin: "10px 2", flexWrap: "wrap" }}>
                  <span>🔴 Vencido: boleto com vencimento passado</span>
                  <span>🟠 Pendente: boleto a vencer</span>
                  <span>🟢 A vencer: próximo a vencer</span>
                  <span>🔵 Pago: quitado</span>
                </div>

                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: "1 1 140px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 700 }}>💲 RESUMO FINANCEIRO</div>
                  </div>
                  <div style={{ flex: "1 1 140px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 700 }}>VENCIDOS</div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#B0402E" }}>{BRL(report.totals.vencido)}</div>
                  </div>
                  <div style={{ flex: "1 1 140px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 700 }}>PENDENTES</div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#8A5A00" }}>{BRL(report.totals.pendente)}</div>
                  </div>
                  <div style={{ flex: "1 1 140px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 700 }}>A VENCER</div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#1F6F5C" }}>{BRL(report.totals.aVencer)}</div>
                  </div>
                  <div style={{ flex: "1 1 150px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 10, opacity: 0.85, fontWeight: 700 }}>TOTAL EM ABERTO</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{BRL(report.totals.vencido + report.totals.pendente + report.totals.aVencer)}</div>
                  </div>
                </div>
              </>
            )}
...
export default function App() {
  ...
  // mapeia cada tela pro módulo dela na barra lateral nova — assim, mesmo quando
  // algum atalho (tipo um alerta) muda a view direto, o destaque do módulo
  // certo na lateral continua batendo
  useEffect(() => {
    const modulosPorView = {
      dashboard: "dashboard", dashboardGeral: "dashboard",
      viagens: "operacao", trocaoleo: "operacao", semparar: "operacao", multas: "operacao",
      boletos: "financeiro", financeiropessoal: "financeiro",
      seguro: "frota",
    };
    if (modulosPorView[view]) setModuloAtivo(modulosPorView[view]);
  }, [view]);
  ...
  const [financeiroPessoal, setFinanceiroPessoal] = useState([]);
  const [multas, setMultas] = useState([]);
  const [config, setConfig] = useState([]);
  const senhaAppSalva = (config.find((c) => c.chave === "senhaApp") || {}).valor || "";
  const [configOpen, setConfigOpen] = useState(false);
  const [configSecaoAberta, setConfigSecaoAberta] = useState(null); // null = grid de cards | geral | seguradoras | caminhoes | ...
  const [backupsAutomaticos, setBackupsAutomaticos] = useState(null); // null = ainda não carregado
  ...
  const [rotasPedagio, setRotasPedagio] = useState([]);
  const salvarRotasPedagio = (novaLista) => {
    setRotasPedagio(novaLista);
    const nextConfig = [...config.filter((c) => c.chave !== "rotasPedagio"), { chave: "rotasPedagio", valor: JSON.stringify(novaLista) }];
  ...
  const [novaRotaDestino, setNovaRotaDestino] = useState("");
  const [novaRotaValor5, setNovaRotaValor5] = useState("");
  const [novaRotaValor6, setNovaRotaValor6] = useState("");
  const [empresasComPedagio, setEmpresasComPedagio] = useState([]);
  const salvarEmpresasComPedagio = (novaLista) => {
    setEmpresasComPedagio(novaLista);
    const nextConfig = [...config.filter((c) => c.chave !== "empresasComPedagio"), { chave: "empresasComPedagio", valor: JSON.stringify(novaLista) }];
    setConfig(nextConfig);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, nextConfig);
  };
  const [novaEmpresaComPedagio, setNovaEmpresaComPedagio] = useState("");
  const [metaFaturamentoMensal, setMetaFaturamentoMensal] = useState("");
  const salvarMetaFaturamento = (valor) => {
    setMetaFaturamentoMensal(valor);
    const nextConfig = [...config.filter((c) => c.chave !== "metaFaturamentoMensal"), { chave: "metaFaturamentoMensal", valor: String(valor) }];
    setConfig(nextConfig);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, nextConfig);
  };
  ...
  const precoFocoRef = useRef({});
  const [postosComHistoricoAberto, setPostosComHistoricoAberto] = useState({});
  const registrarHistoricoPreco = (id, campo, valorAntigo, valorNovo) => {
    const antigo = (valorAntigo || "").toString().trim();
    const novo = (valorNovo || "").toString().trim();
    if (antigo === novo) return;
    const rotulo = campo === "precoArla" ? "Arla" : "Diesel";
    salvarPrecosPostos(
      precosPostos.map((p) =>
        p.id === id
          ? {
              ...p,
              historico: [
                { id: uid(), data: todayISO(), campo: rotulo, valorAntigo: antigo, valorNovo: novo },
                ...(p.historico || []),
              ],
            }
          : p
  ...
  // qual dos 6 módulos da barra lateral está em destaque — controla qual
  // submenu horizontal aparece embaixo do cabeçalho
  ...
  const [servicosPlacaFiltro, setServicosPlacaFiltro] = useState("all");
  ...
  const relatorioServicos = useMemo(() => {
    ...
    servicosVeiculo.forEach((s) => {
      if (s.empresa) {
        linhas.push({
          ...
          data: s.data, km: s.km || "", servico: s.tipoServico || "Serviço", empresa: s.empresa,
          valor: (s.itens || []).reduce((sum, i) => sum + (Number(i.valor) || 0), 0) + (s.pecas || []).reduce((sum, p) => sum + (Number(p.valor) || 0), 0),
          origem: "servico",
        });
      }
    });
    trocasOleo.forEach((t) => {
      if (t.empresa) {
        const totalPecas = (t.pecas || []).reduce((sum, p) => sum + (Number(p.valor) || 0), 0);
        linhas.push({
          id: t.id, tripId: null, caminhaoId: t.caminhaoId, placa: truckLabel(t.caminhaoId),
          data: t.data, km: t.km || "", servico: "Troca de óleo", empresa: t.empresa,
          valor: (Number(t.valor) || 0) + totalPecas,
          origem: "trocaOleo",
        });
      }
    });
    ...
    linhas = linhas
      ...
      .filter((l) => servicosPlacaFiltro === "all" || l.caminhaoId === servicosPlacaFiltro)
      .sort((a, b) => (b.data || "").localeCompare(a.data || ""));
    const porPlacaMap = {};
    linhas.forEach((l) => {
      if (!porPlacaMap[l.caminhaoId]) porPlacaMap[l.caminhaoId] = { caminhaoId: l.caminhaoId, placa: l.placa, items: [], total: 0 };
      porPlacaMap[l.caminhaoId].items.push(l);
      porPlacaMap[l.caminhaoId].total += l.valor;
    });
    const porPlaca = Object.values(porPlacaMap).sort((a, b) => a.placa.localeCompare(b.placa));
    return { items: linhas, porPlaca };
  }, [trips, servicosVeiculo, trocasOleo, servicosPeriodStart, servicosPeriodEnd, servicosEmpresaFiltro, servicosPlacaFiltro]);
  ...
  // abre um relatório fechando todos os outros — sem isso, dava pra abrir um
  // relatório novo sem fechar o anterior, e na hora de gerar o PDF os dois
  // ficavam "prontos pra imprimir" ao mesmo tempo, saindo misturado
  const [relatorioContadorOpen, setRelatorioContadorOpen] = useState(false);
  const [contadorMes, setContadorMes] = useState(() => todayISO().slice(0, 7));
  const abrirRelatorio = (qual) => {
    setReportOpen(qual === "mensal");
    setRelatorioRecebimentoOpen(qual === "recebimento");
    setRelatorioGastosOpen(qual === "gastos");
    setRelatorioConsumoOpen(qual === "consumo");
    setRelatorioServicosOpen(qual === "servicos");
    setBoletosReportOpen(qual === "boletos");
    setRelatorioContadorOpen(qual === "contador");
  };
  ...
  const [trocaOleoValor, setTrocaOleoValor] = useState("80");
  const [trocaOleoPecas, setTrocaOleoPecas] = useState([]);
  const [trocaOleoEmpresa, setTrocaOleoEmpresa] = useState("");
  const [trocaOleoGerarBoleto, setTrocaOleoGerarBoleto] = useState(false);
  const [trocaOleoVencimentoBoleto, setTrocaOleoVencimentoBoleto] = useState("");
  ...
  const startAddTrocaOleo = (caminhaoIdPreSelecionado) => {
    ...
    setTrocaOleoObs("");
    setTrocaOleoValor("80");
    setTrocaOleoPecas([]);
    setTrocaOleoEmpresa(boletosEmpresas.includes("FORÇA DIESEL") ? "FORÇA DIESEL" : (boletosEmpresas.find((n) => n.toUpperCase().includes("FORÇA DIESEL")) || ""));
    setTrocaOleoGerarBoleto(false);
    setTrocaOleoVencimentoBoleto("");
  };
  ...
  const startEditTrocaOleo = (t) => {
    ...
    setTrocaOleoValor(t.valor || "80");
    setTrocaOleoPecas(t.pecas || []);
    setTrocaOleoEmpresa(t.empresa || "");
    setTrocaOleoGerarBoleto(false);
    setTrocaOleoVencimentoBoleto("");
  };
  ...
  const confirmAddTrocaOleo = () => {
    ...
    const pecasValidas = trocaOleoPecas.filter((p) => p.descricao.trim());
    const totalComPecas = (Number(trocaOleoValor) || 0) + pecasValidas.reduce((s, p) => s + (Number(p.valor) || 0), 0);
    if (trocaOleoGerarBoleto && (!trocaOleoEmpresa || !trocaOleoVencimentoBoleto || !(totalComPecas > 0))) {
      alert("Pra gerar o boleto, preencha a empresa, o valor e o vencimento.");
      return;
    }
    if (editingTrocaOleoId) {
      updateTrocaOleo(editingTrocaOleoId, trocaOleoCaminhaoId, trocaOleoData, trocaOleoKm, trocaOleoFiltro, trocaOleoObs.trim(), trocaOleoValor, trocaOleoEmpresa, pecasValidas);
    } else {
      addTrocaOleo(trocaOleoCaminhaoId, trocaOleoData, trocaOleoKm, trocaOleoFiltro, trocaOleoObs.trim(), trocaOleoValor, trocaOleoEmpresa, pecasValidas);
    }
    if (trocaOleoGerarBoleto && trocaOleoEmpresa) {
      const novoBoleto = {
        id: uid(),
        empresa: trocaOleoEmpresa,
        descricao: `Troca de óleo — ${truckLabel(trocaOleoCaminhaoId)}`,
        notaFiscal: "",
        valor: String(totalComPecas),
        dataVencimento: trocaOleoVencimentoBoleto,
        contaBancaria: "",
        dataPagamento: "",
        observacao: `Gerado automaticamente a partir de troca de óleo do caminhão ${truckLabel(trocaOleoCaminhaoId)} em ${fmtDate(trocaOleoData)}`,
        desconto: "",
        descontoDescricao: "",
        juros: "",
        baixas: [],
  ...
  const exportTrocaOleoCSV = () => {
    const header = ["Placa", "Data", "KM", "Filtro trocado", "Observação", "Valor mão de obra", "Peças (descrição:valor)", "Valor total", "Empresa"];
    const lines = [header.join(";")];
    trocaOleoReport.items.forEach((t) => {
      const totalPecas = (t.pecas || []).reduce((s, p) => s + (Number(p.valor) || 0), 0);
      const pecasTexto = (t.pecas || []).map((p) => `${p.descricao}:${p.valor}`).join(", ");
      lines.push([truckLabel(t.caminhaoId), fmtDate(t.data), t.km, t.filtroTrocado ? "Sim" : "Não", t.observacao || "", t.valor || "", pecasTexto, ((Number(t.valor) || 0) + totalPecas).toFixed(2), t.empresa || ""].join(";"));
    });
  ...
  const [servicoItens, setServicoItens] = useState([{ id: uid(), descricao: "", valor: "" }]);
  const [servicoPecas, setServicoPecas] = useState([]);
  ...
  const startAddServico = (caminhaoIdPreSelecionado) => {
    ...
    setServicoItens([{ id: uid(), descricao: "", valor: "" }]);
    setServicoPecas([]);
    setServicoObs("");
    setServicoEmpresa(boletosEmpresas.includes("FORÇA DIESEL") ? "FORÇA DIESEL" : (boletosEmpresas.find((n) => n.toUpperCase().includes("FORÇA DIESEL")) || ""));
  ...
  const startEditServico = (s) => {
    ...
    if (s.itens && s.itens.length > 0) {
      setServicoItens(s.itens);
    } else {
      const partes = (s.tipoServico || "").split(",").map((p) => p.trim()).filter(Boolean);
      setServicoItens(partes.length > 0 ? partes.map((descricao) => ({ id: uid(), descricao, valor: "" })) : [{ id: uid(), descricao: "", valor: "" }]);
    }
    setServicoObs(s.observacao || "");
    setServicoEmpresa(s.empresa || "");
    setServicoPecas(s.pecas || []);
    setServicoValorBoleto("");
    setServicoGerarBoleto(false);
    setServicoVencimentoBoleto("");
  };
  ...
  const confirmAddServico = () => {
    const itensValidos = servicoItens.filter((i) => i.descricao.trim());
    const pecasValidas = servicoPecas.filter((p) => p.descricao.trim());
    if (!servicoCaminhaoId || !servicoData || itensValidos.length === 0) {
      alert("Preencha o caminhão, a data e pelo menos uma descrição de serviço.");
      return;
    }
    const tipoServicoResumo = itensValidos.map((i) => i.descricao.trim()).join(", ");
    const totalGeral =
      itensValidos.reduce((s, i) => s + (Number(i.valor) || 0), 0) +
      pecasValidas.reduce((s, p) => s + (Number(p.valor) || 0), 0);
    if (servicoGerarBoleto && (!servicoEmpresa || !servicoVencimentoBoleto || !(Number(servicoValorBoleto) > 0))) {
      alert("Pra gerar o boleto, preencha a empresa, o valor e o vencimento.");
      return;
    }
    if (editingServicoId) {
      updateServicoVeiculo(editingServicoId, servicoCaminhaoId, servicoData, servicoKm, tipoServicoResumo, servicoObs.trim(), servicoEmpresa, itensValidos, pecasValidas);
    } else {
      addServicoVeiculo(servicoCaminhaoId, servicoData, servicoKm, tipoServicoResumo, servicoObs.trim(), servicoEmpresa, itensValidos, pecasValidas);
    }
    if (servicoGerarBoleto && servicoEmpresa) {
      const novoBoleto = {
        id: uid(),
        empresa: servicoEmpresa,
        descricao: tipoServicoResumo || "Serviço no caminhão",
        notaFiscal: "",
  ...
  const servicoVeiculoReport = useMemo(() => {
    const doServicoVeiculo = servicosVeiculo.map((s) => ({
      id: s.id, caminhaoId: s.caminhaoId, data: s.data, km: s.km, tipoServico: s.tipoServico, observacao: s.observacao, empresa: s.empresa || "", pecas: s.pecas || [],
      valor: (s.itens || []).reduce((sum, i) => sum + (Number(i.valor) || 0), 0) + (s.pecas || []).reduce((sum, p) => sum + (Number(p.valor) || 0), 0), origem: "servico",
    }));
      ...
      (t.gastosExtras || []).forEach((g) => {
        if (g.importante) {
          dosGastosExtras.push({
            id: g.id, caminhaoId: t.caminhaoId, data: g.data || t.data, km: g.km || "", tipoServico: g.descricao || "Serviço", observacao: g.empresa ? `Feito em ${g.empresa}` : "Feito na estrada", empresa: g.empresa || "", valor: Number(g.valor) || 0, origem: "gasto", tripId: t.id,
          });
        }
      });
    });
    const filtered = [...doServicoVeiculo, ...dosGastosExtras]
      .filter((s) => s.data >= oleoPeriodStart && s.data <= oleoPeriodEnd)
      .filter((s) => oleoPlacaFilter === "all" || s.caminhaoId === oleoPlacaFilter)
      .sort((a, b) => (b.data || "").localeCompare(a.data || ""));
    return { items: filtered };
  }, [servicosVeiculo, trips, oleoPeriodStart, oleoPeriodEnd, oleoPlacaFilter]);

  const exportServicoVeiculoCSV = () => {
    const header = ["Placa", "Data", "KM", "Tipo de Serviço", "Observação", "Empresa", "Valor"];
    const lines = [header.join(";")];
    servicoVeiculoReport.items.forEach((s) => {
      lines.push([truckLabel(s.caminhaoId), fmtDate(s.data), s.km || "", s.tipoServico, s.observacao || "", s.empresa || "", (s.valor || 0).toFixed(2)].join(";"));
    });
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `outros-servicos-${oleoPeriodStart}-a-${oleoPeriodEnd}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  // ---- Financeiro Pessoal ----
  const TITULARES_FINANCEIRO = ["Laís", "Lucas", "Crianças"];

  // ---- Multas de trânsito ----
  const [addingMulta, setAddingMulta] = useState(false);
  const [editingMultaId, setEditingMultaId] = useState(null);
  const [multaCaminhaoId, setMultaCaminhaoId] = useState("");
  const [multaMotorista, setMultaMotorista] = useState("");
  const [multaData, setMultaData] = useState("");
  const [multaDescricao, setMultaDescricao] = useState("");
  const [multaValor, setMultaValor] = useState("");
  const [multaVencimento, setMultaVencimento] = useState("");
  const [multaPagamento, setMultaPagamento] = useState("");
  const [multaDescontado, setMultaDescontado] = useState(false);
  const [multaObs, setMultaObs] = useState("");
  const [multaFiltroCaminhao, setMultaFiltroCaminhao] = useState("all");
  const [multaFiltroStatus, setMultaFiltroStatus] = useState("all");
  const [multaPeriodStart, setMultaPeriodStart] = useState("2000-01-01");
  const [multaPeriodEnd, setMultaPeriodEnd] = useState("2100-01-01");

  const startAddMulta = () => {
    setAddingMulta(true);
    setEditingMultaId(null);
    setMultaCaminhaoId((trucks[0] && trucks[0].id) || "");
    setMultaMotorista("");
    setMultaData(new Date().toISOString().slice(0, 10));
    setMultaDescricao("");
    setMultaValor("");
    setMultaVencimento("");
    setMultaPagamento("");
    setMultaDescontado(false);
    setMultaObs("");
  };
  const startEditMulta = (m) => {
    setAddingMulta(true);
    setEditingMultaId(m.id);
    setMultaCaminhaoId(m.caminhaoId || "");
    setMultaMotorista(m.motorista || "");
    setMultaData(m.data || "");
    setMultaDescricao(m.descricao || "");
    setMultaValor(m.valor || "");
    setMultaVencimento(m.dataVencimento || "");
    setMultaPagamento(m.dataPagamento || "");
    setMultaDescontado(!!m.descontadoMotorista);
    setMultaObs(m.observacao || "");
  };
  const confirmAddMulta = () => {
    if (!multaData || !multaDescricao.trim() || !(Number(multaValor) > 0)) {
      alert("Preencha a data, a descrição e o valor.");
      return;
    }
    const registro = {
      caminhaoId: multaCaminhaoId,
      motorista: multaMotorista.trim(),
      data: multaData,
      descricao: multaDescricao.trim(),
      valor: multaValor,
      dataVencimento: multaVencimento,
      dataPagamento: multaPagamento,
      descontadoMotorista: multaDescontado,
      observacao: multaObs.trim(),
    };
    if (editingMultaId) updateMulta(editingMultaId, registro);
    else addMulta(registro);
    setAddingMulta(false);
    setEditingMultaId(null);
  };

  const multaReport = useMemo(() => {
    const hoje = todayISO();
    let itens = multas.filter((m) => (m.data || "") >= multaPeriodStart && (m.data || "") <= multaPeriodEnd);
    if (multaFiltroCaminhao !== "all") itens = itens.filter((m) => m.caminhaoId === multaFiltroCaminhao);
    if (multaFiltroStatus !== "all") itens = itens.filter((m) => (multaFiltroStatus === "pago" ? !!m.dataPagamento : !m.dataPagamento));
    itens = [...itens].sort((a, b) => (b.data || "").localeCompare(a.data || ""));
    const totalPendente = itens.filter((m) => !m.dataPagamento).reduce((s, m) => s + (Number(m.valor) || 0), 0);
    const totalPago = itens.filter((m) => m.dataPagamento).reduce((s, m) => s + (Number(m.valor) || 0), 0);
    const totalVencido = itens.filter((m) => !m.dataPagamento && m.dataVencimento && m.dataVencimento < hoje).reduce((s, m) => s + (Number(m.valor) || 0), 0);
    return { itens, totalPendente, totalPago, totalVencido };
  }, [multas, multaPeriodStart, multaPeriodEnd, multaFiltroCaminhao, multaFiltroStatus]);
  const [addingFinanceiro, setAddingFinanceiro] = useState(false);
  const [editingFinanceiroId, setEditingFinanceiroId] = useState(null);
  const [financeiroTipo, setFinanceiroTipo] = useState("pagar");
  const [financeiroDescricao, setFinanceiroDescricao] = useState("");
  const [financeiroPessoaNome, setFinanceiroPessoaNome] = useState("");
  const [financeiroValor, setFinanceiroValor] = useState("");
  const [financeiroVencimento, setFinanceiroVencimento] = useState("");
  const [financeiroDataPagamento, setFinanceiroDataPagamento] = useState("");
  const [financeiroObs, setFinanceiroObs] = useState("");
  const [financeiroTitular, setFinanceiroTitular] = useState("");
  const [financeiroOndeComprou, setFinanceiroOndeComprou] = useState("");
  const [financeiroNotaNumero, setFinanceiroNotaNumero] = useState("");
  const [financeiroNotaData, setFinanceiroNotaData] = useState("");
  const [financeiroParcelar, setFinanceiroParcelar] = useState(false);
  const [financeiroNumParcelas, setFinanceiroNumParcelas] = useState("2");
  const [financeiroIntervaloDias, setFinanceiroIntervaloDias] = useState("30");
  const [financeiroFiltroTipo, setFinanceiroFiltroTipo] = useState("all"); // all | pagar | receber
  const [financeiroFiltroStatus, setFinanceiroFiltroStatus] = useState("all"); // all | pendente | pago
  const [financeiroFiltroTitular, setFinanceiroFiltroTitular] = useState("all");
  const [financeiroPeriodStart, setFinanceiroPeriodStart] = useState("2000-01-01");
  const [financeiroPeriodEnd, setFinanceiroPeriodEnd] = useState("2100-01-01");

  // Financeiro Pessoal: filtra por tipo/status/titular/período, e calcula
  // os totais pendentes e já pagos/recebidos de cada tipo
  const financeiroReport = useMemo(() => {
    const hoje = todayISO();
    let itens = financeiroPessoal.filter((f) => (f.dataVencimento || "") >= financeiroPeriodStart && (f.dataVencimento || "") <= financeiroPeriodEnd);
    if (financeiroFiltroTipo !== "all") itens = itens.filter((f) => f.tipo === financeiroFiltroTipo);
    if (financeiroFiltroTitular !== "all") itens = itens.filter((f) => (f.titular || "Não especificado") === financeiroFiltroTitular);
    if (financeiroFiltroStatus !== "all") {
      itens = itens.filter((f) => (financeiroFiltroStatus === "pago" ? !!f.dataPagamento : !f.dataPagamento));
    }
    itens = [...itens].sort((a, b) => (a.dataVencimento || "").localeCompare(b.dataVencimento || ""));
    const totais = itens.reduce(
      (acc, f) => {
        const valor = Number(f.valor) || 0;
        const pago = !!f.dataPagamento;
        const vencido = !pago && (f.dataVencimento || "") < hoje;
        if (f.tipo === "pagar") {
          if (pago) acc.pagoPagar += valor;
          else acc.pendentePagar += valor;
          if (vencido) acc.vencidoPagar += valor;
        } else {
          if (pago) acc.pagoReceber += valor;
          else acc.pendenteReceber += valor;
          if (vencido) acc.vencidoReceber += valor;
        }
        return acc;
      },
      { pendentePagar: 0, pagoPagar: 0, vencidoPagar: 0, pendenteReceber: 0, pagoReceber: 0, vencidoReceber: 0 }
    );
    return { itens, totais };
  }, [financeiroPessoal, financeiroPeriodStart, financeiroPeriodEnd, financeiroFiltroTipo, financeiroFiltroStatus, financeiroFiltroTitular]);

  const startAddFinanceiro = (tipoPreSelecionado) => {
    setAddingFinanceiro(true);
    setEditingFinanceiroId(null);
    setFinanceiroTipo(tipoPreSelecionado || "pagar");
    setFinanceiroDescricao("");
    setFinanceiroPessoaNome("");
    setFinanceiroValor("");
    setFinanceiroVencimento(new Date().toISOString().slice(0, 10));
    setFinanceiroDataPagamento("");
    setFinanceiroObs("");
    setFinanceiroTitular("");
    setFinanceiroOndeComprou("");
    setFinanceiroNotaNumero("");
    setFinanceiroNotaData("");
    setFinanceiroParcelar(false);
    setFinanceiroNumParcelas("2");
    setFinanceiroIntervaloDias("30");
  };

  const startEditFinanceiro = (f) => {
    setAddingFinanceiro(true);
    setEditingFinanceiroId(f.id);
    setFinanceiroTipo(f.tipo || "pagar");
    setFinanceiroDescricao(f.descricao || "");
    setFinanceiroPessoaNome(f.pessoa || "");
    setFinanceiroValor(f.valor || "");
    setFinanceiroVencimento(f.dataVencimento || "");
    setFinanceiroDataPagamento(f.dataPagamento || "");
    setFinanceiroObs(f.observacao || "");
    setFinanceiroTitular(f.titular || "");
    setFinanceiroOndeComprou(f.ondeComprou || "");
    setFinanceiroNotaNumero(f.notaFiscalNumero || "");
    setFinanceiroNotaData(f.notaFiscalData || "");
  };

  const confirmAddFinanceiro = () => {
    if (!financeiroDescricao.trim() || !(Number(financeiroValor) > 0) || !financeiroVencimento) {
      alert("Preencha a descrição, o valor e o vencimento.");
      return;
    }
    const base = {
      tipo: financeiroTipo,
      pessoa: financeiroPessoaNome.trim(),
      observacao: financeiroObs.trim(),
      titular: financeiroTitular,
      ondeComprou: financeiroOndeComprou.trim(),
      notaFiscalNumero: financeiroNotaNumero.trim(),
      notaFiscalData: financeiroNotaData,
    };
    if (!editingFinanceiroId && financeiroParcelar && Number(financeiroNumParcelas) > 1) {
      const n = Math.round(Number(financeiroNumParcelas));
      const intervalo = Number(financeiroIntervaloDias) || 30;
      const totalCentavos = Math.round((Number(financeiroValor) || 0) * 100);
      const porParcelaCentavos = Math.floor(totalCentavos / n);
      const sobraCentavos = totalCentavos - porParcelaCentavos * n;
      const dataBase = new Date(financeiroVencimento + "T00:00:00");
      const registros = Array.from({ length: n }, (_, i) => {
        const centavos = porParcelaCentavos + (i === n - 1 ? sobraCentavos : 0);
        const dataParcela = new Date(dataBase);
        dataParcela.setDate(dataBase.getDate() + intervalo * i);
        return {
          ...base,
          descricao: `${financeiroDescricao.trim()} (parcela ${i + 1}/${n})`,
          valor: (centavos / 100).toFixed(2),
          dataVencimento: dataParcela.toISOString().slice(0, 10),
          dataPagamento: i === 0 ? financeiroDataPagamento : "",
        };
      });
      addFinanceiroPessoalParcelado(registros);
    ...
    const registro = {
      ...base,
      descricao: financeiroDescricao.trim(),
      valor: financeiroValor,
      dataVencimento: financeiroVencimento,
      dataPagamento: financeiroDataPagamento,
    };
    if (editingFinanceiroId) {
      updateFinanceiroPessoal(editingFinanceiroId, registro);
    } else {
      addFinanceiroPessoal(registro);
    }
    setAddingFinanceiro(false);
    setEditingFinanceiroId(null);
  };
  ...
  // controla o espaçamento entre backups automáticos, pra não gravar um a cada
  // tecla digitada — só cria um novo backup se já fez pelo menos 10 minutos
  // desde o último (nessa mesma sessão aberta do app)
  const ultimoBackupAutomaticoRef = useRef(0);
  const criandoBackupAutomaticoRef = useRef(false);
  ...
  const reloadFromSheets = async () => {
    try {
      const { trucks: t, trips: v, vales: vl, boletos: bo, empresas: emp, fechamentos: fe, despesasVeiculo: dv, taxasPool: tp, motoristas: mo, contas: ct, trocasOleo: to, servicosVeiculo: sv, semParar: sp, seguro: sg, semPararOutros: spo, financeiroPessoal: fp, multas: mt, config: cfg } = await loadFromSheets(tokenRef.current);
      ...
      const empresasComPedagioSalvas = cfg.find((c) => c.chave === "empresasComPedagio");
      if (empresasComPedagioSalvas) {
        const parsed = safeParseJSON(empresasComPedagioSalvas.valor, null);
        if (parsed) setEmpresasComPedagio(parsed);
      }
      const rotasPedagioSalvas = cfg.find((c) => c.chave === "rotasPedagio");
      if (rotasPedagioSalvas) {
        const parsed = safeParseJSON(rotasPedagioSalvas.valor, null);
        if (parsed) setRotasPedagio(parsed);
      }
      const metaSalva = cfg.find((c) => c.chave === "metaFaturamentoMensal");
      if (metaSalva) setMetaFaturamentoMensal(metaSalva.valor || "");
      setLoaded(true);
      // garante pelo menos 1 backup automático por dia — checa se já existe um
      // de hoje na aba Backups, e se não tiver, cria um agora
      (async () => {
        try {
          const hojeISO = new Date().toISOString().slice(0, 10);
          const resBackups = await sheetsFetch(`/values/Backups!A2:A`, tokenRef.current);
          const datas = (resBackups.values || []).map((row) => row[0]);
          if (!datas.includes(hojeISO)) {
            criarBackupAutomatico();
          } else {
            ultimoBackupAutomaticoRef.current = Date.now();
          }
        } catch (e) {
          // se essa checagem falhar, não é grave — o backup ainda vai acontecer
          // sozinho na primeira alteração salva com sucesso
        }
      })();
  ...
  const persist = async (nextTrucks, nextTrips, nextVales = vales, nextBoletos = boletos, nextEmpresas = empresas, nextFechamentos = fechamentos, nextDespesasVeiculo = despesasVeiculo, nextTaxasPool = taxasPool, nextMotoristas = motoristas, nextContas = contas, nextTrocasOleo = trocasOleo, nextServicosVeiculo = servicosVeiculo, nextSemParar = semParar, nextSeguro = seguro, nextSemPararOutros = semPararOutros, nextConfig = config, nextFinanceiroPessoal = financeiroPessoal, nextMultas = multas) => {
    ...
    if (nextFinanceiroPessoal !== financeiroPessoal) changes.financeiroPessoal = nextFinanceiroPessoal;
    if (nextMultas !== multas) changes.multas = nextMultas;
  ...
  const addTrocaOleo = (caminhaoId, data, km, filtroTrocado, observacao, valor, empresa, pecas) => {
    const nextTrocas = [...trocasOleo, { id: uid(), caminhaoId, data, km, filtroTrocado, observacao, valor: valor || "", empresa: empresa || "", pecas: pecas || [] }];
  ...
  const updateTrocaOleo = (id, caminhaoId, data, km, filtroTrocado, observacao, valor, empresa, pecas) => {
    const nextTrocas = trocasOleo.map((t) => (t.id === id ? { ...t, caminhaoId, data, km, filtroTrocado, observacao, valor: valor || "", empresa: empresa || "", pecas: pecas || [] } : t));
    setTrocasOleo(nextTrocas);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, nextTrocas);
  };
  ...
  // ---- Financeiro Pessoal: área separada de contas a pagar/receber que não
  // tem nenhuma ligação com viagens, boletos ou os relatórios/dashboard ----
  const addFinanceiroPessoal = (registro) => {
    const nextLista = [...financeiroPessoal, { id: uid(), ...registro }];
    setFinanceiroPessoal(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, nextLista);
  };

  const addFinanceiroPessoalParcelado = (registros) => {
    const nextLista = [...financeiroPessoal, ...registros.map((r) => ({ id: uid(), ...r }))];
    setFinanceiroPessoal(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, nextLista);
  };

  const updateFinanceiroPessoal = (id, registro) => {
    const nextLista = financeiroPessoal.map((f) => (f.id === id ? { ...f, ...registro } : f));
    setFinanceiroPessoal(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, nextLista);
  };

  const deleteFinanceiroPessoal = (id) => {
    if (!window.confirm("Tem certeza que deseja excluir esse lançamento?")) return;
    const nextLista = financeiroPessoal.filter((f) => f.id !== id);
    setFinanceiroPessoal(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, nextLista);
  };

  // ---- Multas de trânsito ----
  const addMulta = (registro) => {
    const nextLista = [...multas, { id: uid(), ...registro }];
    setMultas(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, financeiroPessoal, nextLista);
  };
  const updateMulta = (id, registro) => {
    const nextLista = multas.map((m) => (m.id === id ? { ...m, ...registro } : m));
    setMultas(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, financeiroPessoal, nextLista);
  };
  const deleteMulta = (id) => {
    if (!window.confirm("Tem certeza que deseja excluir essa multa?")) return;
    const nextLista = multas.filter((m) => m.id !== id);
    setMultas(nextLista);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, config, financeiroPessoal, nextLista);
  };

  const addServicoVeiculo = (caminhaoId, data, km, tipoServico, observacao, empresa, itens, pecas) => {
    const nextServicos = [...servicosVeiculo, { id: uid(), caminhaoId, data, km, tipoServico, observacao, empresa: empresa || "", itens: itens || [], pecas: pecas || [] }];
  ...
  const updateServicoVeiculo = (id, caminhaoId, data, km, tipoServico, observacao, empresa, itens, pecas) => {
    const nextServicos = servicosVeiculo.map((s) => (s.id === id ? { ...s, caminhaoId, data, km, tipoServico, observacao, empresa: empresa || "", itens: itens || [], pecas: pecas || [] } : s));
    setServicosVeiculo(nextServicos);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, nextServicos);
  };
  ...
  const saveTrip = () => {
    ...
    if (!editing.motorista || !editing.motorista.trim()) {
      alert("Preenche o motorista antes de salvar — sem isso a viagem fica sem comissão vinculada e corre o risco de esquecer de pagar.");
      return;
    }
    if (editing.contrato && editing.contrato.trim()) {
      const contratoNormalizado = editing.contrato.trim().toLowerCase();
      const jaExiste = trips.some((t) => t.id !== editing.id && (t.contrato || "").trim().toLowerCase() === contratoNormalizado);
      if (jaExiste) {
        alert(`Já existe uma viagem lançada com o contrato "${editing.contrato.trim()}". Confere o número, ou muda se for mesmo uma viagem nova.`);
        return;
      }
    }
    if (editing.empresa && empresasComPedagio.some((nome) => nome.trim().toLowerCase() === editing.empresa.trim().toLowerCase()) && !(Number(editing.pedagio) > 0)) {
      alert(`A empresa "${editing.empresa}" está marcada como "tem pedágio" nas configurações. Preenche o valor do pedágio antes de salvar (e não esquece de descontar do motorista, se for o caso).`);
      return;
    }
  ...
  const relatorioRecebimento = useMemo(() => {
    ...
    const totalAdiantamento = items.filter((r) => r.tipo === "Adiantamento").reduce((s, r) => s + r.valor, 0);
    const totalSaldo = items.filter((r) => r.tipo === "Saldo").reduce((s, r) => s + r.valor, 0);
    return { items, total, totalAdiantamento, totalSaldo };
  ...
  const commissionByDriver = useMemo(() => {
    ...
    return Object.values(groups)
      .map((g) => {
        ...
        const tripsAtuaisTodas = g.trips.filter((tv) => !tripIdsFechados.has(tv.trip.id) && (tv.trip.data || "") > legacyCutoffTrips);
        // só entra no total (e no fechamento) quem já tem "data fim" preenchida —
        // viagem sem data fim ainda está em andamento (não descarregou), então
        // aparece na lista mas fica de fora da comissão a receber por enquanto
        const tripsAtuais = tripsAtuaisTodas.filter((tv) => (tv.trip.dataFim || "").trim());
        const tripsPendentesConclusao = tripsAtuaisTodas.filter((tv) => !(tv.trip.dataFim || "").trim());
        ...
        return {
          ...
          tripsPendentesConclusao: tripsPendentesConclusao.sort((a, b) => (a.trip.data || "").localeCompare(b.trip.data || "")),
          vales: valesAtuais,
          valesFuturos,
          total: totalAtual,
          totalPago,
  ...
  // ids de viagens/vales que a pessoa desmarcou nesse fechamento — ficam de
  // fora dessa rodada e continuam pendentes pro próximo fechamento
  const [fechamentoTripsExcluidos, setFechamentoTripsExcluidos] = useState({});
  const [fechamentoValesExcluidos, setFechamentoValesExcluidos] = useState({});

  const startFecharSaldo = (motorista) => {
    setFechandoMotorista(motorista);
    setFechamentoDataEditavel(todayISO());
    setFechamentoTripsExcluidos({});
    setFechamentoValesExcluidos({});
  };

  const confirmFecharSaldo = (valor, tripsAtuais, valesAtuais) => {
    const tripsIncluidas = tripsAtuais.filter((tv) => !fechamentoTripsExcluidos[tv.trip.id]);
    const valesIncluidos = valesAtuais.filter((v) => !fechamentoValesExcluidos[v.id]);
    const tripIds = tripsIncluidas.map((tv) => tv.trip.id);
    const valeIds = valesIncluidos.map((v) => v.id);
    const totalTripsIncluidas = tripsIncluidas.reduce((s, tv) => s + tv.valor, 0);
    const totalReembolsoIncluido = valesIncluidos.filter((v) => v.tipo === "reembolso").reduce((s, v) => s + (Number(v.valor) || 0), 0);
    const totalPagoIncluido = valesIncluidos.filter((v) => v.tipo !== "reembolso").reduce((s, v) => s + (Number(v.valor) || 0), 0);
    const valorFinal = totalTripsIncluidas + totalReembolsoIncluido - totalPagoIncluido;
    const nextFechamentos = [...fechamentos, { id: uid(), motorista: fechandoMotorista, data: fechamentoDataEditavel, valor: valorFinal, tripIds, valeIds }];
    setFechamentos(nextFechamentos);
    persist(trucks, trips, vales, boletos, empresas, nextFechamentos);
  ...
  // Evolução mensal: mesma lógica do Dashboard Geral, mas mês a mês, pros
  // últimos 6 meses — pra ver a tendência em vez de só uma "foto" de um período
  const evolucaoMensal = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const tripsNoMes = trips.filter((t) => (t.data || "") >= inicio && (t.data || "") <= fim);
      const receita = tripsNoMes.reduce((s, t) => s + valorTotal(t), 0);
      const comissoesMes = tripsNoMes.reduce((s, t) => s + comissao(t), 0);
      const gastosEstrada = tripsNoMes.reduce(
        (s, t) => s + (t.gastosExtras || []).filter((g) => (g.categoria || "") === "Despesas Estrada").reduce((s2, g) => s2 + (Number(g.valor) || 0), 0),
        0
      );
      let boletosMes = 0;
      boletos.forEach((b) => {
        const categoriaEmpresa = empresasPorCategoria[b.empresa];
        if (!categoriaEmpresa || !categoriasIncluidasDashboardGeral.includes(categoriaEmpresa)) return;
        eventosPagamentoBoleto(b).forEach((ev) => { if (ev.data >= inicio && ev.data <= fim) boletosMes += ev.valor; });
      });
      const taxaMes = despesasVeiculo
        .filter((dv) => (dv.data || "") >= inicio && (dv.data || "") <= fim && (dv.descricao || "").startsWith("Taxa de viagem"))
        .reduce((s, dv) => s + (Number(dv.valor) || 0), 0);
      const gastosTotais = comissoesMes + gastosEstrada + boletosMes + taxaMes;
      const liquido = receita - gastosTotais;
      meses.push({
        mes: inicio.slice(0, 7),
        label: d.toLocaleDateString("pt-BR", { month: "short", year: "2-digit" }).replace(".", ""),
        receita, gastos: gastosTotais, liquido,
      });
    }
    const maiorValor = Math.max(1, ...meses.map((m) => Math.max(m.receita, m.gastos)));
    return { meses, maiorValor };
  }, [trips, boletos, empresas, despesasVeiculo, categoriasIncluidasDashboardGeral]);

  // Relatório pro Contador: resumo fiscal simplificado (receita x despesas
  // categorizadas) de UM mês escolhido, pronto pra mandar
  const relatorioContador = useMemo(() => {
    const inicio = contadorMes + "-01";
    const fimDate = new Date(Number(contadorMes.slice(0, 4)), Number(contadorMes.slice(5, 7)), 0);
    const fim = fimDate.toISOString().slice(0, 10);
    const tripsNoMes = trips.filter((t) => (t.data || "") >= inicio && (t.data || "") <= fim);
    const receita = tripsNoMes.reduce((s, t) => s + valorTotal(t), 0);
    const comissoesMes = tripsNoMes.reduce((s, t) => s + comissao(t), 0);
    ...
    boletos.forEach((b) => {
      ...
      if (!categoriaEmpresa || !categoriasIncluidasDashboardGeral.includes(categoriaEmpresa)) return;
      eventosPagamentoBoleto(b).forEach((ev) => {
        if (ev.data >= inicio && ev.data <= fim) boletosDoMes.push({ empresa: b.empresa, descricao: b.descricao, data: ev.data, valor: ev.valor });
      });
    });
    const boletosMes = boletosDoMes.reduce((s, ev) => s + ev.valor, 0);
    const taxaMes = despesasVeiculo
      .filter((dv) => (dv.data || "") >= inicio && (dv.data || "") <= fim && (dv.descricao || "").startsWith("Taxa de viagem"))
      .reduce((s, dv) => s + (Number(dv.valor) || 0), 0);
    const abastecimentosMes = tripsNoMes.reduce(
      (s, t) => s + (t.abastecimentos || []).reduce((s2, a) => s2 + (Number(a.valor) || 0), 0),
      0
    );
    const gastosTotais = comissoesMes + gastosEstrada + boletosMes + taxaMes;
    return {
      inicio, fim, receita, comissoesMes, gastosEstrada, boletosMes, boletosDoMes, taxaMes, abastecimentosMes,
      gastosTotais, liquido: receita - gastosTotais, qtdViagens: tripsNoMes.length,
    };
  }, [trips, boletos, empresas, despesasVeiculo, categoriasIncluidasDashboardGeral, contadorMes]);
  ...
  const detailedReport = useMemo(() => {
    ...
    const despesasInMonth = despesasVeiculo.filter((d) => (d.data || "").slice(0, 7) === reportMonth);
    const trucksFiltered = filterTruck === "all" ? trucksCavalos : trucksCavalos.filter((tr) => tr.id === filterTruck);
    let receitaGeral = 0, comissaoGeral = 0, combustivelGeral = 0, gastosExtrasGeral = 0, despesasVeiculoGeral = 0;
    const groups = trucksFiltered
      .map((tr) => {
        ...
        const tripRows = tripsTruck.map((t, idx) => {
          ...
          const carregamento = Number(t.carregamento) || 0;
          const comissaoItens = [];
          if (com > 0) comissaoItens.push({ descricao: "Comissão" + (t.motorista ? ` — ${t.motorista}` : ""), valor: com });
          if (carregamento > 0) comissaoItens.push({ descricao: "Carregamento" + (t.carregamentoMotorista ? ` — ${t.carregamentoMotorista}` : ""), valor: carregamento });
          const totalComissao = com + carregamento;

          const abastecimentos = (t.abastecimentos || []).filter((a) => Number(a.valor) > 0).map((a) => ({ data: a.data || t.data, posto: a.posto || "—", litragem: Number(a.litragem) || 0, valor: Number(a.valor) || 0, km: a.km || "" }));
          const totalCombustivel = abastecimentos.reduce((s, a) => s + a.valor, 0);
          const totalLitros = abastecimentos.reduce((s, a) => s + a.litragem, 0);

          const gastosExtrasItens = (t.gastosExtras || []).filter((g) => Number(g.valor) > 0).map((g) => ({ descricao: g.descricao || "Gasto extra", valor: Number(g.valor) || 0 }));
          const totalGastosExtras = gastosExtrasItens.reduce((s, g) => s + g.valor, 0);

          const valorViagem = valorTotal(t);
          const totalDespesas = totalComissao + totalCombustivel + totalGastosExtras;

          receitaGeral += valorViagem;
          comissaoGeral += totalComissao;
          combustivelGeral += totalCombustivel;
          gastosExtrasGeral += totalGastosExtras;
          ...
          return {
            ...
            destino: t.destino || "—",
            valorViagem,
            comissaoItens, totalComissao,
            abastecimentos, totalCombustivel, totalLitros,
            gastosExtrasItens, totalGastosExtras,
            totalDespesas,
            liquidoViagem: valorViagem - totalDespesas,
          };
        });
        ...
        despesasVeiculoGeral += despesasVeiculoTotal;
        ...
        const totalViagensReceita = tripRows.reduce((s, t) => s + t.valorViagem, 0);
        const totalComissaoTruck = tripRows.reduce((s, t) => s + t.totalComissao, 0);
        const totalCombustivelTruck = tripRows.reduce((s, t) => s + t.totalCombustivel, 0);
        const totalGastosExtrasTruck = tripRows.reduce((s, t) => s + t.totalGastosExtras, 0);
        return {
          id: tr.id, placa: tr.placa, tripRows, despesasVeiculoTruck, despesasVeiculoTotal, totalVeiculo,
          resumo: {
            totalViagens: totalViagensReceita - totalComissaoTruck,
            totalCombustivel: totalCombustivelTruck,
            totalGastosExtras: totalGastosExtrasTruck,
            despesasVeiculo: despesasVeiculoTotal,
          },
        };
      })
      .filter((g) => g.tripRows.length > 0 || g.despesasVeiculoTruck.length > 0);

    const despesasTotaisGeral = comissaoGeral + combustivelGeral + gastosExtrasGeral + despesasVeiculoGeral;
    const resultadoLiquidoGeral = receitaGeral - despesasTotaisGeral;
    const resumoGeral = {
      receitaGeral, comissaoGeral, combustivelGeral, gastosExtrasGeral, despesasVeiculoGeral,
      despesasTotaisGeral, resultadoLiquidoGeral,
      pctReceitaLiquida: receitaGeral > 0 ? (resultadoLiquidoGeral / receitaGeral) * 100 : 0,
      pctComissoes: receitaGeral > 0 ? (comissaoGeral / receitaGeral) * 100 : 0,
      pctDespesas: receitaGeral > 0 ? (despesasTotaisGeral / receitaGeral) * 100 : 0,
      categorias: [
        { nome: "Combustível", valor: combustivelGeral, cor: "#2451A6" },
        { nome: "Despesas (Gastos Extras)", valor: gastosExtrasGeral, cor: "#B0402E" },
        { nome: "Comissões", valor: comissaoGeral, cor: "#D9A419" },
        { nome: "Despesas do Veículo", valor: despesasVeiculoGeral, cor: "#1F6F5C" },
      ].map((c) => ({ ...c, pct: despesasTotaisGeral > 0 ? (c.valor / despesasTotaisGeral) * 100 : 0 })),
    };
    return { groups, resumoGeral };
  ...
  const abastecPostosList = useMemo(() => {
    ...
    (editing && editing.abastecimentos ? editing.abastecimentos : []).forEach((a) => { if (a.posto) set.add(a.posto); });
    set.delete("");
    set.delete(undefined);
    return Array.from(set).sort();
  }, [abastecimentosFlat, precosPostos, editing]);
...
function calcularConsumoEntreAbastecimentos(itensDieselOrdenados) {
  ...
  const abastecReport = useMemo(() => {
    const filtered = abastecimentosFlat
      ...
      .sort((a, b) => (a.posto || "").localeCompare(b.posto || "") || (a.data || "").localeCompare(b.data || "") || (a.numeroCupom || "").localeCompare(b.numeroCupom || ""));

    const porPostoMap = {};
    filtered.forEach((a) => {
      if (!porPostoMap[a.posto]) porPostoMap[a.posto] = { posto: a.posto, items: [], valor: 0, valorDiesel: 0, valorArla: 0, litragemDiesel: 0, litragemArla: 0 };
      porPostoMap[a.posto].items.push(a);
      porPostoMap[a.posto].valor += a.valor;
      if (a.tipo === "arla") {
        porPostoMap[a.posto].litragemArla += Number(a.litragem) || 0;
        porPostoMap[a.posto].valorArla += Number(a.valor) || 0;
      } else if (a.tipo === "diesel") {
        porPostoMap[a.posto].litragemDiesel += Number(a.litragem) || 0;
        porPostoMap[a.posto].valorDiesel += Number(a.valor) || 0;
      }
    });
  ...
  const boletosReport = useMemo(() => {
    ...
    const totals = { pendente: 0, vencido: 0, pago: 0, total: 0, aVencer: 0, countVencido: 0, countPendente: 0, countAVencer: 0, countAberto: abertos.length };
    abertos.forEach((b) => {
      if (b.__status === "vencido") { totals.vencido += b.__valorItem; totals.countVencido++; }
      else if (b.__status === "pendente" && b.__dataItem <= em7diasISO) { totals.aVencer += b.__valorItem; totals.countAVencer++; }
      else { totals.pendente += b.__valorItem; totals.countPendente++; }
    });
    eventosPagos.forEach((ev) => { totals.pago += ev.__valorItem; });
    totals.total = totals.pendente + totals.vencido + totals.pago + totals.aVencer;
  ...
  // salva uma cópia completa dos dados na aba "Backups" da própria planilha —
  // mantém só os 30 mais recentes, pra não crescer sem limite
  const criarBackupAutomatico = async () => {
    if (criandoBackupAutomaticoRef.current) return;
    if (!tokenRef.current) return;
    criandoBackupAutomaticoRef.current = true;
    try {
      const conteudo = JSON.stringify({ trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, financeiroPessoal, multas });
      const agora = new Date();
      const novaLinha = [agora.toISOString().slice(0, 10), agora.toISOString(), conteudo];
      const res = await sheetsFetch(`/values/Backups!A2:C`, tokenRef.current);
      const existentes = (res.values || []).filter((row) => row[0]);
      const combinados = [...existentes, novaLinha].slice(-30);
      await sheetsFetch(`/values/Backups!A2:C2000:clear`, tokenRef.current, { method: "POST" });
      await sheetsFetch(`/values/Backups!A2:C${combinados.length + 1}?valueInputOption=RAW`, tokenRef.current, {
        method: "PUT",
        body: JSON.stringify({ range: `Backups!A2:C${combinados.length + 1}`, values: combinados }),
      });
      ultimoBackupAutomaticoRef.current = Date.now();
    } catch (e) {
      // backup automático não deve travar o uso do app — falha silenciosa
    } finally {
      criandoBackupAutomaticoRef.current = false;
    }
  };

  // chamado toda vez que algo é salvo com sucesso — só cria um backup novo se
  // já fez pelo menos 10 minutos desde o último, pra não gravar a cada lançamento
  const agendarBackupAutomatico = () => {
    const dezMinutos = 10 * 60 * 1000;
    if (Date.now() - ultimoBackupAutomaticoRef.current < dezMinutos) return;
    criarBackupAutomatico();
  };

  const carregarListaBackupsAutomaticos = async () => {
    if (!tokenRef.current) return;
    try {
      const res = await sheetsFetch(`/values/Backups!A2:B`, tokenRef.current);
      const lista = (res.values || []).filter((row) => row[0]).map((row) => ({ data: row[0], criadoEm: row[1] })).reverse();
      setBackupsAutomaticos(lista);
    } catch (e) {
      setBackupsAutomaticos([]);
    }
  };

  const baixarBackupAutomatico = async (criadoEm) => {
    if (!tokenRef.current) return;
    try {
      const res = await sheetsFetch(`/values/Backups!A2:C`, tokenRef.current);
      const linha = (res.values || []).find((row) => row[1] === criadoEm);
      if (!linha) { alert("Não encontrei esse backup — pode ter sido substituído por um mais recente."); return; }
      const blob = new Blob([linha[2]], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `backup-automatico-${linha[0]}.json`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      alert("Não consegui baixar esse backup agora. Tenta de novo em instantes.");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="nav-group" style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {[
            { id: "dashboard", icone: "🏠", rotulo: "Dashboard", onClick: () => { setModuloAtivo("dashboard"); setView("dashboard"); } },
            { id: "operacao", icone: "🚛", rotulo: "Operação", onClick: () => { setModuloAtivo("operacao"); setView("viagens"); } },
            { id: "financeiro", icone: "💰", rotulo: "Financeiro", onClick: () => { setModuloAtivo("financeiro"); setView("boletos"); } },
            { id: "frota", icone: "🚚", rotulo: "Frota", onClick: () => { setModuloAtivo("frota"); setView("seguro"); } },
            { id: "relatorios", icone: "📊", rotulo: "Relatórios", onClick: () => setModuloAtivo("relatorios") },
            { id: "configuracoes", icone: "⚙️", rotulo: "Configurações", onClick: () => { setModuloAtivo("configuracoes"); abrirConfiguracoes(); } },
          ].map((mod) => (
            <button
              key={mod.id}
              className="nav-item"
              ...
              style={{
                ...
                background: moduloAtivo === mod.id ? "#2563EB" : "transparent",
                color: moduloAtivo === mod.id ? "#fff" : "#9AA5B1",
                fontWeight: moduloAtivo === mod.id ? 700 : 500, fontSize: 14, cursor: "pointer",
              }}
            >
              <span>{mod.icone}</span> {mod.rotulo}
            </button>
          ))}
        </div>
      ...
      {/* submenu horizontal do módulo ativo */}
      {(() => {
        const submenus = {
          dashboard: [
            { id: "dashboard", icone: "🚚", rotulo: "Viagens", onClick: () => setView("dashboard") },
            { id: "dashboardGeral", icone: "🌐", rotulo: "Geral", onClick: () => setView("dashboardGeral") },
          ],
          operacao: [
            { id: "viagens", icone: "🚚", rotulo: "Viagens", onClick: () => setView("viagens") },
            { id: "abastecimentos", icone: "⛽", rotulo: "Abastecimentos", onClick: () => setView("abastecimentos") },
            { id: "trocaoleo", icone: "🛢️", rotulo: "Troca de Óleo", onClick: () => setView("trocaoleo") },
            { id: "semparar", icone: "🛣️", rotulo: "Sem Parar", onClick: () => setView("semparar") },
            { id: "multas", icone: "🚨", rotulo: "Multas", onClick: () => setView("multas") },
          ],
          financeiro: [
            { id: "boletos", icone: "🧾", rotulo: "Contas a pagar", onClick: () => setView("boletos") },
            { id: "financeiropessoal", icone: "💳", rotulo: "Financeiro Pessoal", onClick: () => setView("financeiropessoal") },
            { id: "abastecimentos-fin", icone: "⛽", rotulo: "Abastecimentos", onClick: () => setView("abastecimentos") },
          ],
          frota: [
            { id: "seguro", icone: "🛡️", rotulo: "Seguro", onClick: () => setView("seguro") },
            { id: "config-caminhoes", icone: "🚚", rotulo: "Caminhões", onClick: () => { setConfigSecaoAberta("caminhoes"); setConfigOpen(true); } },
            { id: "config-carretas", icone: "🚛", rotulo: "Carretas", onClick: () => { setConfigSecaoAberta("carretas"); setConfigOpen(true); } },
          ],
          relatorios: [
            { id: "rel-mensal", icone: "📊", rotulo: "Relatório mensal", onClick: () => abrirRelatorio("mensal") },
            { id: "rel-recebimento", icone: "💰", rotulo: "Recebimento", onClick: () => abrirRelatorio("recebimento") },
            { id: "rel-gastos", icone: "🧾", rotulo: "Gastos extras", onClick: () => abrirRelatorio("gastos") },
            { id: "rel-consumo", icone: "⛽", rotulo: "Consumo (km/L)", onClick: () => abrirRelatorio("consumo") },
            { id: "rel-servicos", icone: "🔧", rotulo: "Serviços por empresa", onClick: () => abrirRelatorio("servicos") },
            { id: "rel-contador", icone: "🧮", rotulo: "Relatório pro Contador", onClick: () => abrirRelatorio("contador") },
          ],
        };
        const itens = submenus[moduloAtivo];
        if (!itens) return null;
        return (
          <div style={{ background: "#F7F8F9", borderBottom: "1px solid #E4E7EB", padding: "0 28px", display: "flex", gap: 4, overflowX: "auto" }}>
            {itens.map((item) => (
              <button
                key={item.id}
                ...
                style={{
                  ...
                  color: view === item.id ? "#2451A6" : "#5A6472",
                  borderBottom: view === item.id ? "2px solid #2451A6" : "2px solid transparent",
                }}
              >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
            {dashboardReport.rows.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  onClick={() => { setFilterTruck(r.id); setReportMonth(dashboardPeriodEnd.slice(0, 7)); setReportView("detalhado"); abrirRelatorio("mensal"); }}
                  className="linha-clicavel"
                  style={{
      ...
      {view === "dashboardGeral" && (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 28px" }}>
          ...
          {(() => {
            ...
            const meta = Number(metaFaturamentoMensal) || 0;
            return (
              <>
                {meta > 0 && (() => {
                  const pct = Math.min(100, (dashboardGeralReport.receitaBruta / meta) * 100);
                  const bateu = dashboardGeralReport.receitaBruta >= meta;
                  return (
                    <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: 14, marginTop: 20 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, fontSize: 13 }}>
                        <span style={{ fontWeight: 700, color: "#1B2430" }}>🎯 Meta de faturamento do período</span>
                        <span style={{ color: bateu ? "#1F6F5C" : "#5A6472" }}>
                          {BRL(dashboardGeralReport.receitaBruta)} de {BRL(meta)} ({pct.toFixed(0)}%)
                        </span>
                      </div>
                      <div style={{ background: "#EEF0F2", borderRadius: 6, height: 10, overflow: "hidden" }}>
                        <div style={{ width: `${pct}%`, height: "100%", background: bateu ? "#1F6F5C" : "#2563EB", borderRadius: 6, transition: "width 0.3s" }} />
                      </div>
                      {bateu && <div style={{ fontSize: 12, color: "#1F6F5C", fontWeight: 700, marginTop: 6 }}>🎉 Meta batida nesse período!</div>}
                    </div>
                  );
                })()}
          ...
          <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, padding: 16, marginTop: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#1B2430", marginBottom: 4 }}>📈 Evolução mensal (últimos 6 meses)</div>
            <div style={{ display: "flex", gap: 14, fontSize: 11, color: "#5A6472", marginBottom: 10 }}>
              <span><span style={{ display: "inline-block", width: 10, height: 10, background: "#1F6F5C", borderRadius: 2, marginRight: 4 }} />Receita bruta</span>
              <span><span style={{ display: "inline-block", width: 10, height: 10, background: "#B0402E", borderRadius: 2, marginRight: 4 }} />Gastos totais</span>
            </div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 140, borderBottom: "1px solid #EEF0F2", paddingBottom: 4 }}>
              {evolucaoMensal.meses.map((m) => (
                <div key={m.mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%", justifyContent: "flex-end" }}>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: "100%" }}>
                    <div
                      ...
                      style={{ width: 14, height: `${Math.max(2, (m.receita / evolucaoMensal.maiorValor) * 100)}%`, background: "#1F6F5C", borderRadius: "3px 3px 0 0" }}
                    />
                    <div
                      title={`Gastos: ${BRL(m.gastos)}`}
                      style={{ width: 14, height: `${Math.max(2, (m.gastos / evolucaoMensal.maiorValor) * 100)}%`, background: "#B0402E", borderRadius: "3px 3px 0 0" }}
                    />
                  </div>
                </div>
              ))}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              {evolucaoMensal.meses.map((m) => (
                <div key={m.mes} style={{ flex: 1, textAlign: "center", fontSize: 11, color: "#5A6472" }}>{m.label}</div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              {evolucaoMensal.meses.map((m) => (
                <div key={m.mes} style={{ flex: 1, textAlign: "center", fontSize: 11, fontWeight: 700, color: m.liquido >= 0 ? "#1F6F5C" : "#B0402E" }}>
                  {BRL(m.liquido)}
                </div>
              ))}
            </div>
            ...
            {tripsParaExibir.map((t) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: `linear-gradient(160deg, #fff 0%, ${badgeBg} 100%)`,
                    borderRadius: 14,
                    padding: "16px 18px",
                    display: "flex",
                ...
                >
                  <div style={{ minWidth: 90 }}>
                    <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 16, fontVariantNumeric: "tabular-nums" }}>{truckLabel(t.caminhaoId)}</div>
                    {t.contrato && (
                      <div style={{ fontSize: 11, color: "#9AA5B1", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", marginTop: 2 }}>
                        Cr: {t.contrato}
                      </div>
                    )}
                  </div>
                  <div style={{ flex: "1 1 260px" }}>
                    <div style={{ fontSize: 14, marginBottom: 4 }}>📍 <strong>{t.origem || "?"}</strong> → <strong>{t.destino || "?"}</strong></div>
                    <div style={{ display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12, color: "#5A6472" }}>
                      <span>📅 {fmtDate(t.data)}{t.dataFim && ` até ${fmtDate(t.dataFim)}`}</span>
                      {t.motorista && <span>👤 {t.motorista}</span>}
                    </div>
                    {t.empresa && <div style={{ fontSize: 12, color: "#5A6472", marginTop: 2 }}>🏢 {t.empresa}</div>}
                    {Number(t.carregamento) > 0 && (
                      <div style={{ fontSize: 12, color: "#8A5A00", marginTop: 4 }}>
                        Carregamento {BRL(Number(t.carregamento))}{t.carregamentoMotorista && ` (${t.carregamentoMotorista})`} (INCLUSO)
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign: "right", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>
                    <div style={{ fontWeight: 700, fontSize: 17 }}>{BRL(valorTotal(t))}</div>
                    <div style={{ fontSize: 11, color: "#5A6472", marginTop: 2 }}>comissão {BRL(comissao(t))}</div>
                    <div
                      style={{
                        fontSize: 11,
                        fontWeight: 700,
      ...
      {view === "boletos" && (
        <BoletosView
          ...
          setReportOpen={(v) => (v ? abrirRelatorio("boletos") : setBoletosReportOpen(false))}
                ...
                {commissionByDriver
                  ...
                  <div key={g.motorista}>
                    ...
                    {fechandoMotorista === g.motorista && (() => {
                      const tripsIncluidas = g.trips.filter((tv) => !fechamentoTripsExcluidos[tv.trip.id]);
                      const valesIncluidos = g.vales.filter((v) => !fechamentoValesExcluidos[v.id]);
                      const totalTripsIncluidas = tripsIncluidas.reduce((s, tv) => s + tv.valor, 0);
                      const totalReembolsoIncluido = valesIncluidos.filter((v) => v.tipo === "reembolso").reduce((s, v) => s + (Number(v.valor) || 0), 0);
                      const totalPagoIncluido = valesIncluidos.filter((v) => v.tipo !== "reembolso").reduce((s, v) => s + (Number(v.valor) || 0), 0);
                      const valorRecalculado = totalTripsIncluidas + totalReembolsoIncluido - totalPagoIncluido;
                      const temExcluidos = Object.values(fechamentoTripsExcluidos).some(Boolean) || Object.values(fechamentoValesExcluidos).some(Boolean);
                      return (
                        <div style={{ background: "#F7F8F9", padding: 10, borderRadius: 12, marginBottom: 10 }}>
                          <div style={{ fontSize: 11, color: "#5A6472", marginBottom: 8 }}>
                            Desmarque abaixo qualquer viagem ou vale que não deve entrar nesse fechamento — fica pendente pro próximo.
                          </div>
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                            ...
                            <div style={{ fontSize: 13, fontWeight: 700, color: "#5A6472" }}>
                              Valor{temExcluidos ? " (recalculado)" : ""}: {BRL(valorRecalculado)}
                            </div>
                            <button onClick={() => confirmFecharSaldo(valorRecalculado, g.trips, g.vales)} style={{ background: "#1F6F5C", color: "#fff", border: "none", borderRadius: 10, padding: "8px 14px", fontWeight: 700, cursor: "pointer" }}>
                          ...
                          {(g.trips.length > 0 || g.vales.length > 0) && (
                            <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 10 }}>
                              {g.trips.map(({ trip, valor }) => (
                                <label key={trip.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, background: "#fff", borderRadius: 8, padding: "6px 10px" }}>
                                  <input
                                    type="checkbox"
                                    checked={!fechamentoTripsExcluidos[trip.id]}
                                    onChange={(e) => setFechamentoTripsExcluidos((prev) => ({ ...prev, [trip.id]: !e.target.checked }))}
                                  />
                                  <span style={{ flex: 1 }}>{truckLabel(trip.caminhaoId)} · {fmtDate(trip.data)} · {trip.origem} → {trip.destino}</span>
                                  <strong>{BRL(valor)}</strong>
                                </label>
                              ))}
                              {g.vales.map((v) => (
                                <label key={v.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, background: "#fff", borderRadius: 8, padding: "6px 10px" }}>
                                  <input
                                    type="checkbox"
                                    checked={!fechamentoValesExcluidos[v.id]}
                                    onChange={(e) => setFechamentoValesExcluidos((prev) => ({ ...prev, [v.id]: !e.target.checked }))}
                                  />
                                  <span style={{ flex: 1 }}>{v.tipo === "reembolso" ? "Reembolso" : "Vale"} · {fmtDate(v.data)} {v.observacao && `· ${v.observacao}`}</span>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                      ...
                      {g.tripsPendentesConclusao.length > 0 && (
                        <>
                          <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 700, marginTop: 4 }}>
                            AGUARDANDO CONCLUSÃO (não entram no total até ter "data fim")
                          </div>
                          {g.tripsPendentesConclusao.map(({ trip, valor }) => (
                            <div
                              key={trip.id}
                              className="linha-clicavel"
                              onClick={() => { setStatsDetailOpen(null); openEditTrip(trip); }}
                              style={{ background: "#EAF2FB", border: "1px dashed #2451A655", borderRadius: 12, padding: "8px 12px", cursor: "pointer", fontSize: 12 }}
                            >
                              <div style={{ display: "flex", justifyContent: "space-between" }}>
                                <strong style={{ color: "#1B4E8C" }}>{truckLabel(trip.caminhaoId)} · {fmtDate(trip.data)}</strong>
                                <span style={{ fontWeight: 700, color: "#1B4E8C" }}>{BRL(valor)} (previsto)</span>
                              </div>
                              <div style={{ color: "#5A6472" }}>
                                {trip.origem} → {trip.destino} {trip.empresa && `· ${trip.empresa}`}
                              </div>
      ...
      {reportOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: 18, background: "linear-gradient(160deg, #fff 0%, #FFF6E2 100%)", border: "1px solid #EDEFF2", borderRadius: 12, padding: 12 }}>
            ...
            {reportView === "resumo" ? (
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO GERENCIAL</div>
                    <div style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.1 }}>RESULTADO MENSAL DA FROTA</div>
                    <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>Referência: {reportMonth} · {filterTruck === "all" ? "Todos os caminhões" : truckLabel(filterTruck)}</div>
                    <div style={{ fontSize: 10, color: "#5A6472" }}>Líquido = receita das viagens − comissão − carregamento − abastecimento − gastos extras − despesas do veículo (data da viagem).</div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
                      <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
                      <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(reportMonth + "-01")} a {fmtDate(new Date(Number(reportMonth.slice(0, 4)), Number(reportMonth.slice(5, 7)), 0).toISOString().slice(0, 10))}</div>
                    </div>
                    <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
                      <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
                      <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")} às {new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</div>
                    </div>
                  </div>
                </div>

                {(() => {
                  const t = monthlyReport.totals;
                  const margem = t.receita > 0 ? (t.liquido / t.receita) * 100 : 0;
                  const cards = [
                    { label: "TOTAL DE VIAGENS", value: `${t.viagens} viagem${t.viagens !== 1 ? "ns" : ""}`, icon: "🚚", cor: "#1B2E6B" },
                    { label: "RECEITA TOTAL", value: BRL(t.receita), icon: "💲", cor: "#1F6F5C" },
                    { label: "COMISSÕES", value: `−${BRL(t.comissaoTotal)}`, icon: "👤", cor: "#D9A419" },
                    { label: "ABASTECIMENTO", value: `−${BRL(t.abastecimentoTotal)}`, icon: "⛽", cor: "#7A3FBF" },
                    { label: "GASTOS EXTRAS", value: `−${BRL(t.gastosTotal)}`, icon: "🔧", cor: "#B0402E" },
                    { label: "DESP. DO VEÍCULO", value: `−${BRL(t.despesasVeiculoTotal)}`, icon: "🏢", cor: "#1B4E8C" },
                  ];
                  return (
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
                      {cards.map((c) => (
                        <div key={c.label} style={{ flex: "1 1 140px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px" }}>
                          <div style={{ fontSize: 16 }}>{c.icon}</div>
                          <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700, marginTop: 4 }}>{c.label}</div>
                          <div style={{ fontSize: 15, fontWeight: 700, color: c.cor }}>{c.value}</div>
                        </div>
                      ))}
                      <div style={{ flex: "1 1 160px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
                        <div style={{ fontSize: 9, opacity: 0.8, fontWeight: 700 }}>RESULTADO LÍQUIDO</div>
                        <div style={{ fontSize: 17, fontWeight: 700 }}>{BRL(t.liquido)}</div>
                        <div style={{ fontSize: 9, marginTop: 4 }}>
                          MARGEM LÍQUIDA <span style={{ background: "rgba(255,255,255,0.2)", borderRadius: 10, padding: "1px 8px", marginLeft: 4 }}>{margem.toFixed(1)}%</span>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <div style={{ overflowX: "auto", border: "1px solid #E4E7EB", borderRadius: 10, marginBottom: 16 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "#1B2E6B", color: "#fff" }}>
                      {["#", "Cavalo/Carreta", "Viagens", "Receita (R$)", "Comissão (R$)", "Carregamento (R$)", "Abastecimento (R$)", "Gastos Extras (R$)", "Desp. Veículo (R$)", "Resultado Líquido (R$)"].map((h, i) => (
                        <th key={h} style={{ textAlign: i > 1 ? "right" : "left", padding: "8px 10px", fontWeight: 700, fontSize: 10, whiteSpace: "nowrap" }}>
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {monthlyReport.rows.map((r, i) => (
                      <tr key={r.id} style={{ borderBottom: "1px solid #EEF0F2", background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        <td style={{ padding: "8px 10px", color: "#9AA5B1" }}>{i + 1}</td>
                        <td style={{ padding: "8px 10px", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums", fontWeight: 700 }}>
                        ...
                        <td style={{ padding: "8px 10px", textAlign: "right", color: "#1F6F5C" }}>{BRL(r.receita)}</td>
                  ...
                  <tfoot>
                    <tr style={{ background: "#1B2E6B", color: "#fff" }}>
                      <td colSpan={2} style={{ padding: "10px", fontWeight: 700 }}>TOTAL DA FROTA</td>
                      ...
                      <td style={{ padding: "10px", textAlign: "right", fontWeight: 700 }}>−{BRL(monthlyReport.totals.comissaoTotal)}</td>
                      <td style={{ padding: "10px", textAlign: "right", fontWeight: 700 }}>−{BRL(monthlyReport.totals.carregamentoTotal)}</td>
                      <td style={{ padding: "10px", textAlign: "right", fontWeight: 700 }}>−{BRL(monthlyReport.totals.abastecimentoTotal)}</td>
                      <td style={{ padding: "10px", textAlign: "right", fontWeight: 700 }}>−{BRL(monthlyReport.totals.gastosTotal)}</td>
                      <td style={{ padding: "10px", textAlign: "right", fontWeight: 700 }}>−{BRL(monthlyReport.totals.despesasVeiculoTotal)}</td>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 12 }}>
                  <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 10 }}>🚚 RESULTADO LÍQUIDO POR VEÍCULO</div>
                    {(() => {
                      const maxLiq = Math.max(1, ...monthlyReport.rows.map((r) => r.liquido));
                      return monthlyReport.rows.map((r) => (
                        <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                          <div style={{ width: 60, fontSize: 10, fontWeight: 700, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>{r.placa}</div>
                          <div style={{ flex: 1, background: "#EEF0F2", borderRadius: 4, height: 16, position: "relative" }}>
                            <div style={{ width: `${Math.max(2, (r.liquido / maxLiq) * 100)}%`, height: "100%", background: "#1B2E6B", borderRadius: 4 }} />
                          </div>
                          <div style={{ width: 90, fontSize: 10, fontWeight: 700, textAlign: "right", color: r.liquido >= 0 ? "#12503F" : "#B0402E" }}>{BRL(r.liquido)}</div>
                        </div>
                      ));
                    })()}
                  </div>
                  ...
                  <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 10 }}>🍩 DISTRIBUIÇÃO DAS DESPESAS (sobre a receita)</div>
                    <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
                      {(() => {
                        const t = monthlyReport.totals;
                        const cats = [
                          { nome: "Abastecimento", valor: t.abastecimentoTotal, cor: "#2451A6" },
                          { nome: "Comissões", valor: t.comissaoTotal, cor: "#D9A419" },
                          { nome: "Despesas do Veículo", valor: t.despesasVeiculoTotal, cor: "#B0402E" },
                          { nome: "Gastos Extras", valor: t.gastosTotal, cor: "#1F6F5C" },
                          { nome: "Carregamento", valor: t.carregamentoTotal, cor: "#7A3FBF" },
                        ].map((c) => ({ ...c, pct: t.receita > 0 ? (c.valor / t.receita) * 100 : 0 }));
                        const margem = t.receita > 0 ? (t.liquido / t.receita) * 100 : 0;
                        const r = 40, cx = 50, cy = 50, circunf = 2 * Math.PI * r;
                        let acumulado = 0;
                        return (
                          <>
                            <div style={{ position: "relative", width: 120, height: 120, flexShrink: 0 }}>
                              <svg width="120" height="120" viewBox="0 0 100 100" style={{ transform: "rotate(-90deg)" }}>
                                <circle cx={cx} cy={cy} r={r} fill="none" stroke="#EEF0F2" strokeWidth="16" />
                                {cats.map((c) => {
                                  const seg = (c.pct / 100) * circunf;
                                  const offset = -((acumulado / 100) * circunf);
                                  acumulado += c.pct;
                                  return <circle key={c.nome} cx={cx} cy={cy} r={r} fill="none" stroke={c.cor} strokeWidth="16" strokeDasharray={`${seg} ${circunf - seg}`} strokeDashoffset={offset} />;
                                })}
                              </svg>
                              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                <div style={{ fontWeight: 700, fontSize: 15 }}>{margem.toFixed(1)}%</div>
                                <div style={{ fontSize: 7, color: "#9AA5B1", fontWeight: 700 }}>MARGEM LÍQUIDA</div>
                              </div>
                            </div>
                            <table style={{ fontSize: 10, borderCollapse: "collapse", flex: 1, minWidth: 200 }}>
                              <tbody>
                                {cats.map((c) => (
                                  <tr key={c.nome}>
                                    <td style={{ padding: "2px 6px 2px 0" }}><span style={{ display: "inline-block", width: 8, height: 8, borderRadius: 2, background: c.cor, marginRight: 4 }} />{c.nome}</td>
                                    <td style={{ padding: "2px 6px", textAlign: "right" }}>{BRL(c.valor)}</td>
                                    <td style={{ padding: "2px 0", textAlign: "right", color: "#9AA5B1" }}>{c.pct.toFixed(1)}%</td>
                                  </tr>
                                ))}
                                <tr style={{ borderTop: "1px solid #E4E7EB", fontWeight: 700 }}>
                                  <td style={{ padding: "4px 6px 0 0" }}>Receita Total</td>
                                  <td style={{ padding: "4px 6px 0", textAlign: "right" }}>{BRL(t.receita)}</td>
                                  <td style={{ padding: "4px 0 0", textAlign: "right" }}>100%</td>
                                </tr>
                              </tbody>
                            </table>
                          </>
            ...
            ) : detailedReport.groups.length === 0 ? (
              <div style={{ padding: "30px 10px", textAlign: "center", color: "#5A6472", border: "1px dashed #D7DBE0", borderRadius: 12 }}>
                Nenhuma viagem lançada nesse mês ainda.
              </div>
            ) : (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #2451A6", paddingBottom: 14, marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#2451A6", letterSpacing: 1 }}>RELATÓRIO DETALHADO</div>
                    <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>VIAGENS E DESPESAS</div>
                    <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>Relatório de viagens com lançamentos de despesa</div>
                    <div style={{ fontSize: 10, color: "#5A6472" }}>Referência: {reportMonth} · {filterTruck === "all" ? "Todos os caminhões" : truckLabel(filterTruck)}</div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
                      <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>PERÍODO</div>
                    ...
                    <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
                      <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🚚 VEÍCULO</div>
                      <div style={{ fontWeight: 700, fontSize: 11, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>{filterTruck === "all" ? "Todos os caminhões" : truckLabel(filterTruck)}</div>
                    </div>
                  </div>
                </div>

                <div style={{ display: "flex", gap: 8, marginBottom: 14, fontSize: 10, fontWeight: 700, flexWrap: "wrap" }}>
                  <span style={{ color: "#1F6F5C", background: "#E9F5F1", borderRadius: 20, padding: "4px 12px" }}>✓ RECEITA</span>
                  <span style={{ color: "#8A5A00", background: "#FFF6E2", borderRadius: 20, padding: "4px 12px" }}>👤 COMISSÃO</span>
                  <span style={{ color: "#1B4E8C", background: "#EAF2FB", borderRadius: 20, padding: "4px 12px" }}>⛽ COMBUSTÍVEL</span>
                  <span style={{ color: "#7A2A1D", background: "#FBEBE8", borderRadius: 20, padding: "4px 12px" }}>🧾 GASTOS EXTRAS</span>
                  <span style={{ color: "#3A4351", background: "#EEF0F2", borderRadius: 20, padding: "4px 12px" }}>📋 OUTROS</span>
                </div>

                <div style={{ background: "#1B2430", color: "#fff", padding: "7px 12px", fontWeight: 700, fontSize: 12, marginBottom: 12, letterSpacing: 0.5, borderRadius: 6 }}>
                  DETALHAMENTO DAS VIAGENS
                </div>

                {detailedReport.groups.map((g) => (
                  <div key={g.id} style={{ marginBottom: 16 }}>
                    <div style={{ background: "#1B2430", color: "#fff", padding: "6px 12px", fontWeight: 700, fontSize: 13, marginBottom: 8, borderRadius: 6, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>
                      🚚 {g.placa}
                    </div>
                    {g.tripRows.map((t) => (
                      <div key={t.id} style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}>
                        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                          <div style={{ minWidth: 90, flexShrink: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                              <div style={{ width: 24, height: 24, borderRadius: 6, background: "#2451A6", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                                {String(t.numero).padStart(2, "0")}
                              </div>
                              <div style={{ fontSize: 10, color: "#5A6472" }}>{fmtDate(t.data)}</div>
                            </div>
                            <div style={{ fontSize: 9, color: "#5A6472", marginBottom: 6 }}>Nº {t.codigo}</div>
                            <div style={{ fontSize: 9, color: "#5A6472" }}>Motorista:<br />{t.motorista}</div>
                          </div>
                          <div style={{ flex: 1, minWidth: 240 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 8 }}>
                              <div style={{ fontWeight: 700, fontSize: 15 }}>📍 {t.origem} → {t.destino}</div>
                              <div style={{ textAlign: "right" }}>
                                <div style={{ fontSize: 8, color: "#9AA5B1", textTransform: "uppercase", fontWeight: 700 }}>Total da viagem</div>
                                <div style={{ fontWeight: 700, fontSize: 15, color: t.liquidoViagem >= 0 ? "#1F6F5C" : "#B0402E" }}>{BRL(t.liquidoViagem)}</div>
                              </div>
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                              <div style={{ display: "flex", flexDirection: "column" }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#1F6F5C", marginBottom: 3 }}>💰 RECEITA</div>
                                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{t.origem} - {t.destino}</span><span>{BRL(t.valorViagem)}</span></div>
                                <div style={{ fontSize: 10, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#1F6F5C" }}>Total: {BRL(t.valorViagem)}</div>
                              </div>
                              <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#8A5A00", marginBottom: 3 }}>👤 COMISSÃO</div>
                                {t.comissaoItens.length === 0 ? <div style={{ fontSize: 10, color: "#9AA5B1" }}>—</div> : t.comissaoItens.map((c, i) => (
                                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{c.descricao}</span><span>−{BRL(c.valor)}</span></div>
                                ))}
                                <div style={{ fontSize: 10, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#8A5A00" }}>Total: −{BRL(t.totalComissao)}</div>
                              </div>
                              <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#1B4E8C", marginBottom: 3 }}>⛽ ABASTECIMENTOS</div>
                                {t.abastecimentos.length === 0 ? <div style={{ fontSize: 10, color: "#9AA5B1" }}>—</div> : t.abastecimentos.map((a, i) => (
                                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{fmtDate(a.data)} {a.posto}</span><span>{BRL(a.valor)}</span></div>
                                ))}
                                <div style={{ fontSize: 10, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#1B4E8C" }}>Total: −{BRL(t.totalCombustivel)}</div>
                              </div>
                              <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#7A2A1D", marginBottom: 3 }}>🧾 GASTOS EXTRAS</div>
                                {t.gastosExtrasItens.length === 0 ? <div style={{ fontSize: 10, color: "#9AA5B1" }}>—</div> : t.gastosExtrasItens.map((ge, i) => (
                                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{ge.descricao}</span><span>{BRL(ge.valor)}</span></div>
                                ))}
                                <div style={{ fontSize: 10, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#7A2A1D" }}>Total: −{BRL(t.totalGastosExtras)}</div>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: g.despesasVeiculoTruck.length > 0 ? "1fr 1fr" : "1fr", gap: 10, alignItems: "start", flexWrap: "wrap" }}>
                      {g.despesasVeiculoTruck.length > 0 && (
                        <div style={{ border: "1px solid #E4E7EB", borderRadius: 8, overflow: "hidden" }}>
                          <div style={{ background: "#FBEBE8", color: "#B0402E", fontWeight: 700, fontSize: 11, padding: "6px 12px" }}>🧾 DESPESAS DO VEÍCULO</div>
                          {g.despesasVeiculoTruck.map((d) =>
                            editingDespesaId === d.id ? (
                              <div key={d.id} style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10 }}>
                                <Field label="Data">
                                  <input type="date" style={inputStyle} value={despesaData} onChange={(e) => setDespesaData(e.target.value)} onBlur={(e) => setDespesaData(corrigirAnoDigitado(e.target.value))} />
                                </Field>
                                <Field label="Descrição">
                              ...
                              <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", fontSize: 11, borderTop: "1px solid #EEF0F2" }}>
                                ...
                                <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                  ...
                                  <button onClick={() => startEditDespesa(d)} style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 11 }}>editar</button>
                                  <button onClick={() => deleteDespesaVeiculo(d.id)} style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 12 }}>×</button>
                                </span>
                              </div>
                            )
                          )}
                      ...
                      <div style={{ border: "1px solid #E4E7EB", borderRadius: 8, overflow: "hidden", fontSize: 11 }}>
                        <div style={{ background: "#1B2E6B", color: "#fff", fontWeight: 700, padding: "6px 12px" }}>RESUMO DO VEÍCULO ({g.placa} · período)</div>
                        <div style={{ padding: "8px 12px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Total de Viagens (Receita − Comissões)</span><strong>{BRL(g.resumo.totalViagens)}</strong></div>
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Total de Combustível</span><strong>{BRL(g.resumo.totalCombustivel)}</strong></div>
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Gastos Extras</span><strong>{BRL(g.resumo.totalGastosExtras)}</strong></div>
                          <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Despesas do Veículo</span><strong>{BRL(g.resumo.despesasVeiculo)}</strong></div>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", background: "#1B2E6B", color: "#fff", padding: "7px 12px", fontWeight: 700 }}>
                          <span>TOTAL LÍQUIDO DO VEÍCULO</span><span>{BRL(g.totalVeiculo)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginTop: 8 }}>
                  <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 8 }}>📊 TOTAL DO PERÍODO</div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span>Receita Total</span><strong style={{ color: "#1F6F5C" }}>{BRL(detailedReport.resumoGeral.receitaGeral)}</strong></div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span>Despesas Totais</span><strong style={{ color: "#B0402E" }}>{BRL(detailedReport.resumoGeral.despesasTotaisGeral)}</strong></div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span>Comissões</span><strong style={{ color: "#8A5A00" }}>{BRL(detailedReport.resumoGeral.comissaoGeral)}</strong></div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: 6, paddingTop: 6 }}>
                      <span>Resultado Líquido</span><span style={{ color: detailedReport.resumoGeral.resultadoLiquidoGeral >= 0 ? "#1F6F5C" : "#B0402E" }}>{BRL(detailedReport.resumoGeral.resultadoLiquidoGeral)}</span>
                    </div>
                  </div>

                  <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 14, display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 8, alignSelf: "flex-start" }}>🍩 DISTRIBUIÇÃO DA RECEITA</div>
                    {(() => {
                      const r = 40, cx = 50, cy = 50, circunf = 2 * Math.PI * r;
                      const pctLiq = detailedReport.resumoGeral.pctReceitaLiquida;
                      const pctCom = detailedReport.resumoGeral.pctComissoes;
                      const pctDesp = Math.max(0, 100 - pctLiq - pctCom);
                      const segLiq = (pctLiq / 100) * circunf;
                      const segCom = (pctCom / 100) * circunf;
                      const segDesp = (pctDesp / 100) * circunf;
                      return (
                        <div style={{ position: "relative", width: 110, height: 110, margin: "4px 0 10px" }}>
                          <svg width="110" height="110" viewBox="0 0 100 100" style={{ transform: "rotate(-90deg)" }}>
                            <circle cx={cx} cy={cy} r={r} fill="none" stroke="#EEF0F2" strokeWidth="16" />
                            <circle cx={cx} cy={cy} r={r} fill="none" stroke="#1F6F5C" strokeWidth="16" strokeDasharray={`${segLiq} ${circunf - segLiq}`} strokeDashoffset="0" />
                            <circle cx={cx} cy={cy} r={r} fill="none" stroke="#D9A419" strokeWidth="16" strokeDasharray={`${segCom} ${circunf - segCom}`} strokeDashoffset={-segLiq} />
                            <circle cx={cx} cy={cy} r={r} fill="none" stroke="#B0402E" strokeWidth="16" strokeDasharray={`${segDesp} ${circunf - segDesp}`} strokeDashoffset={-(segLiq + segCom)} />
                          </svg>
                          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 17 }}>
                            {pctLiq.toFixed(0)}%
                          </div>
                        </div>
                      );
                    ...
                    <div style={{ fontSize: 11, width: "100%" }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#1F6F5C" }}>● Receita Líquida</span><span>{BRL(detailedReport.resumoGeral.resultadoLiquidoGeral)}</span></div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#D9A419" }}>● Comissões</span><span>{BRL(detailedReport.resumoGeral.comissaoGeral)}</span></div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#B0402E" }}>● Despesas Totais</span><span>{BRL(detailedReport.resumoGeral.despesasTotaisGeral)}</span></div>
                    </div>
                  </div>

                  <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 10 }}>📈 DESPESAS POR CATEGORIA</div>
                    {detailedReport.resumoGeral.categorias.map((c) => (
                      <div key={c.nome} style={{ marginBottom: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                          <span>{c.nome}</span>
                          <span style={{ fontWeight: 700 }}>{BRL(c.valor)} ({c.pct.toFixed(1)}%)</span>
                        </div>
                        <div style={{ background: "#EEF0F2", borderRadius: 4, height: 7, overflow: "hidden" }}>
        ...
        >
          ...
          >
            ...
            {relatorioRecebimento.items.length === 0 ? (
              ...
              <>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>📄</div>
                    <div>
                      <div style={{ fontSize: 17, fontWeight: 700 }}>{relatorioRecebimento.items.length}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>RECEBIMENTOS</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>💰</div>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#8A5A00" }}>{BRL(relatorioRecebimento.totalAdiantamento)}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>ADIANTAMENTOS</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>✅</div>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#1F6F5C" }}>{BRL(relatorioRecebimento.totalSaldo)}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>SALDOS</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>TOTAL RECEBIDO</div>
                    <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(relatorioRecebimento.total)}</div>
                  </div>
                </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: "#1B2E6B" }}>
                      {["Data da viagem", "Data recebido", "Caminhão", "Contrato", "Empresa", "Tipo", "Valor"].map((h, i) => (
                        <th key={h} style={{ textAlign: i === 6 ? "right" : "left", padding: "9px 10px", color: "#fff", fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 14 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {relatorioRecebimento.items.map((r, i) => (
                      <tr key={`${r.trip.id}-${r.tipo}`} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(r.trip.data)}</td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(r.data)}</td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", fontWeight: 600 }}>{truckLabel(r.trip.caminhaoId)}</td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>{r.trip.contrato || ""}</td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>{r.trip.empresa || ""}</td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>
                          <span style={{ background: r.tipo === "Adiantamento" ? "#FFF6E2" : "#E9F5F1", color: r.tipo === "Adiantamento" ? "#8A5A00" : "#1F6F5C", fontWeight: 700, fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>{r.tipo.toUpperCase()}</span>
                        </td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700, color: "#1F6F5C" }}>{BRL(r.valor)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>🚚</div>
                    <div>
                      <div style={{ fontSize: 17, fontWeight: 700 }}>{relatorioGastosExtras.rows.length}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>CAMINHÕES COM GASTO</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>🧾</div>
                    <div>
                      <div style={{ fontSize: 17, fontWeight: 700 }}>{relatorioGastosExtras.rows.reduce((s, r) => s + r.gastos.length, 0)}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>LANÇAMENTOS</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>TOTAL GERAL</div>
                    <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(relatorioGastosExtras.totalGeral)}</div>
                  </div>
                </div>

                {relatorioGastosExtras.rows.map((r) => (
                  <div key={r.caminhaoId} style={{ marginBottom: 18 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: "#fff", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>🚚 {r.placa}</div>
                      <div style={{ fontWeight: 700, fontSize: 14, color: "#fff" }}>{BRL(r.total)}</div>
                    </div>
                      ...
                      <tbody>
                        {r.gastos.map((g, i) => (
                          <tr key={g.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                            ...
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {g.descricao || ""}
                              {g.estabelecimento && <div style={{ fontSize: 10, color: "#9AA5B1" }}>{g.estabelecimento}</div>}
                            </td>
        ...
        >
          ...
          >
            ...
            {consumoCaminhaoFiltro === "all" && relatorioConsumo.gruposPorPlaca.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: "#9AA5B1", textTransform: "uppercase", fontWeight: 700, marginBottom: 6 }}>Ranking de consumo (melhor pro pior) nesse período</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {[...relatorioConsumo.gruposPorPlaca]
                    .sort((a, b) => (b.mediaGeral ?? -1) - (a.mediaGeral ?? -1))
                    .map((g, idx, arr) => {
                      const comMedia = arr.filter((x) => x.mediaGeral != null);
                      const ehMelhor = comMedia.length > 1 && g === comMedia[0];
                      const ehPior = comMedia.length > 1 && g === comMedia[comMedia.length - 1];
                      return (
                        <button
                          key={g.placa}
                          ...
                          className="linha-clicavel"
                          style={{
                            background: ehMelhor ? "#E9F5F1" : ehPior ? "#FBEBE8" : "#F7F8F9",
                            border: `1px solid ${ehMelhor ? "#1F6F5C55" : ehPior ? "#B0402E55" : "#E4E7EB"}`,
                            borderRadius: 10, padding: "8px 12px", cursor: "pointer", textAlign: "left", position: "relative",
                          }}
                        >
                          <div style={{ fontSize: 11, color: "#9AA5B1", fontWeight: 700 }}>
                            {g.mediaGeral != null ? `${idx + 1}º` : "—"} {ehMelhor && "🏆"} {ehPior && "⚠️"}
                          </div>
              ...
              <>
                {relatorioConsumo.gruposPorPlaca.map((g) => (
                  <div key={g.placa} style={{ marginBottom: 18 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: "#fff", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>🚚 {g.placa}</div>
      ...
      {relatorioServicosOpen && (
        ...
        >
          <div
            ...
            style={{ width: "min(1300px, 98vw)", maxHeight: "94vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 22 }}>
            ...
            <div className="filtros-relatorio" style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <Field label="Placa">
                <select style={inputStyle} value={servicosPlacaFiltro} onChange={(e) => setServicosPlacaFiltro(e.target.value)}>
                  <option value="all">Todas</option>
                  {trucksCavalos.map((tr) => <option key={tr.id} value={tr.id}>{tr.placa}</option>)}
                </select>
              </Field>
              ...
              <>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
                  <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>🔧</div>
                    <div>
                      <div style={{ fontSize: 17, fontWeight: 700 }}>{relatorioServicos.items.length}</div>
                      <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>SERVIÇOS</div>
                    </div>
                  </div>
                  <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>TOTAL</div>
                    <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(relatorioServicos.items.reduce((s, l) => s + l.valor, 0))}</div>
                  </div>
                </div>
                {relatorioServicos.porPlaca.map((grupo) => (
                  <div key={grupo.caminhaoId} style={{ marginBottom: 18 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: "#fff", fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>🚚 {grupo.placa}</div>
                      <div style={{ fontWeight: 700, fontSize: 14, color: "#fff" }}>{BRL(grupo.total)}</div>
                    </div>
                    <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, overflow: "hidden" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        <thead>
                          ...
                          {grupo.items.map((l, i) => (
                            <tr
                              ...
                              onClick={() => {
                                ...
                                } else if (l.origem === "trocaOleo") {
                                  const troca = trocasOleo.find((t) => t.id === l.id);
                                  if (troca) { setRelatorioServicosOpen(false); setView("trocaoleo"); startEditTrocaOleo(troca); }
                                } else {
      ...
      {relatorioContadorOpen && (
        <div
          onClick={() => setRelatorioContadorOpen(false)}
          style={{ position: "fixed", inset: 0, background: "rgba(27,36,48,0.45)", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="modal-relatorio"
            style={{ width: "min(1000px, 98vw)", maxHeight: "94vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 22 }}>
            ...
            <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 14 }}>
              Resumo fiscal simplificado de um mês: receita bruta e despesas categorizadas, pronto pra mandar.
            </div>

            <div className="filtros-relatorio" style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="Mês">
                <input type="month" style={inputStyle} value={contadorMes} onChange={(e) => setContadorMes(e.target.value)} />
              </Field>
              <button onClick={() => window.print()} style={{ background: "#6B4423", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                Baixar PDF
              </button>
            ...
            <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 10 }}>{relatorioContador.qtdViagens} viagem{relatorioContador.qtdViagens !== 1 ? "ns" : ""} no mês</div>

            <div style={{ background: "#F7F8F9", borderRadius: 12, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 700, marginBottom: 10 }}>
                <span>Receita bruta</span>
                <span style={{ color: "#1F6F5C" }}>{BRL(relatorioContador.receita)}</span>
              </div>
              {[
                { label: "Comissões", valor: relatorioContador.comissoesMes },
                { label: "Gastos extras (estrada)", valor: relatorioContador.gastosEstrada },
                { label: `Boletos pagos (${relatorioContador.boletosDoMes.length})`, valor: relatorioContador.boletosMes },
                { label: "Taxa de viagem (rateio)", valor: relatorioContador.taxaMes },
              ].map((linha) => (
                <div key={linha.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#5A6472", padding: "4px 0" }}>
                  <span>{linha.label}</span>
                  <span style={{ color: "#B0402E" }}>− {BRL(linha.valor)}</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, borderTop: "1px solid #D7DBE0", marginTop: 6, paddingTop: 6, fontWeight: 700 }}>
                <span>Total de despesas</span>
                <span style={{ color: "#B0402E" }}>− {BRL(relatorioContador.gastosTotais)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 16, fontWeight: 700, marginTop: 10, background: relatorioContador.liquido >= 0 ? "#E9F5F1" : "#FBEBE8", borderRadius: 10, padding: "10px 12px" }}>
                <span>Líquido do mês</span>
                <span style={{ color: relatorioContador.liquido >= 0 ? "#12503F" : "#B0402E" }}>{BRL(relatorioContador.liquido)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#9AA5B1", marginTop: 6 }}>
                <span>Total abastecido (dentro da receita, sem entrar nas despesas)</span>
                <span>{BRL(relatorioContador.abastecimentosMes)}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              {abastecReport.porPosto.map((grupo) => (
                <div key={grupo.posto} style={{ background: "#fff", borderRadius: 12, padding: 14 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8, color: "#6B4423" }}>{grupo.posto}</div>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
                    <div style={{ flex: "1 1 140px", background: "linear-gradient(160deg, #fff 0%, #FBEBE8 100%)", border: "1px solid #B0402E33", borderRadius: 10, padding: "8px 12px" }}>
                      <div style={{ fontSize: 10, color: "#8A5A5A", textTransform: "uppercase", fontWeight: 700 }}>Total</div>
                      <div style={{ fontSize: 17, fontWeight: 700, color: "#7A2A1D" }}>{BRL(grupo.valor)}</div>
                    </div>
                    {grupo.litragemDiesel > 0 && (
                      <div style={{ flex: "1 1 140px", background: "linear-gradient(160deg, #fff 0%, #EAF2FB 100%)", border: "1px solid #2451A633", borderRadius: 10, padding: "8px 12px" }}>
                        <div style={{ fontSize: 10, color: "#4A72A8", textTransform: "uppercase", fontWeight: 700 }}>Diesel</div>
                        <div style={{ fontSize: 17, fontWeight: 700, color: "#1B4E8C" }}>{BRL(grupo.valorDiesel)}</div>
                        <div style={{ fontSize: 11, color: "#9AA5B1" }}>{formatLitros(grupo.litragemDiesel)} L</div>
                      </div>
                    )}
                    {grupo.litragemArla > 0 && (
                      <div style={{ flex: "1 1 140px", background: "linear-gradient(160deg, #fff 0%, #EAF2FB 100%)", border: "1px solid #2451A633", borderRadius: 10, padding: "8px 12px" }}>
                        <div style={{ fontSize: 10, color: "#4A72A8", textTransform: "uppercase", fontWeight: 700 }}>Arla</div>
                        <div style={{ fontSize: 17, fontWeight: 700, color: "#1B4E8C" }}>{BRL(grupo.valorArla)}</div>
                        <div style={{ fontSize: 11, color: "#9AA5B1" }}>{formatLitros(grupo.litragemArla)} L</div>
                      </div>
                    )}
                  </div>
      ...
      {view === "trocaoleo" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <Field label="Valor (R$)">
                    <input type="number" style={{ ...inputStyle, width: 90 }} value={trocaOleoValor} onChange={(e) => setTrocaOleoValor(e.target.value)} />
                  </Field>
                  ...
                  <Field label={<>Empresa <span title="Preenche se a troca foi feita numa empresa cadastrada (ex: Força Diesel)" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <select style={inputStyle} value={trocaOleoEmpresa} onChange={(e) => { setTrocaOleoEmpresa(e.target.value); if (!e.target.value) setTrocaOleoGerarBoleto(false); }}>
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#2451A6", marginBottom: 6 }}>Peças usadas (opcional)</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {trocaOleoPecas.map((peca) => (
                      <div key={peca.id} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        <input
                          style={{ ...inputStyle, flex: 1 }}
                          value={peca.descricao}
                          onChange={(e) => setTrocaOleoPecas(trocaOleoPecas.map((p) => (p.id === peca.id ? { ...p, descricao: e.target.value } : p)))}
                        ...
                        <input
                          ...
                          value={peca.valor}
                          onChange={(e) => setTrocaOleoPecas(trocaOleoPecas.map((p) => (p.id === peca.id ? { ...p, valor: e.target.value } : p)))}
                          placeholder="Valor R$"
                        />
                        <button
                          onClick={() => setTrocaOleoPecas(trocaOleoPecas.filter((p) => p.id !== peca.id))}
                          style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 18 }}
                        >
                          ×
                        </button>
                      </div>
                  ...
                  <button
                    onClick={() => setTrocaOleoPecas([...trocaOleoPecas, { id: uid(), descricao: "", valor: "" }])}
                    style={{ background: "none", border: "1px dashed #C7CFFA", color: "#2451A6", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 4 }}
                  >
                    + adicionar peça
                  </button>
                  {trocaOleoPecas.filter((p) => Number(p.valor) > 0).length > 0 && (
                    <div style={{ fontSize: 12, color: "#5A6472", marginTop: 6 }}>
                      Total com peças: <strong>{BRL((Number(trocaOleoValor) || 0) + trocaOleoPecas.reduce((s, p) => s + (Number(p.valor) || 0), 0))}</strong>
                    </div>
                  )}
                </div>
                ...
                {trocaOleoEmpresa && (
                  <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#5A6472", marginTop: 10 }}>
                    <input
                      type="checkbox"
                      checked={trocaOleoGerarBoleto}
                      onChange={(e) => setTrocaOleoGerarBoleto(e.target.checked)}
                    />
                    Gerar boleto pendente pra {trocaOleoEmpresa} no valor de {BRL((Number(trocaOleoValor) || 0) + trocaOleoPecas.reduce((s, p) => s + (Number(p.valor) || 0), 0))}
                  </label>
                )}
                {trocaOleoEmpresa && trocaOleoGerarBoleto && (
                  <div style={{ marginTop: 10 }}>
                    <Field label="Vencimento do boleto">
                      <input type="date" style={inputStyle} value={trocaOleoVencimentoBoleto} onChange={(e) => setTrocaOleoVencimentoBoleto(e.target.value)} onBlur={(e) => setTrocaOleoVencimentoBoleto(corrigirAnoDigitado(e.target.value))} />
                    </Field>
                  </div>
                )}
          ...
          {/* relatorio por periodo — troca de óleo e outros serviços juntos, por placa */}
          <div style={{ background: "#fff", borderRadius: 12, padding: 16 }}>
            <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 18, marginBottom: 12 }}>
              Histórico de trocas e outros serviços
            </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <button
                onClick={() => startAddServico(oleoPlacaFilter !== "all" ? oleoPlacaFilter : undefined)}
                style={{ border: "2px dashed #8A94D9", background: "transparent", borderRadius: 10, padding: "9px 14px", fontSize: 13, color: "#2E3A8C", cursor: "pointer" }}
              >
              ...
              <button onClick={exportServicoVeiculoCSV} style={{ background: "#2E3A8C", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                Baixar CSV (serviços)
              </button>
              <button onClick={() => window.print()} style={{ background: "#6B4423", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                Baixar PDF
            ...
            {trocaOleoReport.items.length === 0 && servicoVeiculoReport.items.length === 0 ? (
              ...
              (() => {
                const trocasPorPlaca = {};
                trocaOleoReport.items.forEach((t) => {
                  const placa = truckLabel(t.caminhaoId);
                  (trocasPorPlaca[placa] = trocasPorPlaca[placa] || []).push(t);
                });
                const servicosPorPlaca = {};
                servicoVeiculoReport.items.forEach((s) => {
                  const placa = truckLabel(s.caminhaoId);
                  (servicosPorPlaca[placa] = servicosPorPlaca[placa] || []).push(s);
                });
                const todasAsPlacas = Array.from(new Set([...Object.keys(trocasPorPlaca), ...Object.keys(servicosPorPlaca)])).sort();
                return todasAsPlacas.map((placa) => {
                  const itensTroca = trocasPorPlaca[placa] || [];
                  const itensServico = servicosPorPlaca[placa] || [];
                  return (
                    <div key={placa} style={{ marginBottom: 24 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8, background: "#1B2E6B", color: "#fff", borderRadius: 6, padding: "6px 12px" }}>
                        🚚 {placa}
                      </div>
                      {itensTroca.length > 0 && (
                        <div style={{ marginBottom: 10 }}>
                          ...
                          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                            <thead>
                              <tr style={{ background: "#1B2E6B" }}>
                                {["Data", "KM", "Filtro", "Observação", "Valor", "Empresa", ""].map((h) => (
                                  <th key={h} style={{ textAlign: "left", padding: "8px 10px", fontFamily: "'Inter', sans-serif", fontWeight: 700, color: "#fff" }}>{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {itensTroca.map((t) => (
                                <tr key={t.id} style={{ borderBottom: "1px solid #EEF0F2" }}>
                                  ...
                                  <td style={{ padding: "8px 10px", color: "#5A6472" }}>
                                    {t.observacao || "—"}
                                    {(t.pecas || []).length > 0 && (
                                      <div style={{ fontSize: 10, color: "#8A5A00" }}>
                                        + {t.pecas.length} peça{t.pecas.length !== 1 ? "s" : ""} ({BRL(t.pecas.reduce((s, p) => s + (Number(p.valor) || 0), 0))})
                                      </div>
                                    )}
                                  </td>
                                  <td style={{ padding: "8px 10px", fontWeight: 700 }}>
                                    {(() => {
                                      const total = (Number(t.valor) || 0) + (t.pecas || []).reduce((s, p) => s + (Number(p.valor) || 0), 0);
                                      return total > 0 ? BRL(total) : "—";
                                    })()}
                                  </td>
                                  <td style={{ padding: "8px 10px", color: "#5A6472" }}>{t.empresa || "Estrada"}</td>
                      ...
                      {itensServico.length > 0 && (
                        <div>
                          ...
                          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, background: "#EEF0FF", borderRadius: 8 }}>
                            <thead>
                              <tr style={{ background: "#1B2E6B" }}>
                                {["Data", "KM", "Serviço", "Peças", "Empresa", "Observação", "Valor", ""].map((h) => (
                                  <th key={h} style={{ textAlign: "left", padding: "8px 10px", fontFamily: "'Inter', sans-serif", fontWeight: 700, color: "#fff" }}>{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {itensServico.map((s) => (
                                <tr key={s.id} style={{ borderBottom: "1px solid #D7DBE0" }}>
                                  ...
                                  <td style={{ padding: "8px 10px", color: "#5A6472" }}>
                                    {(s.pecas || []).length > 0
                                      ? s.pecas.map((p) => `${p.descricao}${Number(p.valor) > 0 ? ` (${BRL(Number(p.valor))})` : ""}`).join(", ")
                                      : "—"}
                                  </td>
                                  <td style={{ padding: "8px 10px", color: "#5A6472" }}>{s.empresa || "Estrada"}</td>
                                  <td style={{ padding: "8px 10px", color: "#5A6472" }}>{s.observacao || "—"}</td>
                                  <td style={{ padding: "8px 10px", fontWeight: 700 }}>{s.valor > 0 ? BRL(s.valor) : "—"}</td>
                                  <td style={{ padding: "8px 10px", textAlign: "right" }}>
                                    {s.origem === "gasto" ? (
                                      <button
                                        onClick={() => {
                                      ...
                                      <>
                                        <button onClick={() => { const original = servicosVeiculo.find((sv) => sv.id === s.id); startEditServico(original || s); }} style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 13, marginRight: 10 }}>editar</button>
          ...
          {addingServico && (
            ...
            >
              <div
                onClick={(e) => e.stopPropagation()}
                    style={{ width: "min(680px, 96vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 20 }}
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#2451A6", marginBottom: 6 }}>Serviços feitos (descrição e valor de cada um)</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {servicoItens.map((item, idx) => (
                          <div key={item.id} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            <input
                              style={{ ...inputStyle, flex: 1 }}
                              value={item.descricao}
                              onChange={(e) => setServicoItens(servicoItens.map((i) => (i.id === item.id ? { ...i, descricao: e.target.value } : i)))}
                            ...
                            <input
                              ...
                              style={{ ...inputStyle, width: 110 }}
                              value={item.valor}
                              onChange={(e) => setServicoItens(servicoItens.map((i) => (i.id === item.id ? { ...i, valor: e.target.value } : i)))}
                              placeholder="Valor R$"
                            />
                            {servicoItens.length > 1 && (
                              <button
                                onClick={() => setServicoItens(servicoItens.filter((i) => i.id !== item.id))}
                                style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 18 }}
                              >
                                ×
                              </button>
                      ...
                      <button
                        onClick={() => setServicoItens([...servicoItens, { id: uid(), descricao: "", valor: "" }])}
                        style={{ background: "none", border: "1px dashed #C7CFFA", color: "#2451A6", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 8 }}
                      >
                        + adicionar outro serviço
                      </button>
                      {servicoItens.filter((i) => Number(i.valor) > 0).length > 0 && (
                        <div style={{ fontSize: 12, color: "#5A6472", marginTop: 6 }}>
                          Total: <strong>{BRL(servicoItens.reduce((s, i) => s + (Number(i.valor) || 0), 0))}</strong>
                        </div>
                      )}
                    </div>
                    <div style={{ marginTop: 10 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#2451A6", marginBottom: 6 }}>Peças usadas (opcional)</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {servicoPecas.map((peca) => (
                          <div key={peca.id} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            <input
                              style={{ ...inputStyle, flex: 1 }}
                              value={peca.descricao}
                              onChange={(e) => setServicoPecas(servicoPecas.map((p) => (p.id === peca.id ? { ...p, descricao: e.target.value } : p)))}
                              placeholder="ex: Filtro de óleo"
                            />
                            <input
                              type="number"
                              style={{ ...inputStyle, width: 110 }}
                              value={peca.valor}
                              onChange={(e) => setServicoPecas(servicoPecas.map((p) => (p.id === peca.id ? { ...p, valor: e.target.value } : p)))}
                              placeholder="Valor R$"
                            />
                            <button
                              onClick={() => setServicoPecas(servicoPecas.filter((p) => p.id !== peca.id))}
                              style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 18 }}
                            >
                              ×
                            </button>
                      ...
                      <button
                        onClick={() => setServicoPecas([...servicoPecas, { id: uid(), descricao: "", valor: "" }])}
                        style={{ background: "none", border: "1px dashed #C7CFFA", color: "#2451A6", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 4 }}
                      >
                        + adicionar peça
                      </button>
                      {servicoPecas.filter((p) => Number(p.valor) > 0).length > 0 && (
                        <div style={{ fontSize: 12, color: "#5A6472", marginTop: 6 }}>
                          Total peças: <strong>{BRL(servicoPecas.reduce((s, p) => s + (Number(p.valor) || 0), 0))}</strong>
                        </div>
                      )}
                    </div>
                    {(servicoItens.some((i) => Number(i.valor) > 0) || servicoPecas.some((p) => Number(p.valor) > 0)) && (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, background: "#EEF0FF", borderRadius: 10, padding: "10px 14px" }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: "#2E3A8C" }}>Valor total (serviços + peças)</span>
                        <span style={{ fontSize: 17, fontWeight: 700, color: "#2E3A8C" }}>
                          {BRL(
                            servicoItens.reduce((s, i) => s + (Number(i.valor) || 0), 0) +
                            servicoPecas.reduce((s, p) => s + (Number(p.valor) || 0), 0)
                          )}
                        </span>
                      </div>
                    )}
                    <div style={{ marginTop: 10 }}>
                      <Field label="Observação (opcional)">
                        <textarea
                          style={{ ...inputStyle, width: "100%", minHeight: 60, resize: "vertical", fontFamily: "'Inter', sans-serif" }}
                          value={servicoObs}
                          onChange={(e) => setServicoObs(e.target.value)}
                    ...
                    {servicoEmpresa && (
                      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#5A6472", marginTop: 10 }}>
                        <input
                          ...
                          onChange={(e) => {
                            setServicoGerarBoleto(e.target.checked);
                            if (e.target.checked && !servicoValorBoleto) {
                              const total =
                                servicoItens.filter((i) => i.descricao.trim()).reduce((s, i) => s + (Number(i.valor) || 0), 0) +
                                servicoPecas.filter((p) => p.descricao.trim()).reduce((s, p) => s + (Number(p.valor) || 0), 0);
                              if (total > 0) setServicoValorBoleto(String(total));
                            }
                          }}
                        />
          ...
          {semPararReport.porData.length === 0 ? (
            ...
            <>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
                <div style={{ flex: "1 1 150px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>🛣️</div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#B0402E" }}>{BRL(semPararReport.totals.pedagio)}</div>
                    <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>PEDÁGIO</div>
                  </div>
                </div>
                <div style={{ flex: "1 1 150px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>💳</div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#1F6F5C" }}>{BRL(semPararReport.totals.credito)}</div>
                    <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>CRÉDITO</div>
                  </div>
                </div>
                <div style={{ flex: "1 1 150px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>➕</div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "#8A5A00" }}>{BRL(semPararReport.totals.outros)}</div>
                    <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>OUTRAS ARRECADAÇÕES</div>
                  </div>
                </div>
                <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
                  <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>TOTAL GERAL</div>
                  <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(semPararReport.totals.total)}</div>
                </div>
              </div>
      ...
      {view === "financeiropessoal" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 16 }}>
            Área separada, só sua — não entra em nenhum dashboard ou relatório da empresa. Contas a pagar e a receber suas, do Lucas ou das crianças.
          </div>

          <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
            <Field label="De">
              <input type="date" style={inputStyle} value={financeiroPeriodStart} onChange={(e) => setFinanceiroPeriodStart(e.target.value)} onBlur={(e) => setFinanceiroPeriodStart(corrigirAnoDigitado(e.target.value))} />
            </Field>
            <Field label="Até">
              <input type="date" style={inputStyle} value={financeiroPeriodEnd} onChange={(e) => setFinanceiroPeriodEnd(e.target.value)} onBlur={(e) => setFinanceiroPeriodEnd(corrigirAnoDigitado(e.target.value))} />
            </Field>
            <Field label="Tipo">
              <select style={inputStyle} value={financeiroFiltroTipo} onChange={(e) => setFinanceiroFiltroTipo(e.target.value)}>
                <option value="all">Todos</option>
            ...
            <Field label="Status">
              <select style={inputStyle} value={financeiroFiltroStatus} onChange={(e) => setFinanceiroFiltroStatus(e.target.value)}>
                <option value="all">Todos</option>
                <option value="pendente">Pendente</option>
            ...
            <Field label="De quem é">
              <select style={inputStyle} value={financeiroFiltroTitular} onChange={(e) => setFinanceiroFiltroTitular(e.target.value)}>
                <option value="all">Todos</option>
                {TITULARES_FINANCEIRO.map((t) => <option key={t} value={t}>{t}</option>)}
                <option value="Não especificado">Não especificado</option>
              </select>
            </Field>
            <button onClick={() => startAddFinanceiro("pagar")} style={{ background: "#B0402E", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              + A pagar
            </button>
            <button onClick={() => startAddFinanceiro("receber")} style={{ background: "#1F6F5C", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              + A receber
            </button>
            <button onClick={() => window.print()} style={{ background: "#6B4423", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              Baixar PDF
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
            <div style={{ flex: "1 1 190px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>📤</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#B0402E" }}>{BRL(financeiroReport.totais.pendentePagar)}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>A PAGAR (PENDENTE)</div>
                {financeiroReport.totais.vencidoPagar > 0 && <div style={{ fontSize: 10, color: "#B0402E", fontWeight: 700 }}>⚠️ {BRL(financeiroReport.totais.vencidoPagar)} vencido</div>}
              </div>
            </div>
            <div style={{ flex: "1 1 190px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>📥</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#1F6F5C" }}>{BRL(financeiroReport.totais.pendenteReceber)}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>A RECEBER (PENDENTE)</div>
                {financeiroReport.totais.vencidoReceber > 0 && <div style={{ fontSize: 10, color: "#8A5A00", fontWeight: 700 }}>⚠️ {BRL(financeiroReport.totais.vencidoReceber)} vencido</div>}
              </div>
            </div>
            <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>✅</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(financeiroReport.totais.pagoPagar)}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>JÁ PAGO</div>
              </div>
            </div>
            <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
              <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>JÁ RECEBIDO</div>
              <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(financeiroReport.totais.pagoReceber)}</div>
            </div>
          </div>

          {financeiroReport.itens.length === 0 ? (
            <div style={{ background: "#fff", padding: "30px 10px", textAlign: "center", color: "#5A6472", border: "1px dashed #D7DBE0", borderRadius: 12 }}>
              Nenhum lançamento nesse filtro.
            </div>
          ) : (
            <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden", border: "1px solid #E4E7EB" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#1B2E6B" }}>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Tipo</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Descrição</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Pessoa/empresa</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>De quem é</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Vencimento</th>
                    <th style={{ textAlign: "center", padding: "9px 10px", color: "#fff" }}>Status</th>
                    <th style={{ textAlign: "right", padding: "9px 10px", color: "#fff" }}>Valor</th>
                    <th style={{ color: "#fff" }}></th>
                  </tr>
                </thead>
                <tbody>
                  {financeiroReport.itens.map((f, i) => {
                    const pago = !!f.dataPagamento;
                    const vencido = !pago && (f.dataVencimento || "") < todayISO();
                    const cor = f.tipo === "pagar" ? "#B0402E" : "#1F6F5C";
                    return (
                      <tr key={f.id} style={{ borderTop: "1px solid #EEF0F2", background: vencido ? "#FFF6E2" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        <td style={{ padding: "8px 10px" }}>
                          <span style={{ background: f.tipo === "pagar" ? "#FBEBE8" : "#E9F5F1", color: cor, fontWeight: 700, fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>
                            {f.tipo === "pagar" ? "A PAGAR" : "A RECEBER"}
                          </span>
                        </td>
                        <td style={{ padding: "8px 10px" }}>
                          {f.descricao}
                          {f.observacao && <div style={{ fontSize: 11, color: "#9AA5B1" }}>{f.observacao}</div>}
                          {(f.ondeComprou || f.notaFiscalNumero) && (
                            <div style={{ fontSize: 11, color: "#9AA5B1" }}>
                              {f.ondeComprou && `Comprado em ${f.ondeComprou}`}
                              {f.notaFiscalNumero && ` · NF ${f.notaFiscalNumero}${f.notaFiscalData ? ` (${fmtDate(f.notaFiscalData)})` : ""}`}
                            </div>
                          )}
                        </td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{f.pessoa || "—"}</td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{f.titular || "—"}</td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{fmtDate(f.dataVencimento)}</td>
                        <td style={{ padding: "8px 10px", textAlign: "center" }}>
                          <span style={{ background: pago ? "#EAF2FB" : vencido ? "#FBEBE8" : "#FFF6E2", color: pago ? "#1B4E8C" : vencido ? "#B0402E" : "#8A5A00", fontWeight: 700, fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>
                            {pago ? (f.tipo === "pagar" ? "PAGO" : "RECEBIDO") : vencido ? "VENCIDO" : "PENDENTE"}
                          </span>
                        </td>
                        <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700 }}>{BRL(Number(f.valor) || 0)}</td>
                        <td style={{ padding: "8px 10px", textAlign: "right", whiteSpace: "nowrap" }}>
                          {!pago && (
                            <button
                              onClick={() => updateFinanceiroPessoal(f.id, { ...f, dataPagamento: todayISO() })}
                              style={{ background: "none", border: "none", color: "#1F6F5C", cursor: "pointer", fontSize: 12, fontWeight: 700, marginRight: 8 }}
                            >
                              {f.tipo === "pagar" ? "marcar pago" : "marcar recebido"}
                            </button>
                          )}
                          <button onClick={() => startEditFinanceiro(f)} style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 12, marginRight: 8 }}>editar</button>
                          <button onClick={() => deleteFinanceiroPessoal(f.id)} style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 14 }}>×</button>
                        </td>
                      </tr>
                    );
                  })}
          ...
          {addingFinanceiro && (
            <div
              onClick={() => setAddingFinanceiro(false)}
              style={{ position: "fixed", inset: 0, background: "rgba(27,36,48,0.45)", zIndex: 30, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}
            >
              <div
                onClick={(e) => e.stopPropagation()}
                style={{ width: "min(560px, 94vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 20 }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingFinanceiroId ? "Editar lançamento" : "Novo lançamento"}</div>
                  <button onClick={() => setAddingFinanceiro(false)} style={{ background: "none", border: "none", fontSize: 26, cursor: "pointer", color: "#5A6472", padding: "10px", margin: "-10px", lineHeight: 1 }}>×</button>
                </div>
                <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                  <button
                    onClick={() => setFinanceiroTipo("pagar")}
                    style={{ flex: 1, padding: "10px", borderRadius: 10, border: financeiroTipo === "pagar" ? "2px solid #B0402E" : "1px solid #D7DBE0", background: financeiroTipo === "pagar" ? "#FBEBE8" : "#fff", color: "#B0402E", fontWeight: 700, cursor: "pointer" }}
                  >
                    A pagar
                  </button>
                  <button
                    onClick={() => setFinanceiroTipo("receber")}
                    style={{ flex: 1, padding: "10px", borderRadius: 10, border: financeiroTipo === "receber" ? "2px solid #1F6F5C" : "1px solid #D7DBE0", background: financeiroTipo === "receber" ? "#E9F5F1" : "#fff", color: "#1F6F5C", fontWeight: 700, cursor: "pointer" }}
                  >
                    A receber
                  </button>
                </div>
                <Field label="Descrição">
                  <input style={{ ...inputStyle, width: "100%" }} value={financeiroDescricao} onChange={(e) => setFinanceiroDescricao(e.target.value)} placeholder="ex: Fatura do cartão, Empréstimo pro fulano..." autoFocus />
                </Field>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                  <Field label="Pessoa/empresa (opcional)">
                    <input style={inputStyle} value={financeiroPessoaNome} onChange={(e) => setFinanceiroPessoaNome(e.target.value)} />
                  </Field>
                  <Field label="Valor (R$)">
                    <input type="number" style={{ ...inputStyle, width: 110 }} value={financeiroValor} onChange={(e) => setFinanceiroValor(e.target.value)} />
                  </Field>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                  <Field label="Vencimento">
                    <input type="date" style={inputStyle} value={financeiroVencimento} onChange={(e) => setFinanceiroVencimento(e.target.value)} onBlur={(e) => setFinanceiroVencimento(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label={financeiroTipo === "pagar" ? "Pago em (opcional)" : "Recebido em (opcional)"}>
                    <input type="date" style={inputStyle} value={financeiroDataPagamento} onChange={(e) => setFinanceiroDataPagamento(e.target.value)} onBlur={(e) => setFinanceiroDataPagamento(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label="De quem é">
                    <select style={inputStyle} value={financeiroTitular} onChange={(e) => setFinanceiroTitular(e.target.value)}>
                ...
                {!editingFinanceiroId && (
                  <div style={{ marginTop: 10, background: "#F7F8F9", borderRadius: 10, padding: 10 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600 }}>
                      <input type="checkbox" checked={financeiroParcelar} onChange={(e) => setFinanceiroParcelar(e.target.checked)} />
                      Parcelar esse valor
                    </label>
                    {financeiroParcelar && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                        <Field label="Nº de parcelas">
                          <input type="number" min="2" style={{ ...inputStyle, width: 90 }} value={financeiroNumParcelas} onChange={(e) => setFinanceiroNumParcelas(e.target.value)} />
                        </Field>
                        <Field label="Dias entre parcelas">
                          <input type="number" style={{ ...inputStyle, width: 110 }} value={financeiroIntervaloDias} onChange={(e) => setFinanceiroIntervaloDias(e.target.value)} />
                        </Field>
                        <div style={{ fontSize: 11, color: "#5A6472", alignSelf: "center" }}>
                          {Number(financeiroNumParcelas) > 1 && Number(financeiroValor) > 0 &&
                            `${financeiroNumParcelas}x de ${BRL((Number(financeiroValor) || 0) / Number(financeiroNumParcelas))}, a partir do vencimento acima`}
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                  <Field label={<>Onde comprou <span title="Opcional — a loja/empresa onde o item foi comprado, pra referência" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <input style={inputStyle} value={financeiroOndeComprou} onChange={(e) => setFinanceiroOndeComprou(e.target.value)} placeholder="ex: União" />
                  </Field>
                  <Field label="Nº da nota fiscal (opcional)">
                    <input style={{ ...inputStyle, width: 130 }} value={financeiroNotaNumero} onChange={(e) => setFinanceiroNotaNumero(e.target.value)} />
                  </Field>
                  <Field label="Data da nota (opcional)">
                    <input type="date" style={inputStyle} value={financeiroNotaData} onChange={(e) => setFinanceiroNotaData(e.target.value)} onBlur={(e) => setFinanceiroNotaData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                </div>
                <div style={{ marginTop: 10 }}>
                  <Field label="Observação (opcional)">
                    <input style={{ ...inputStyle, width: "100%" }} value={financeiroObs} onChange={(e) => setFinanceiroObs(e.target.value)} />
                  </Field>
                </div>
                <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                  <button onClick={confirmAddFinanceiro} style={{ flex: 1, background: "#2451A6", color: "#fff", border: "none", borderRadius: 10, padding: "10px", fontWeight: 700, cursor: "pointer" }}>
                    Salvar
                  </button>
                  <button onClick={() => setAddingFinanceiro(false)} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 10, padding: "10px 16px", cursor: "pointer" }}>
                    Cancelar
                  </button>
                </div>
              </div>
      ...
      {view === "multas" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
            <Field label="De">
              <input type="date" style={inputStyle} value={multaPeriodStart} onChange={(e) => setMultaPeriodStart(e.target.value)} onBlur={(e) => setMultaPeriodStart(corrigirAnoDigitado(e.target.value))} />
            </Field>
            <Field label="Até">
              <input type="date" style={inputStyle} value={multaPeriodEnd} onChange={(e) => setMultaPeriodEnd(e.target.value)} onBlur={(e) => setMultaPeriodEnd(corrigirAnoDigitado(e.target.value))} />
            </Field>
            <Field label="Caminhão">
              <select style={inputStyle} value={multaFiltroCaminhao} onChange={(e) => setMultaFiltroCaminhao(e.target.value)}>
                <option value="all">Todos</option>
                {trucksCavalos.map((tr) => <option key={tr.id} value={tr.id}>{tr.placa}</option>)}
              </select>
            </Field>
            <Field label="Status">
              <select style={inputStyle} value={multaFiltroStatus} onChange={(e) => setMultaFiltroStatus(e.target.value)}>
                <option value="all">Todos</option>
                <option value="pendente">Pendente</option>
                <option value="pago">Pago</option>
              </select>
            </Field>
            <button onClick={startAddMulta} style={{ background: "#2451A6", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              + Lançar multa
            </button>
            <button onClick={() => window.print()} style={{ background: "#6B4423", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              Baixar PDF
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
            <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>📄</div>
              <div>
                <div style={{ fontSize: 17, fontWeight: 700 }}>{multaReport.itens.length}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>MULTAS</div>
              </div>
            </div>
            <div style={{ flex: "1 1 200px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>⏳</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#8A5A00" }}>{BRL(multaReport.totalPendente)}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>PENDENTE</div>
                {multaReport.totalVencido > 0 && <div style={{ fontSize: 10, color: "#B0402E", fontWeight: 700 }}>⚠️ {BRL(multaReport.totalVencido)} vencido</div>}
              </div>
            </div>
            <div style={{ flex: "1 1 160px", border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>✅</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#1F6F5C" }}>{BRL(multaReport.totalPago)}</div>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700 }}>JÁ PAGO</div>
              </div>
            </div>
            <div style={{ flex: "1 1 170px", background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 14px" }}>
              <div style={{ fontSize: 9, opacity: 0.85, fontWeight: 700 }}>TOTAL EM ABERTO</div>
              <div style={{ fontSize: 19, fontWeight: 700 }}>{BRL(multaReport.totalPendente)}</div>
            </div>
          </div>

          {multaReport.itens.length === 0 ? (
            <div style={{ background: "#fff", padding: "30px 10px", textAlign: "center", color: "#5A6472", border: "1px dashed #D7DBE0", borderRadius: 12 }}>
              Nenhuma multa nesse filtro.
            </div>
          ) : (
            <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden", border: "1px solid #E4E7EB" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#1B2E6B" }}>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Data</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Placa</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Motorista</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Descrição</th>
                    <th style={{ textAlign: "left", padding: "9px 10px", color: "#fff" }}>Vencimento</th>
                    <th style={{ textAlign: "center", padding: "9px 10px", color: "#fff" }}>Status</th>
                    <th style={{ textAlign: "right", padding: "9px 10px", color: "#fff" }}>Valor</th>
                ...
                <tbody>
                  {multaReport.itens.map((m, i) => {
                    const pago = !!m.dataPagamento;
                    const vencida = !pago && m.dataVencimento && m.dataVencimento < todayISO();
                    return (
                      <tr key={m.id} style={{ borderTop: "1px solid #EEF0F2", background: vencida ? "#FFF6E2" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{fmtDate(m.data)}</td>
                        <td style={{ padding: "8px 10px", fontWeight: 700 }}>{truckLabel(m.caminhaoId)}</td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{m.motorista || "—"}</td>
                        <td style={{ padding: "8px 10px" }}>
                          {m.descricao}
                          {m.descontadoMotorista && <div style={{ fontSize: 10, color: "#8A5A00" }}>descontado do motorista</div>}
                        </td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{m.dataVencimento ? fmtDate(m.dataVencimento) : "—"}</td>
                        <td style={{ padding: "8px 10px", textAlign: "center" }}>
                          <span style={{ background: pago ? "#E9F5F1" : vencida ? "#FBEBE8" : "#FFF6E2", color: pago ? "#1F6F5C" : vencida ? "#B0402E" : "#8A5A00", fontWeight: 700, fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>
                            {pago ? "PAGO" : vencida ? "VENCIDA" : "PENDENTE"}
                          </span>
                        </td>
                        <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700 }}>{BRL(Number(m.valor) || 0)}</td>
                        <td style={{ padding: "8px 10px", textAlign: "right", whiteSpace: "nowrap" }}>
                          {!pago && (
                            <button onClick={() => updateMulta(m.id, { ...m, dataPagamento: todayISO() })} style={{ background: "none", border: "none", color: "#1F6F5C", cursor: "pointer", fontSize: 12, fontWeight: 700, marginRight: 8 }}>
                              marcar pago
                            </button>
                          )}
                          <button onClick={() => startEditMulta(m)} style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 12, marginRight: 8 }}>editar</button>
                          <button onClick={() => deleteMulta(m.id)} style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 14 }}>×</button>
                        </td>
                      </tr>
                    );
                  })}
          ...
          {addingMulta && (
            <div onClick={() => setAddingMulta(false)} style={{ position: "fixed", inset: 0, background: "rgba(27,36,48,0.45)", zIndex: 30, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}>
              <div onClick={(e) => e.stopPropagation()} style={{ width: "min(560px, 94vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingMultaId ? "Editar multa" : "Lançar multa"}</div>
                  <button onClick={() => setAddingMulta(false)} style={{ background: "none", border: "none", fontSize: 26, cursor: "pointer", color: "#5A6472", padding: "10px", margin: "-10px", lineHeight: 1 }}>×</button>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <Field label="Caminhão">
                    <select style={inputStyle} value={multaCaminhaoId} onChange={(e) => setMultaCaminhaoId(e.target.value)}>
                      {trucksCavalos.map((tr) => <option key={tr.id} value={tr.id}>{tr.placa}</option>)}
                    </select>
                  </Field>
                  <Field label="Motorista (opcional)">
                    <input style={inputStyle} value={multaMotorista} onChange={(e) => setMultaMotorista(e.target.value)} />
                  </Field>
                  <Field label="Data da infração">
                    <input type="date" style={inputStyle} value={multaData} onChange={(e) => setMultaData(e.target.value)} onBlur={(e) => setMultaData(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                </div>
                <div style={{ marginTop: 10 }}>
                  <Field label="Descrição">
                    <input style={{ ...inputStyle, width: "100%" }} value={multaDescricao} onChange={(e) => setMultaDescricao(e.target.value)} placeholder="ex: Excesso de velocidade" autoFocus />
                  </Field>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                  <Field label="Valor (R$)">
                    <input type="number" style={{ ...inputStyle, width: 110 }} value={multaValor} onChange={(e) => setMultaValor(e.target.value)} />
                  </Field>
                  <Field label="Vencimento (opcional)">
                    <input type="date" style={inputStyle} value={multaVencimento} onChange={(e) => setMultaVencimento(e.target.value)} onBlur={(e) => setMultaVencimento(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                  <Field label="Pago em (opcional)">
                    <input type="date" style={inputStyle} value={multaPagamento} onChange={(e) => setMultaPagamento(e.target.value)} onBlur={(e) => setMultaPagamento(corrigirAnoDigitado(e.target.value))} />
                  </Field>
                </div>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, marginTop: 10 }}>
                  <input type="checkbox" checked={multaDescontado} onChange={(e) => setMultaDescontado(e.target.checked)} />
                  Vai ser descontado do motorista
                </label>
                <div style={{ marginTop: 10 }}>
                  <Field label="Observação (opcional)">
                    <input style={{ ...inputStyle, width: "100%" }} value={multaObs} onChange={(e) => setMultaObs(e.target.value)} />
                  </Field>
                </div>
                <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                  <button onClick={confirmAddMulta} style={{ flex: 1, background: "#2451A6", color: "#fff", border: "none", borderRadius: 10, padding: "10px", fontWeight: 700, cursor: "pointer" }}>Salvar</button>
                  <button onClick={() => setAddingMulta(false)} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 10, padding: "10px 16px", cursor: "pointer" }}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
      ...
      {configOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ width: configSecaoAberta === null ? "min(920px, 96vw)" : "min(560px, 94vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 22 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: configSecaoAberta === null ? 4 : 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {configSecaoAberta !== null && (
                  <button onClick={() => setConfigSecaoAberta(null)} style={{ background: "#F7F8F9", border: "none", borderRadius: 8, width: 32, height: 32, cursor: "pointer", fontSize: 16, color: "#5A6472" }}>←</button>
                )}
            ...
            {configSecaoAberta === null && (
              <>
                <div style={{ fontSize: 13, color: "#5A6472", marginBottom: 18 }}>Gerencie as configurações do sistema de acordo com a sua operação.</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14 }}>
                  {[
                    { chave: "geral", emoji: "📊", cor: "#1F6F5C", bg: "#E9F5F1", titulo: "Comissão e óleo", desc: "Configure as regras de cálculo de comissão e óleo dos motoristas." },
                    { chave: "metaFaturamento", emoji: "🎯", cor: "#B0402E", bg: "#FBEBE8", titulo: "Meta de faturamento", desc: "Defina e acompanhe a meta mensal de faturamento." },
                    { chave: "precosPostos", emoji: "⛽", cor: "#D9A419", bg: "#FFF6E2", titulo: "Preços combinados por posto", desc: "Cadastre e gerencie os preços combinados por posto." },
                    { chave: "empresasPedagio", emoji: "🏢", cor: "#7A3FBF", bg: "#F1EAFB", titulo: "Empresas com pedágio obrigatório", desc: "Gerencie as empresas que possuem pedágio obrigatório." },
                    { chave: "rotasPedagio", emoji: "🛣️", cor: "#2451A6", bg: "#EAF2FB", titulo: "Rotas — valor do pedágio", desc: "Cadastre o valor do pedágio de cada rota, por eixos." },
                    { chave: "seguradoras", emoji: "🛡️", cor: "#1B4E8C", bg: "#EAF2FB", titulo: "Vencimento por seguradora", desc: "Acompanhe os vencimentos de seguros por seguradora." },
                    { chave: "caminhoes", emoji: "🚚", cor: "#1F6F5C", bg: "#E9F5F1", titulo: "Caminhões", desc: "Cadastre e gerencie os caminhões da frota com fotos, modelos e documentação." },
                    { chave: "carretas", emoji: "🚛", cor: "#7A3FBF", bg: "#F1EAFB", titulo: "Carretas", desc: "Cadastre e gerencie as carretas da frota com fotos, modelos e documentação." },
                    { chave: "backups", emoji: "☁️", cor: "#7A3FBF", bg: "#F1EAFB", titulo: "Backups automáticos", desc: "Veja e baixe os backups automáticos do sistema." },
                  ].map((c) => (
                    <button
                      key={c.chave}
                      onClick={() => {
                        setConfigSecaoAberta(c.chave);
                        if (c.chave === "backups" && backupsAutomaticos === null) carregarListaBackupsAutomaticos();
                      }}
                      style={{ textAlign: "left", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 14, padding: 18, cursor: "pointer", display: "flex", flexDirection: "column", gap: 10 }}
                    >
                      <div style={{ width: 52, height: 52, borderRadius: "50%", background: c.bg, color: c.cor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>{c.emoji}</div>
                      <div style={{ fontWeight: 700, fontSize: 15 }}>{c.titulo}</div>
                      <div style={{ fontSize: 12, color: "#5A6472", lineHeight: 1.4 }}>{c.desc}</div>
                      <div style={{ color: c.cor, fontSize: 18, fontWeight: 700 }}>→</div>
                    </button>
                  ))}
                </div>
              </>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>📊 Geral (comissão e óleo)</div>
            ...
            {configSecaoAberta === "backups" && (
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🛟 Backups automáticos</div>
            )}
            {configSecaoAberta === "backups" && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 10 }}>
                  O app salva uma cópia completa dos dados sozinho — pelo menos uma vez por dia, e também depois de qualquer alteração (a cada 10 minutos no máximo, pra não sobrecarregar). Ficam guardados os últimos 30 backups. Clica em qualquer um pra baixar aquela cópia como arquivo.
                </div>
                {backupsAutomaticos === null ? (
                  <div style={{ fontSize: 12, color: "#9AA5B1" }}>Carregando lista de backups...</div>
                ) : backupsAutomaticos.length === 0 ? (
                  <div style={{ fontSize: 12, color: "#9AA5B1" }}>Nenhum backup automático ainda — o primeiro é criado logo que você usar o sistema um pouco.</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 220, overflowY: "auto" }}>
                    {backupsAutomaticos.map((b) => (
                      <button
                        key={b.criadoEm}
                        onClick={() => baixarBackupAutomatico(b.criadoEm)}
                        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#F7F8F9", border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 12, cursor: "pointer", textAlign: "left" }}
                      >
                        <span>{fmtDate(b.data)} às {new Date(b.criadoEm).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span>
                        <span style={{ color: "#2451A6", fontWeight: 700 }}>⬇️ baixar</span>
                      </button>
                    ))}
                  </div>
                )}
                <button
                  onClick={carregarListaBackupsAutomaticos}
                  style={{ marginTop: 8, background: "none", border: "1px dashed #B7BFC8", color: "#5A6472", borderRadius: 10, padding: "6px 12px", fontSize: 12, cursor: "pointer" }}
                >
                  🔄 Atualizar lista
                </button>
              </div>
            ...
            {/* seção: meta de faturamento mensal */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🎯 Meta de faturamento mensal</div>
            {configSecaoAberta === "metaFaturamento" && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 10 }}>
                  Define um valor de referência de receita bruta. Aparece como uma barrinha de progresso no Dashboard Geral, comparando com o que já foi faturado no período selecionado ali.
                </div>
                <Field label="Meta (R$)">
                  <input
                    type="number"
                    ...
                    onBlur={(e) => salvarMetaFaturamento(e.target.value)}
            ...
            {/* seção: rotas com valor de pedágio combinado */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🛣️ Rotas — valor do pedágio</div>
            {configSecaoAberta === "rotasPedagio" && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 10 }}>
                  Cadastra o valor do pedágio de cada rota (origem → destino), separado por número de eixos do caminhão. Ao lançar uma viagem com essa mesma origem/destino, o campo "Pedágio" já vem preenchido sozinho de acordo com os eixos do caminhão selecionado.
                </div>
                <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                  <input style={{ ...inputStyle, flex: "1 1 140px" }} placeholder="Origem" value={novaRotaOrigem} onChange={(e) => setNovaRotaOrigem(e.target.value)} />
                  <input style={{ ...inputStyle, flex: "1 1 140px" }} placeholder="Destino" value={novaRotaDestino} onChange={(e) => setNovaRotaDestino(e.target.value)} />
                  <input type="number" style={{ ...inputStyle, width: 130 }} placeholder="Pedágio 5 eixos" value={novaRotaValor5} onChange={(e) => setNovaRotaValor5(e.target.value)} />
                  <input type="number" style={{ ...inputStyle, width: 130 }} placeholder="Pedágio 6+ eixos" value={novaRotaValor6} onChange={(e) => setNovaRotaValor6(e.target.value)} />
                  <button
                    onClick={() => {
                      if (novaRotaOrigem.trim() && novaRotaDestino.trim() && (Number(novaRotaValor5) > 0 || Number(novaRotaValor6) > 0)) {
                        salvarRotasPedagio([...rotasPedagio, { id: uid(), origem: novaRotaOrigem.trim(), destino: novaRotaDestino.trim(), valor5: novaRotaValor5, valor6: novaRotaValor6 }]);
                        setNovaRotaOrigem("");
                        setNovaRotaDestino("");
                        setNovaRotaValor5("");
                        setNovaRotaValor6("");
                ...
                {rotasPedagio.length === 0 ? (
                  <div style={{ fontSize: 12, color: "#9AA5B1" }}>Nenhuma rota cadastrada ainda.</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {rotasPedagio.map((r) => (
                      <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#FFF6E2", borderRadius: 10, padding: "8px 12px", flexWrap: "wrap", gap: 6 }}>
                        <span style={{ fontSize: 13, fontWeight: 600 }}>🛣️ {r.origem} → {r.destino}</span>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ fontSize: 12, color: "#5A6472" }}>5 eixos: <strong style={{ color: "#1B2430" }}>{BRL(Number(r.valor5) || 0)}</strong></span>
                          <span style={{ fontSize: 12, color: "#5A6472" }}>6+ eixos: <strong style={{ color: "#1B2430" }}>{BRL(Number(r.valor6) || 0)}</strong></span>
                          <button
                            onClick={() => salvarRotasPedagio(rotasPedagio.filter((x) => x.id !== r.id))}
                            style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 16 }}
                          >
                            ×
                          </button>
            ...
            {/* seção: empresas com pedágio obrigatório */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🛣️ Empresas com pedágio obrigatório</div>
            {configSecaoAberta === "empresasPedagio" && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 10 }}>
                  Marca aqui as empresas cujas viagens sempre têm pedágio. Ao lançar uma viagem pra uma dessas empresas, o sistema não deixa salvar sem o valor do pedágio preenchido — pra não esquecer de descontar do motorista.
                </div>
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <input
                    style={{ ...inputStyle, flex: 1 }}
                    placeholder="Nome da empresa (ex: Nome da transportadora)"
                    value={novaEmpresaComPedagio}
                    onChange={(e) => setNovaEmpresaComPedagio(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && novaEmpresaComPedagio.trim()) {
                        salvarEmpresasComPedagio([...empresasComPedagio, novaEmpresaComPedagio.trim()]);
                        setNovaEmpresaComPedagio("");
                      }
                    }}
                  />
                  <button
                    onClick={() => {
                      if (novaEmpresaComPedagio.trim()) {
                        salvarEmpresasComPedagio([...empresasComPedagio, novaEmpresaComPedagio.trim()]);
                        setNovaEmpresaComPedagio("");
                      }
                    }}
                ...
                {empresasComPedagio.length === 0 ? (
                  <div style={{ fontSize: 12, color: "#9AA5B1" }}>Nenhuma empresa marcada ainda.</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {empresasComPedagio.map((nome, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#FFF6E2", borderRadius: 10, padding: "8px 12px" }}>
                        <span style={{ fontSize: 13, fontWeight: 600 }}>🛣️ {nome}</span>
                        <button
                          onClick={() => salvarEmpresasComPedagio(empresasComPedagio.filter((_, idx) => idx !== i))}
                          style={{ background: "none", border: "none", color: "#B0402E", cursor: "pointer", fontSize: 16 }}
                        >
                          ×
                        </button>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>⛽ Preços combinados por posto</div>
            {configSecaoAberta === "precosPostos" && (
              <div style={{ padding: "0 4px", marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                  {precosPostos.map((p) => (
                    <div key={p.id} style={{ background: "#F7F8F9", borderRadius: 10, padding: 8 }}>
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      <div style={{ flex: "1 1 140px", fontWeight: 600, fontSize: 13 }}>{p.posto}</div>
                      <Field label="Diesel R$/L">
                        <input
                          type="number" step="0.01" style={{ ...inputStyle, width: 90 }} value={p.precoDiesel}
                          onFocus={() => { precoFocoRef.current[`${p.id}-precoDiesel`] = p.precoDiesel; }}
                          onChange={(e) => updatePrecoPosto(p.id, "precoDiesel", e.target.value)}
                          onBlur={(e) => registrarHistoricoPreco(p.id, "precoDiesel", precoFocoRef.current[`${p.id}-precoDiesel`], e.target.value)}
                        />
                      </Field>
                      <Field label="Arla R$/L">
                        <input
                          type="number" step="0.01" style={{ ...inputStyle, width: 90 }} value={p.precoArla}
                          onFocus={() => { precoFocoRef.current[`${p.id}-precoArla`] = p.precoArla; }}
                          onChange={(e) => updatePrecoPosto(p.id, "precoArla", e.target.value)}
                          onBlur={(e) => registrarHistoricoPreco(p.id, "precoArla", precoFocoRef.current[`${p.id}-precoArla`], e.target.value)}
                        />
                      </Field>
                      ...
                      {(p.historico || []).length > 0 && (
                        <div style={{ marginTop: 4 }}>
                          <button
                            onClick={() => setPostosComHistoricoAberto((prev) => ({ ...prev, [p.id]: !prev[p.id] }))}
                            style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 11, textDecoration: "underline", padding: 0 }}
                          >
                            {postosComHistoricoAberto[p.id] ? "esconder" : "ver"} histórico de preços ({p.historico.length})
                          </button>
                          {postosComHistoricoAberto[p.id] && (
                            <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 3 }}>
                              {p.historico.map((h) => (
                                <div key={h.id} style={{ fontSize: 11, color: "#5A6472" }}>
                                  {fmtDate(h.data)} — {h.campo}: {h.valorAntigo ? `R$ ${h.valorAntigo}` : "(vazio)"} → <strong>{h.valorNovo ? `R$ ${h.valorNovo}` : "(vazio)"}</strong>
                                </div>
                              ))}
                            </div>
                          )}
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <CampoComSugestoes
                    opcoes={abastecPostosList}
                    value={novoPostoPrecoNome}
                    onChange={setNovoPostoPrecoNome}
            ...
            {/* seção: seguradoras */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🛡️ Vencimento por seguradora</div>
            ...
            {/* seção: caminhões */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🚚 Caminhões (foto, modelo e documentação)</div>
                    ...
                    <div style={{ padding: "0 10px 12px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <div style={{ flex: "0 0 90px" }}>
                          <Field label="Eixos">
                            <select value={tr.eixos || "5"} onChange={(e) => updateTruckInfo(tr.id, { eixos: e.target.value })} style={{ ...inputStyle, fontSize: 12, padding: "6px 8px" }}>
            ...
            {/* seção: carretas */}
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>🚛 Carretas (documentação)</div>
        ...
        <>
          ...
          >
            ...
            <Section title="Viagem" icon="🚚">
              <Field label="Caminhão">
                <select
                  ...
                  onChange={(e) => {
                    const novoCaminhaoId = e.target.value;
                    const rota = rotasPedagio.find((r) => r.origem.trim().toLowerCase() === (editing.origem || "").trim().toLowerCase() && r.destino.trim().toLowerCase() === (editing.destino || "").trim().toLowerCase());
                    if (rota && !(Number(editing.pedagio) > 0)) {
                      const caminhaoSelecionado = trucks.find((tr) => tr.id === novoCaminhaoId);
                      const ehSeisMais = Number(caminhaoSelecionado?.eixos) >= 6;
                      const valorRota = ehSeisMais ? rota.valor6 : rota.valor5;
                      setEditing({ ...editing, caminhaoId: novoCaminhaoId, pedagio: Number(valorRota) > 0 ? valorRota : editing.pedagio });
                    } else {
                      setEditing({ ...editing, caminhaoId: novoCaminhaoId });
                    }
                  }}
                  style={inputStyle}
                >
              ...
              <Field label="Origem">
                <input
                  ...
                  onChange={(e) => setEditing({ ...editing, origem: e.target.value })}
                  onBlur={(e) => {
                    const rota = rotasPedagio.find((r) => r.origem.trim().toLowerCase() === e.target.value.trim().toLowerCase() && r.destino.trim().toLowerCase() === (editing.destino || "").trim().toLowerCase());
                    if (rota && !(Number(editing.pedagio) > 0)) {
                      const caminhaoSelecionado = trucks.find((tr) => tr.id === editing.caminhaoId);
                      const ehSeisMais = Number(caminhaoSelecionado?.eixos) >= 6;
                      const valorRota = ehSeisMais ? rota.valor6 : rota.valor5;
                      if (Number(valorRota) > 0) setEditing((prev) => ({ ...prev, pedagio: valorRota }));
                    }
                  }}
                />
              </Field>
              <Field label="Destino">
                <input
                  ...
                  onChange={(e) => setEditing({ ...editing, destino: e.target.value })}
                  onBlur={(e) => {
                    const rota = rotasPedagio.find((r) => r.destino.trim().toLowerCase() === e.target.value.trim().toLowerCase() && r.origem.trim().toLowerCase() === (editing.origem || "").trim().toLowerCase());
                    if (rota && !(Number(editing.pedagio) > 0)) {
                      const caminhaoSelecionado = trucks.find((tr) => tr.id === editing.caminhaoId);
                      const ehSeisMais = Number(caminhaoSelecionado?.eixos) >= 6;
                      const valorRota = ehSeisMais ? rota.valor6 : rota.valor5;
            ...
            <Section title="Financeiro" icon="💰">
              ...
              <Field label={<strong>Adiantamento (R$)</strong>}>
                <input
                  type="number"
                  style={{
                    ...inputStyle,
                    ...(editing.dataRecebAdiantamento
                      ? { background: "#E9F5F1", borderColor: "#1F6F5C", color: "#12503F", fontWeight: 700 }
                      : {}),
                  }}
                  value={editing.adiantamento}
                  onChange={(e) => setEditing({ ...editing, adiantamento: e.target.value })}
                />
              </Field>
              ...
              <Field label={<strong>Saldo a receber (R$)</strong>}>
                <input
                  type="number"
                  style={{
                    ...inputStyle,
                    ...(editing.dataPagamentoSaldo
                      ? { background: "#E9F5F1", borderColor: "#1F6F5C", color: "#12503F", fontWeight: 700 }
                      : {}),
                  }}
                  value={editing.saldoReceber}
                  onChange={(e) => setEditing({ ...editing, saldoReceber: e.target.value })}
                />
              </Field>
            ...
            <RepeatingSection
              ...
              renderItem={(item, update) => (
                <>
                  ...
                  <Field label="Posto">
                    <CampoComSugestoes style={inputStyle} opcoes={abastecPostosList} value={item.posto} onChange={(v) => update({ ...item, posto: v })} />
                  </Field>
                  ...
                  {(() => {
                    ...
                    return (
                      <div
                        style={{
                          gridColumn: "1 / -1", borderRadius: 10, padding: "8px 12px", fontSize: 12,
                          background: diferenca > 0 ? "#FBEBE8" : "#E9F5F1",
                          border: `1px solid ${diferenca > 0 ? "#B0402E55" : "#1F6F5C55"}`,
                          color: diferenca > 0 ? "#7A2A1D" : "#12503F",
                        }}
                      >
                        {diferenca > 0 ? "⚠️" : "✅"} Preço lançado no posto <strong>{combinado.posto}</strong>: <strong>{BRL(precoLancado)}/L</strong> — o combinado é{" "}
              ...
              renderItem={(item, update) => (
                <>
                  ...
                  <Field label={<>Categoria <span title="Opcional" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <CampoComSugestoes style={inputStyle} opcoes={categoriasGastoList} value={item.categoria || ""} onChange={(v) => update({ ...item, categoria: v })} placeholder="ex: Despesas Estrada" />
                  </Field>
                  <Field label={<>Setor <span title="Opcional — só se quiser separar um tipo específico, tipo Borracharia ou Lavador" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <CampoComSugestoes style={inputStyle} opcoes={setoresGastoList} value={item.setor || ""} onChange={(v) => update({ ...item, setor: v })} placeholder="ex: Borracharia Miradouro" />
                  </Field>
                  <Field label={<>Posto <span title="Preenche se for do mesmo cupom/posto do abastecimento" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <CampoComSugestoes style={inputStyle} opcoes={abastecPostosList} value={item.posto} onChange={(v) => update({ ...item, posto: v })} />
                  </Field>
                  <Field label={<>Onde foi feito <span title="Livre, só pra referência (ex: achar a nota fiscal depois) — não entra no relatório de Serviços por empresa nem gera boleto" style={{ cursor: "help", color: "#9AA5B1" }}>ⓘ</span></>}>
                    <input style={inputStyle} value={item.estabelecimento || ""} onChange={(e) => update({ ...item, estabelecimento: e.target.value })} placeholder="ex: Movesa Motores" />
                  </Field>
                  <Field label="Nº cupom / NFC-e">
    ...
    {reportOpen && reportView === "resumo" && (
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 11 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <img src={LOGO_BASE64} alt="L.L. Campos Transportes" style={{ height: 52 }} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO GERENCIAL</div>
              <div style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.1 }}>RESULTADO MENSAL DA FROTA</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>Referência: {reportMonth} · {filterTruck === "all" ? "Todos os caminhões" : truckLabel(filterTruck)}</div>
        ...
        {(() => {
          ...
          return (
            <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
              {cards.map((c) => (
                <div key={c.label} style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "8px 10px" }}>
                  <div style={{ fontSize: 14 }}>{c.icon}</div>
                  <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700, marginTop: 3 }}>{c.label}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: c.cor }}>{c.value}</div>
                </div>
              ))}
              <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "8px 10px" }}>
                <div style={{ fontSize: 8, opacity: 0.8, fontWeight: 700 }}>RESULTADO LÍQUIDO</div>
                <div style={{ fontSize: 13, fontWeight: 700 }}>{BRL(t.liquido)}</div>
                <div style={{ fontSize: 8, marginTop: 3 }}>MARGEM {margem.toFixed(1)}%</div>
              </div>
            </div>
          );
        })()}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10, marginBottom: 16 }}>
          <thead>
            <tr style={{ background: "#1B2E6B" }}>
              {["#", "Cavalo/Carreta", "Viagens", "Receita", "Comissão", "Carregamento", "Abastec.", "Gastos", "Desp. Veíc.", "Líquido"].map((h, i) => (
                <th key={h} style={{ textAlign: i > 1 ? "right" : "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 10 }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {monthlyReport.rows.map((r, i) => (
              <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#9AA5B1" }}>{i + 1}</td>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>
                  {r.placa}{r.carretaVinculada && <span style={{ color: "#5A6472", fontWeight: 400 }}> / {r.carretaVinculada}</span>}
                </td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2" }}>{r.viagens}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#12503F" }}>{BRL(r.receita)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#B0402E" }}>−{BRL(r.comissaoTotal)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#B0402E" }}>−{BRL(r.carregamentoTotal)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#B0402E" }}>−{BRL(r.abastecimentoTotal)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#B0402E" }}>−{BRL(r.gastosTotal)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", color: "#B0402E" }}>−{BRL(r.despesasVeiculoTotal)}</td>
                <td style={{ padding: "5px 8px", textAlign: "right", fontWeight: 700, borderBottom: "1px solid #EEF0F2", color: r.liquido >= 0 ? "#12503F" : "#B0402E" }}>{BRL(r.liquido)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ background: "#1B2E6B" }}>
              <td colSpan={2} style={{ padding: "7px 8px", fontWeight: 700, color: "#fff" }}>TOTAL DA FROTA</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>{monthlyReport.totals.viagens}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>{BRL(monthlyReport.totals.receita)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>−{BRL(monthlyReport.totals.comissaoTotal)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>−{BRL(monthlyReport.totals.carregamentoTotal)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>−{BRL(monthlyReport.totals.abastecimentoTotal)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>−{BRL(monthlyReport.totals.gastosTotal)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>−{BRL(monthlyReport.totals.despesasVeiculoTotal)}</td>
              <td style={{ padding: "7px 8px", textAlign: "right", fontWeight: 700, color: "#fff" }}>{BRL(monthlyReport.totals.liquido)}</td>
            </tr>
          </tfoot>
        </table>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, breakInside: "avoid" }}>
          <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 8 }}>🚚 RESULTADO LÍQUIDO POR VEÍCULO</div>
            {(() => {
              const maxLiq = Math.max(1, ...monthlyReport.rows.map((r) => r.liquido));
              return monthlyReport.rows.map((r) => (
                <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                  <div style={{ width: 50, fontSize: 9, fontWeight: 700 }}>{r.placa}</div>
                  <div style={{ flex: 1, background: "#EEF0F2", borderRadius: 4, height: 12 }}>
                    <div style={{ width: `${Math.max(2, (r.liquido / maxLiq) * 100)}%`, height: "100%", background: "#1B2E6B", borderRadius: 4 }} />
                  </div>
                  <div style={{ width: 75, fontSize: 9, fontWeight: 700, textAlign: "right", color: r.liquido >= 0 ? "#12503F" : "#B0402E" }}>{BRL(r.liquido)}</div>
                </div>
              ));
            })()}
          </div>
          ...
          <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 8 }}>🍩 DISTRIBUIÇÃO DAS DESPESAS (sobre a receita)</div>
                ...
                return (
                  <>
                    <div style={{ position: "relative", width: 100, height: 100, flexShrink: 0 }}>
                      ...
                      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                        <div style={{ fontWeight: 700, fontSize: 13 }}>{margem.toFixed(1)}%</div>
                        <div style={{ fontSize: 6, color: "#9AA5B1", fontWeight: 700 }}>MARGEM</div>
                      </div>
                    </div>
                    <table style={{ fontSize: 9, borderCollapse: "collapse", flex: 1 }}>
                      <tbody>
                        {cats.map((c) => (
                          <tr key={c.nome}>
                            <td style={{ padding: "2px 6px 2px 0" }}><span style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: c.cor, marginRight: 4 }} />{c.nome}</td>
                            <td style={{ padding: "2px 6px", textAlign: "right" }}>{BRL(c.valor)}</td>
                            <td style={{ padding: "2px 0", textAlign: "right", color: "#9AA5B1" }}>{c.pct.toFixed(1)}%</td>
                          </tr>
                        ))}
    ...
    {reportOpen && reportView === "detalhado" && (
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 11 }}>
        ...
        {/* cabeçalho */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #2451A6", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <img src={LOGO_BASE64} alt="L.L. Campos Transportes" style={{ height: 52 }} />
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#2451A6", letterSpacing: 1 }}>RELATÓRIO DETALHADO</div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 9, color: "#9AA5B1" }}>
        ...
        <div style={{ background: "#1B2430", color: "#fff", padding: "7px 12px", fontWeight: 700, fontSize: 12, marginBottom: 12, letterSpacing: 0.5 }}>
          DETALHAMENTO DAS VIAGENS
        </div>
        ...
        {detailedReport.groups.map((g) => (
          <div key={g.id} style={{ marginBottom: 16 }}>
            ...
            {g.tripRows.map((t) => (
              <div key={t.id} style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: "12px 14px", marginBottom: 10, breakInside: "avoid" }}>
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: "#1F6F5C", marginBottom: 3 }}>💰 RECEITA</div>
                    <table style={{ width: "100%", fontSize: 9, borderCollapse: "collapse" }}>
                      <tbody>
                        <tr><td style={{ padding: "2px 0" }}>{t.origem} - {t.destino}</td><td style={{ textAlign: "right" }}>{BRL(t.valorViagem)}</td></tr>
                      </tbody>
                    </table>
                    <div style={{ fontSize: 9, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#1F6F5C" }}>Total: {BRL(t.valorViagem)}</div>
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: "#D9A419", marginBottom: 3 }}>👤 COMISSÃO</div>
                    {t.comissaoItens.length === 0 ? (
                      <div style={{ fontSize: 9, color: "#9AA5B1" }}>—</div>
                    ) : (
                      <table style={{ width: "100%", fontSize: 9, borderCollapse: "collapse" }}>
                        <tbody>
                          {t.comissaoItens.map((c, i) => (
                            <tr key={i}><td style={{ padding: "2px 0" }}>{c.descricao}</td><td style={{ textAlign: "right" }}>−{BRL(c.valor)}</td></tr>
                          ))}
                        </tbody>
                      </table>
                    )}
                    <div style={{ fontSize: 9, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#D9A419" }}>Total: −{BRL(t.totalComissao)}</div>
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: "#2451A6", marginBottom: 3 }}>⛽ ABASTECIMENTOS</div>
                    {t.abastecimentos.length === 0 ? (
                      <div style={{ fontSize: 9, color: "#9AA5B1" }}>—</div>
                    ) : (
                      <table style={{ width: "100%", fontSize: 9, borderCollapse: "collapse" }}>
                        <tbody>
                          {t.abastecimentos.map((a, i) => (
                            <tr key={i}>
                              <td style={{ padding: "2px 0" }}>{fmtDate(a.data)} {a.posto}</td>
                              <td style={{ textAlign: "right" }}>{BRL(a.valor)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    )}
                    <div style={{ fontSize: 9, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#2451A6" }}>Total: −{BRL(t.totalCombustivel)}</div>
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", borderLeft: "1px solid #E4E7EB", paddingLeft: 10 }}>
                    <div style={{ fontSize: 9, fontWeight: 700, color: "#B0402E", marginBottom: 3 }}>🧾 GASTOS EXTRAS</div>
                    {t.gastosExtrasItens.length === 0 ? (
                      <div style={{ fontSize: 9, color: "#9AA5B1" }}>—</div>
                    ) : (
                      <table style={{ width: "100%", fontSize: 9, borderCollapse: "collapse" }}>
                        <tbody>
                          {t.gastosExtrasItens.map((ge, i) => (
                            <tr key={i}><td style={{ padding: "2px 0" }}>{ge.descricao}</td><td style={{ textAlign: "right" }}>{BRL(ge.valor)}</td></tr>
                          ))}
                        </tbody>
                      </table>
                    )}
                    <div style={{ fontSize: 9, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: "auto", paddingTop: 3, color: "#B0402E" }}>Total: −{BRL(t.totalGastosExtras)}</div>
                  </div>
                    </div>
                    <div style={{ fontSize: 9, color: "#9AA5B1", marginTop: 8, borderTop: "1px solid #F2F3F4", paddingTop: 6 }}>Observação: —</div>
                  </div>
                </div>
              </div>
            ))}

            <div style={{ display: "grid", gridTemplateColumns: g.despesasVeiculoTruck.length > 0 ? "1fr 1fr" : "1fr", gap: 10, alignItems: "start" }}>
              {g.despesasVeiculoTruck.length > 0 && (
                <table style={{ width: "100%", borderCollapse: "collapse", border: "1px solid #E4E7EB" }}>
                  <thead>
                    <tr style={{ background: "#FBEBE8" }}>
                      <td colSpan={3} style={{ padding: "5px 8px", fontWeight: 700, fontSize: 10, color: "#B0402E" }}>🧾 DESPESAS DO VEÍCULO</td>
                    </tr>
                  </thead>
                  <tbody>
                    {g.despesasVeiculoTruck.map((d) => (
                      <tr key={d.id} style={{ borderBottom: "1px solid #EEF0F2" }}>
                        <td style={{ padding: "3px 8px", width: 70, fontSize: 9 }}>{fmtDate(d.data)}</td>
                        <td style={{ padding: "3px 8px", fontSize: 9 }}>{d.descricao} {d.observacao && `(${d.observacao})`}</td>
                        <td style={{ padding: "3px 8px", textAlign: "right", width: 80, color: "#B0402E", fontSize: 9 }}>−{BRL(Math.abs(Number(d.valor) || 0))}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div style={{ border: "1px solid #E4E7EB", borderRadius: 8, overflow: "hidden", fontSize: 10 }}>
                <div style={{ background: "#1B2E6B", color: "#fff", fontWeight: 700, padding: "6px 12px" }}>RESUMO DO VEÍCULO ({g.placa} · período)</div>
                <div style={{ padding: "8px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Total de Viagens (Receita − Comissões)</span><strong>{BRL(g.resumo.totalViagens)}</strong></div>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Total de Combustível</span><strong>{BRL(g.resumo.totalCombustivel)}</strong></div>
        ...
        <div style={{ breakInside: "avoid", marginTop: 8 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.4fr", gap: 12 }}>
            <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 6 }}>📊 TOTAL DO PERÍODO</div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3 }}><span>Receita Total</span><strong style={{ color: "#1F6F5C" }}>{BRL(detailedReport.resumoGeral.receitaGeral)}</strong></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3 }}><span>Despesas Totais</span><strong style={{ color: "#B0402E" }}>{BRL(detailedReport.resumoGeral.despesasTotaisGeral)}</strong></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3 }}><span>Comissões</span><strong style={{ color: "#D9A419" }}>{BRL(detailedReport.resumoGeral.comissaoGeral)}</strong></div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, fontWeight: 700, borderTop: "1px solid #E4E7EB", marginTop: 4, paddingTop: 4 }}>
                <span>Resultado Líquido</span><span style={{ color: detailedReport.resumoGeral.resultadoLiquidoGeral >= 0 ? "#1F6F5C" : "#B0402E" }}>{BRL(detailedReport.resumoGeral.resultadoLiquidoGeral)}</span>
              </div>
            </div>

            <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 12, display: "flex", flexDirection: "column", alignItems: "center" }}>
              <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 6, alignSelf: "flex-start" }}>🍩 DISTRIBUIÇÃO DA RECEITA</div>
              {(() => {
                const r = 40, cx = 50, cy = 50, circunf = 2 * Math.PI * r;
                const pctLiq = detailedReport.resumoGeral.pctReceitaLiquida;
                const pctCom = detailedReport.resumoGeral.pctComissoes;
                ...
                const segDesp = (pctDesp / 100) * circunf;
                return (
                  <div style={{ position: "relative", width: 100, height: 100, margin: "4px 0 8px" }}>
                    <svg width="100" height="100" viewBox="0 0 100 100" style={{ transform: "rotate(-90deg)" }}>
                      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#EEF0F2" strokeWidth="16" />
                      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#1F6F5C" strokeWidth="16" strokeDasharray={`${segLiq} ${circunf - segLiq}`} strokeDashoffset="0" />
                      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#D9A419" strokeWidth="16" strokeDasharray={`${segCom} ${circunf - segCom}`} strokeDashoffset={-segLiq} />
            ...
            <div style={{ border: "1px solid #E4E7EB", borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 8 }}>📈 DESPESAS POR CATEGORIA</div>
              {detailedReport.resumoGeral.categorias.map((c) => (
                <div key={c.nome} style={{ marginBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, marginBottom: 2 }}>
                    <span>{c.nome}</span>
                    <span style={{ fontWeight: 700 }}>{BRL(c.valor)} ({c.pct.toFixed(1)}%)</span>
                  </div>
                  <div style={{ background: "#EEF0F2", borderRadius: 4, height: 6, overflow: "hidden" }}>
                    <div style={{ width: `${c.pct}%`, height: "100%", background: c.cor, borderRadius: 4 }} />
                  </div>
                </div>
              ))}
        ...
        <div style={{ marginTop: 14, fontSize: 9, color: "#9AA5B1", textAlign: "center", borderTop: "1px solid #EEF0F2", paddingTop: 8 }}>
          Este relatório considera todas as viagens e lançamentos de despesas no período selecionado.
          <br />
          LL Campos Transportes · Controle de Viagens
        </div>
      </div>
    )}
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <img src={LOGO_BASE64} alt="L.L. Campos Transportes" style={{ height: 52 }} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>RECEBIMENTO</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{recebimentoCaminhaoFiltro === "all" ? "Todos os caminhões" : truckLabel(recebimentoCaminhaoFiltro)}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(recebimentoPeriodStart)} a {fmtDate(recebimentoPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
            </div>
          </div>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>📄</div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{relatorioRecebimento.items.length}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>RECEBIMENTOS</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>💰</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#8A5A00" }}>{BRL(relatorioRecebimento.totalAdiantamento)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>ADIANTAMENTOS</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✅</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1F6F5C" }}>{BRL(relatorioRecebimento.totalSaldo)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>SALDOS</div>
            </div>
          </div>
          <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL RECEBIDO</div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(relatorioRecebimento.total)}</div>
          </div>
        </div>
          ...
          <tbody>
            {relatorioRecebimento.items.map((r, i) => (
              <tr key={`${r.trip.id}-${r.tipo}`} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                ...
                <td style={{ padding: "6px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{truckLabel(r.trip.caminhaoId)}</td>
                ...
                <td style={{ padding: "6px 8px", borderBottom: "1px solid #EEF0F2" }}>
                  <span style={{ background: r.tipo === "Adiantamento" ? "#FFF6E2" : "#E9F5F1", color: r.tipo === "Adiantamento" ? "#8A5A00" : "#1F6F5C", fontWeight: 700, fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>{r.tipo.toUpperCase()}</span>
                </td>
                <td style={{ padding: "6px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700, color: "#1F6F5C" }}>{BRL(r.valor)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>BOLETOS</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>
                {boletosEmpresasSelecionadas.length < boletosEmpresas.length ? `${boletosEmpresasSelecionadas.length} de ${boletosEmpresas.length} empresas selecionadas` : "Todas as empresas"}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(boletosPeriodStart)} a {fmtDate(boletosPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
            ...
            {boletosReport.items.map((b, i) => {
              ...
              return (
                <tr key={b.__key} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                  <td style={{ padding: "6px 8px", textAlign: "left", borderBottom: "1px solid #EEF0F2", whiteSpace: "normal", wordBreak: "break-word" }}>{b.empresa}</td>
                  <td style={{ padding: "6px 8px", textAlign: "left", borderBottom: "1px solid #EEF0F2", whiteSpace: "normal", wordBreak: "break-word" }}>{b.descricao}</td>
                  <td style={{ padding: "6px 8px", textAlign: "left", borderBottom: "1px solid #EEF0F2", whiteSpace: "normal", wordBreak: "break-word" }}>{b.notaFiscal}</td>
                  <td style={{ padding: "6px 8px", textAlign: "center", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(vencimentoEfetivo(b))}</td>
                  <td style={{ padding: "6px 8px", textAlign: "left", borderBottom: "1px solid #EEF0F2", whiteSpace: "normal", wordBreak: "break-word" }}>{b.__kind === "pago" ? (b.__contaEvento || b.contaBancaria) : b.contaBancaria}</td>
                  <td style={{ padding: "6px 8px", textAlign: "center", borderBottom: "1px solid #EEF0F2" }}>{b.__kind === "pago" ? fmtDate(b.__dataItem) : "—"}</td>
                  <td style={{ padding: "6px 8px", textAlign: "center", borderBottom: "1px solid #EEF0F2" }}>
                    <span style={{ background: bgStatus, color: corStatus, fontWeight: 700, fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>{status.toUpperCase()}</span>
                  </td>
        ...
        <div style={{ display: "flex", gap: 16, fontSize: 9, color: "#5A6472", margin: "10px 0", flexWrap: "wrap" }}>
          <span>🔴 Vencido: boleto com vencimento passado</span>
          <span>🟠 Pendente: boleto a vencer</span>
          <span>🟢 A vencer: próximo a vencer</span>
          <span>🔵 Pago: quitado</span>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "stretch", breakInside: "avoid" }}>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>💲 RESUMO FINANCEIRO</div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>VENCIDOS</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#B0402E" }}>{BRL(boletosReport.totals.vencido)}</div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>PENDENTES</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#8A5A00" }}>{BRL(boletosReport.totals.pendente)}</div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>A VENCER</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#1F6F5C" }}>{BRL(boletosReport.totals.aVencer)}</div>
          </div>
          <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL EM ABERTO</div>
            <div style={{ fontSize: 14, fontWeight: 700 }}>{BRL(boletosReport.totals.vencido + boletosReport.totals.pendente + boletosReport.totals.aVencer)}</div>
          </div>
        </div>
      </div>
    )}
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>ABASTECIMENTOS</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>
                {abastecPostoFilter !== "all" && <>Posto: {abastecPostoFilter} · </>}
                {abastecPlacaFilter !== "all" ? truckLabel(abastecPlacaFilter) : "Todos os caminhões"}
              </div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(abastecPeriodStart)} a {fmtDate(abastecPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>TROCAS DE ÓLEO E OUTROS SERVIÇOS</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{oleoPlacaFilter !== "all" ? truckLabel(oleoPlacaFilter) : "Todos os caminhões"}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(oleoPeriodStart)} a {fmtDate(oleoPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
          ...
          return todosCaminhaoIds.map((caminhaoId) => {
            ...
            return (
              <div key={caminhaoId} style={{ marginBottom: 20, breakInside: "avoid" }}>
                <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 13, marginBottom: 8, background: "#1B2E6B", color: "#fff", borderRadius: 6, padding: "6px 12px" }}>
                ...
                {itensTroca.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                      ...
                      <thead>
                        <tr style={{ background: "#1B2E6B" }}>
                          {["Data", "KM", "Filtro trocado", "Observação", "Valor", "Empresa"].map((h) => (
                      ...
                      <tbody>
                        {itensTroca.map((t, i) => (
                          <tr key={t.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                            ...
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>
                              {(() => {
                                const total = (Number(t.valor) || 0) + (t.pecas || []).reduce((s, p) => s + (Number(p.valor) || 0), 0);
                                return total > 0 ? BRL(total) : "";
                              })()}
                            </td>
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{t.empresa || ""}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                      ...
                      <thead>
                        <tr style={{ background: "#2E3A8C" }}>
                          {["Data", "KM", "Serviço", "Empresa", "Observação", "Valor"].map((h) => (
                            <th key={h} style={{ textAlign: "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {itensServico.map((s, i) => (
                          <tr key={s.id} style={{ background: i % 2 === 0 ? "#fff" : "#EEF0FF" }}>
                            ...
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{s.empresa || "Estrada"}</td>
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", whiteSpace: "normal", wordBreak: "break-word" }}>{s.observacao || ""}</td>
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{s.valor > 0 ? BRL(s.valor) : ""}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>SEM PARAR</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{semPararPlacaFilter !== "all" ? truckLabel(semPararPlacaFilter) : "Todos os caminhões"}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(semPararPeriodStart)} a {fmtDate(semPararPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🛣️</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#B0402E" }}>{BRL(semPararReport.totals.pedagio)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>PEDÁGIO</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>💳</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1F6F5C" }}>{BRL(semPararReport.totals.credito)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>CRÉDITO</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>➕</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#8A5A00" }}>{BRL(semPararReport.totals.outros)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>OUTRAS ARRECADAÇÕES</div>
            </div>
          </div>
          <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL GERAL</div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(semPararReport.totals.total)}</div>
          </div>
        </div>
        {semPararReport.porData.map((remessa) => (
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>SEGURO</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 MÊS</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{seguroMesFiltro}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>FINANCEIRO PESSOAL</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(financeiroPeriodStart)} a {fmtDate(financeiroPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>📤</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#B0402E" }}>{BRL(financeiroReport.totais.pendentePagar)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>A PAGAR</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>📥</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1F6F5C" }}>{BRL(financeiroReport.totais.pendenteReceber)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>A RECEBER</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✅</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700 }}>{BRL(financeiroReport.totais.pagoPagar)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>JÁ PAGO</div>
            </div>
          </div>
          <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>JÁ RECEBIDO</div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(financeiroReport.totais.pagoReceber)}</div>
          </div>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1B2E6B" }}>
              {["Tipo", "Descrição", "Pessoa/empresa", "De quem é", "Vencimento", "Status", "Onde comprou / Nota fiscal", "Valor"].map((h) => (
                <th key={h} style={{ textAlign: h === "Status" ? "center" : "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {financeiroReport.itens.map((f, i) => {
              ...
              const cor = f.tipo === "pagar" ? "#B0402E" : "#1F6F5C";
              return (
                <tr key={f.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>
                    <span style={{ background: f.tipo === "pagar" ? "#FBEBE8" : "#E9F5F1", color: cor, fontWeight: 700, fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>{f.tipo === "pagar" ? "A PAGAR" : "A RECEBER"}</span>
                  </td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{f.descricao}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{f.pessoa || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{f.titular || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(f.dataVencimento)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "center" }}>
                    <span style={{ background: pago ? "#EAF2FB" : vencido ? "#FBEBE8" : "#FFF6E2", color: pago ? "#1B4E8C" : vencido ? "#B0402E" : "#8A5A00", fontWeight: 700, fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>
                      {pago ? (f.tipo === "pagar" ? "PAGO" : "RECEBIDO") : vencido ? "VENCIDO" : "PENDENTE"}
                    </span>
                  </td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>
                    {f.ondeComprou || ""}{f.notaFiscalNumero ? ` · NF ${f.notaFiscalNumero}${f.notaFiscalData ? ` (${fmtDate(f.notaFiscalData)})` : ""}` : ""}
                  </td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700, textAlign: "right" }}>{BRL(Number(f.valor) || 0)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>MULTAS DE TRÂNSITO</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(multaPeriodStart)} a {fmtDate(multaPeriodEnd)}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>📄</div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{multaReport.itens.length}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>MULTAS</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FFF6E2", color: "#8A5A00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>⏳</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#8A5A00" }}>{BRL(multaReport.totalPendente)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>PENDENTE</div>
            </div>
          </div>
          <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E9F5F1", color: "#1F6F5C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✅</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1F6F5C" }}>{BRL(multaReport.totalPago)}</div>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>JÁ PAGO</div>
            </div>
          </div>
          <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL EM ABERTO</div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(multaReport.totalPendente)}</div>
          </div>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1B2E6B" }}>
              {["Data", "Placa", "Motorista", "Descrição", "Vencimento", "Status", "Valor"].map((h) => (
                <th key={h} style={{ textAlign: h === "Status" ? "center" : "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {multaReport.itens.map((m, i) => {
              ...
              const vencida = !pago && m.dataVencimento && m.dataVencimento < todayISO();
              return (
                <tr key={m.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(m.data)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{truckLabel(m.caminhaoId)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{m.motorista || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{m.descricao}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{m.dataVencimento ? fmtDate(m.dataVencimento) : ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "center" }}>
                    <span style={{ background: pago ? "#E9F5F1" : vencida ? "#FBEBE8" : "#FFF6E2", color: pago ? "#1F6F5C" : vencida ? "#B0402E" : "#8A5A00", fontWeight: 700, fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>
                      {pago ? "PAGO" : vencida ? "VENCIDA" : "PENDENTE"}
                    </span>
                  </td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700, textAlign: "right" }}>{BRL(Number(m.valor) || 0)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>RESUMO PRO CONTADOR</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{relatorioContador.qtdViagens} viagens no mês</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 MÊS</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{contadorMes}</div>
            </div>
            <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
              <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
              <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <tbody>
            <tr style={{ background: "#E9F5F1" }}>
              <td style={{ padding: "10px", fontWeight: 700, fontSize: 15 }}>Receita bruta</td>
              <td style={{ padding: "10px", fontWeight: 700, fontSize: 15, textAlign: "right", color: "#12503F" }}>{BRL(relatorioContador.receita)}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>Comissões</td>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", textAlign: "right", color: "#B0402E" }}>− {BRL(relatorioContador.comissoesMes)}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>Gastos extras (estrada)</td>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", textAlign: "right", color: "#B0402E" }}>− {BRL(relatorioContador.gastosEstrada)}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>Boletos pagos ({relatorioContador.boletosDoMes.length})</td>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", textAlign: "right", color: "#B0402E" }}>− {BRL(relatorioContador.boletosMes)}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2" }}>Taxa de viagem (rateio)</td>
              <td style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F2", textAlign: "right", color: "#B0402E" }}>− {BRL(relatorioContador.taxaMes)}</td>
            </tr>
            <tr style={{ background: "#1B2E6B" }}>
              <td style={{ padding: "10px", fontWeight: 700, color: "#fff" }}>LÍQUIDO DO MÊS</td>
              <td style={{ padding: "10px", fontWeight: 700, color: "#fff", textAlign: "right" }}>{BRL(relatorioContador.liquido)}</td>
            </tr>
          </tbody>
        </table>
        {relatorioContador.boletosDoMes.length > 0 && (
          <>
            <div style={{ fontWeight: 700, fontSize: 14, marginTop: 20, marginBottom: 6 }}>Boletos pagos no mês</div>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#1B2E6B" }}>
                  {["Empresa", "Descrição", "Data", "Valor"].map((h) => (
                    <th key={h} style={{ textAlign: "left", padding: "6px 8px", color: "#fff", fontSize: 12 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {relatorioContador.boletosDoMes.map((b, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{b.empresa}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{b.descricao}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(b.data)}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700 }}>{BRL(b.valor)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>{statsDetailOpen === "receber" ? "VIAGENS A RECEBER" : "VIAGENS RECEBIDAS"}</div>
            </div>
          </div>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
              <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>COMISSÃO POR MOTORISTA</div>
              <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{commissionDriverFilter === "all" ? "Todos os motoristas" : commissionDriverFilter}</div>
            </div>
          </div>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
        ...
        {commissionByDriver
          ...
          <div key={g.motorista} style={{ marginBottom: 24, breakInside: "avoid", breakBefore: gi === 0 ? "auto" : "page", pageBreakBefore: gi === 0 ? "auto" : "always" }}>
            ...
            {g.tripsPendentesConclusao.length > 0 && (
              <>
                <div style={{ fontSize: 9, color: "#9AA5B1", fontWeight: 700, marginBottom: 4, textTransform: "uppercase" }}>
                  Aguardando conclusão (fora do total até ter "data fim")
                </div>
                <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 12 }}>
                  <thead>
                    <tr style={{ background: "#2451A6" }}>
                      {["Caminhão", "Data", "Origem → Destino", "Empresa", "Comissão prevista"].map((h) => (
                        <th key={h} style={{ textAlign: "left", padding: "5px 7px", color: "#fff", fontSize: 10 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {g.tripsPendentesConclusao.map(({ trip, valor }, i) => (
                      <tr key={trip.id} style={{ background: i % 2 === 0 ? "#fff" : "#EAF2FB" }}>
                        <td style={{ padding: "4px 7px", borderBottom: "1px solid #EEF0F2", fontWeight: 700, fontSize: 10 }}>{truckLabel(trip.caminhaoId)}</td>
                        <td style={{ padding: "4px 7px", borderBottom: "1px solid #EEF0F2", fontSize: 10 }}>{fmtDate(trip.data)}</td>
                        <td style={{ padding: "4px 7px", borderBottom: "1px solid #EEF0F2", fontSize: 10 }}>{trip.origem} → {trip.destino}</td>
                        <td style={{ padding: "4px 7px", borderBottom: "1px solid #EEF0F2", fontSize: 10 }}>{trip.empresa || ""}</td>
                        <td style={{ padding: "4px 7px", borderBottom: "1px solid #EEF0F2", fontWeight: 700, fontSize: 10, color: "#1B4E8C" }}>{BRL(valor)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
    ...
    {/* ---- versão impressa: Gastos extras por caminhão ---- */}
    {relatorioGastosOpen && (
    <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
            <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>GASTOS EXTRAS POR CAMINHÃO</div>
            <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{gastosSetorFiltro !== "all" ? `Setor: ${gastosSetorFiltro}` : "Todos os setores"}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(gastosPeriodStart)} a {fmtDate(gastosPeriodEnd)}</div>
          </div>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
      ...
      {relatorioGastosExtras.rows.length === 0 ? (
        <div>Nenhum gasto extra lançado nesse período.</div>
      ) : (
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
            <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🚚</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700 }}>{relatorioGastosExtras.rows.length}</div>
                <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>CAMINHÕES COM GASTO</div>
              </div>
            </div>
            <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#FBEBE8", color: "#B0402E", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🧾</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700 }}>{relatorioGastosExtras.rows.reduce((s, r) => s + r.gastos.length, 0)}</div>
                <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>LANÇAMENTOS</div>
              </div>
            </div>
            <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
              <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL GERAL</div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(relatorioGastosExtras.totalGeral)}</div>
            </div>
          </div>

          {relatorioGastosExtras.rows.map((r, ri) => (
            <div key={r.caminhaoId} style={{ marginBottom: 18, breakInside: ri === 0 ? "auto" : "avoid" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
                <div style={{ fontWeight: 700, fontSize: 12, color: "#fff" }}>🚚 {r.placa}</div>
                <div style={{ fontWeight: 700, fontSize: 13, color: "#fff" }}>{BRL(r.total)}</div>
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
                <colgroup>
                  <col style={{ width: "13%" }} />
                ...
                <thead>
                  <tr style={{ background: "#F7F8F9" }}>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Data</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Descrição</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Categoria</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Setor</th>
                ...
                <tbody>
                  {r.gastos.map((g, i) => (
                    <tr key={g.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(g.data)}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>
                        {g.descricao || ""}
                        {g.estabelecimento && <div style={{ fontSize: 9, color: "#9AA5B1" }}>{g.estabelecimento}</div>}
                      </td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{g.categoria || "—"}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{g.setor || "—"}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700 }}>{BRL(Number(g.valor) || 0)}</td>
                    </tr>
                  ))}
                </tbody>
    ...
    <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
            <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>CONSUMO (KM/L)</div>
            <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{consumoCaminhaoFiltro !== "all" ? truckLabel(consumoCaminhaoFiltro) : "Todos os caminhões"}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(consumoPeriodStart)} a {fmtDate(consumoPeriodEnd)}</div>
          </div>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
      ...
      {relatorioConsumo.linhas.length === 0 ? (
        <div>Nenhum intervalo entre abastecimentos de diesel nesse período.</div>
      ) : (
        relatorioConsumo.gruposPorPlaca.map((g, gi) => (
          <div key={g.placa} style={{ marginBottom: 18, breakInside: gi === 0 ? "auto" : "avoid" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
              <div style={{ fontWeight: 700, fontSize: 12, color: "#fff" }}>🚚 {g.placa}</div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
              ...
              <thead>
                <tr style={{ background: "#F7F8F9" }}>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Data</th>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Posto anterior</th>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Posto atual</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#5A6472" }}>Km rodado</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#5A6472" }}>Litros</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#5A6472" }}>Km/L</th>
                </tr>
              </thead>
              <tbody>
                {g.itens.map((l, i) => (
                  <tr key={l.id} style={{ background: l.invalido ? "#FBEBE8" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{fmtDate(l.data)}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{l.postoAnterior || "—"}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{l.posto || "—"}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right" }}>{l.kmRodado}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right" }}>{l.litros ? l.litros.toFixed(1) : "—"}</td>
                    <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700 }}>{l.consumo != null ? l.consumo.toFixed(2) : "—"}</td>
                  </tr>
                ))}
    ...
    {/* ---- versão impressa: Serviços por empresa ---- */}
    {relatorioServicosOpen && (
    <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #1B2E6B", paddingBottom: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#1B2E6B", letterSpacing: 1 }}>RELATÓRIO</div>
            <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>SERVIÇOS POR EMPRESA</div>
            <div style={{ fontSize: 10, color: "#5A6472", marginTop: 2 }}>{servicosEmpresaFiltro !== "all" ? servicosEmpresaFiltro : "Todas as empresas"}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>📅 PERÍODO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{fmtDate(servicosPeriodStart)} a {fmtDate(servicosPeriodEnd)}</div>
          </div>
          <div style={{ border: "1px solid #D7DBE0", borderRadius: 8, padding: "6px 10px" }}>
            <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>🕐 DATA DE EMISSÃO</div>
            <div style={{ fontWeight: 700, fontSize: 11 }}>{new Date().toLocaleDateString("pt-BR")}</div>
      ...
      {relatorioServicos.items.length === 0 ? (
        <div>Nenhum serviço com empresa marcada nesse período.</div>
      ) : (
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 16, breakInside: "avoid" }}>
            <div style={{ flex: 1, border: "1px solid #E4E7EB", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#EAF2FB", color: "#1B4E8C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>🔧</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700 }}>{relatorioServicos.items.length}</div>
                <div style={{ fontSize: 8, color: "#9AA5B1", fontWeight: 700 }}>SERVIÇOS</div>
              </div>
            </div>
            <div style={{ flex: 1, background: "#1B2E6B", color: "#fff", borderRadius: 10, padding: "10px 12px" }}>
              <div style={{ fontSize: 8, opacity: 0.85, fontWeight: 700 }}>TOTAL</div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{BRL(relatorioServicos.items.reduce((s, l) => s + l.valor, 0))}</div>
            </div>
          </div>
          {relatorioServicos.porPlaca.map((grupo, gi) => (
            <div key={grupo.caminhaoId} style={{ marginBottom: 16, breakInside: gi === 0 ? "auto" : "avoid" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, background: "#1B2E6B", borderRadius: 6, padding: "6px 12px" }}>
                <div style={{ fontWeight: 700, fontSize: 12, color: "#fff" }}>🚚 {grupo.placa}</div>
                <div style={{ fontWeight: 700, fontSize: 13, color: "#fff" }}>{BRL(grupo.total)}</div>
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
                <colgroup>
                  <col style={{ width: "14%" }} />
                  <col style={{ width: "12%" }} />
                  <col style={{ width: servicosEmpresaFiltro === "all" ? "36%" : "50%" }} />
                  {servicosEmpresaFiltro === "all" && <col style={{ width: "20%" }} />}
                  <col style={{ width: "18%" }} />
                </colgroup>
                <thead>
                  <tr style={{ background: "#F7F8F9" }}>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Data</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>KM</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Serviço</th>
                    {servicosEmpresaFiltro === "all" && <th style={{ textAlign: "left", padding: "6px 8px", color: "#5A6472" }}>Empresa</th>}
                    <th style={{ textAlign: "right", padding: "6px 8px", color: "#5A6472" }}>Valor</th>
                  </tr>
                </thead>
                <tbody>
                  {grupo.items.map((l, i) => (
                    <tr key={l.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(l.data)}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{l.km ? `${Number(l.km).toLocaleString("pt-BR")} km` : "—"}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{l.servico}</td>
                      {servicosEmpresaFiltro === "all" && <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{l.empresa}</td>}
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", textAlign: "right", fontWeight: 700 }}>{BRL(l.valor)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
