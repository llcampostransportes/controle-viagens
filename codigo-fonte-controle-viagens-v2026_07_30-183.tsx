...
const APP_VERSION = "2026.07.30-183";
  ...
  itensDieselOrdenados.forEach((a) => {
    ...
    if (km > 0) {
      if (ultimoComKm) {
        ...
        pontos.push({ ...a, kmRodado, litros: litrosAcumulados, consumo, postosPulados: [...postosPulados], kmAnterior: ultimoComKm.km, postoAnterior: ultimoComKm.posto });
      } else {
        pontos.push({ ...a, consumo: null, litros: litrosAcumulados, postosPulados: [...postosPulados] });
      }
      ultimoComKm = { km, data: a.data, posto: a.posto };
      litrosAcumulados = 0;
      postosPulados = [];
    } else if (a.posto) {
      postosPulados.push(a.posto);
    }
  });
  return pontos;
}
                      ...
                      <tbody>
                        {g.itens.map((l, i) => (
                          <tr key={l.id} style={{ background: l.invalido ? "#FBEBE8" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                            ...
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.postoAnterior || "—"}</td>
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              ...
                              {l.postosPulados.length > 0 && (
                                <div style={{ fontSize: 9, color: "#8A5A00", whiteSpace: "normal" }} title={l.postosPulados.join(", ")}>
                                  + litros de {l.postosPulados.length} sem km ({l.postosPulados.join(", ")})
                                </div>
                              )}
                            </td>
