...
// comissao: normalmente 13% em cima do valor de comissão menos pedágio (e menos carregamento,
// quando teve troca de motorista no meio da viagem). Mas se a viagem estiver marcada como
// "comissão prefixada", usa direto o valor que foi digitado, sem calcular porcentagem.
const comissao = (t) => {
  ...
  return Math.max(0, (base - pedagio - carregamento) * 0.13);
};
...
export default function App() {
  ...
  const tentarSalvarPendencias = async () => {
    ...
    setSaveState("saving");
    try {
      await saveToSheets(tokenRef.current, pendingChangesRef.current);
        ...
        {abastecReport.porPosto.map((grupo) => (
          <div key={grupo.posto} style={{ marginBottom: 16, breakInside: "avoid" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Data", "Caminhão", "Origem", "Litragem", "R$/L", "Cupom", "Valor"].map((h) => (
              ...
              <tbody>
                {grupo.items.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td style={{ padding: "4px 8px", textAlign: "right", borderBottom: "1px solid #D7DBE0" }}>{Number(a.litragem) > 0 ? BRL(a.valor / Number(a.litragem)) : ""}</td>
        ...
        {statsDetailOpen === "receber" ? (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {pendingList.map((r) => (
                <tr key={r.trip.id}>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{fmtDate(r.trip.data)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0", fontWeight: 700 }}>{truckLabel(r.trip.caminhaoId)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.trip.contrato || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.trip.empresa || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.pendAdiantamento > 0 ? BRL(r.pendAdiantamento) : ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.pendSaldo > 0 ? BRL(r.pendSaldo) : ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0", fontWeight: 700 }}>{BRL(r.pendTotal)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr>
                <td colSpan={6} style={{ padding: "8px", fontWeight: 700, borderTop: "2px solid #1B2430" }}>TOTAL</td>
                <td style={{ padding: "8px", fontWeight: 700, borderTop: "2px solid #1B2430" }}>{BRL(pendingList.reduce((s, r) => s + r.pendTotal, 0))}</td>
              </tr>
            </tfoot>
          </table>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {receivedList.map((r, i) => (
                <tr key={r.trip.id + r.tipo + i}>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{fmtDate(r.data)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0", fontWeight: 700 }}>{truckLabel(r.trip.caminhaoId)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.trip.contrato || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.trip.empresa || ""}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0" }}>{r.tipo}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #D7DBE0", fontWeight: 700 }}>{BRL(r.valor)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr>
                <td colSpan={5} style={{ padding: "8px", fontWeight: 700, borderTop: "2px solid #1B2430" }}>TOTAL</td>
                <td style={{ padding: "8px", fontWeight: 700, borderTop: "2px solid #1B2430" }}>{BRL(receivedList.reduce((s, r) => s + r.valor, 0))}</td>
              </tr>
            </tfoot>
          </table>
        )}
