...
const APP_VERSION = "2026.07.05-50";
...
const BOLETOS_HEADER = ["id", "empresa", "descricao", "notaFiscal", "valor", "dataVencimento", "contaBancaria", "dataPagamento", "observacao", "desconto", "descontoDescricao", "juros"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Boletos!A1:L1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Boletos!A1:L1", values: [BOLETOS_HEADER] }),
  });
...
function boletoToRow(b) {
  return [b.id, b.empresa, b.descricao || "", b.notaFiscal, b.valor, b.dataVencimento, b.contaBancaria, b.dataPagamento, b.observacao || "", b.desconto || "", b.descontoDescricao || "", b.juros || ""];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    ...
    sheetsFetch(`/values/Boletos!A2:L`, token),
...
const SHEET_DEFS = {
  ...
  boletos: { nome: "Boletos", header: BOLETOS_HEADER, colunas: "L", toRow: boletoToRow },
...
function BoletosView({
  ...
  const statsCards = useMemo(() => {
    const base = report.items.filter((b) => {
      if (listFilter && !(b.empresa || "").toLowerCase().includes(listFilter.toLowerCase())) return false;
      return true;
    });
    return base.reduce(
  ...
  const openEdit = (b) => { setEditing({ ...b }); setBoletoTravado(!!b.dataPagamento); setPanelOpen(true); };
