...
const APP_VERSION = "2026.07.30-228";
...
async function ensureSheetsExist(token) {
  ...
  const missing = ["Caminhoes", "Viagens", "Vales", "Boletos", "Empresas", "Fechamentos", "DespesasVeiculo", "TaxasPool", "Motoristas", "Contas", "TrocasOleo", "ServicosVeiculo", "SemParar", "Seguro", "SemPararOutros", "FinanceiroPessoal", "Config"].filter((t) => !titles.includes(t));
  if (missing.length > 0) {
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, financeiroPessoalRes, configRes] = await Promise.all([
  ...
  const config = (configRes.values || [])
    .filter((row) => row[0])
    .map(rowToConfig);
  return { trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, servicosVeiculo, semParar, seguro, semPararOutros, financeiroPessoal, config };
}
...
export default function App() {
  ...
  const reloadFromSheets = async () => {
    try {
      const { trucks: t, trips: v, vales: vl, boletos: bo, empresas: emp, fechamentos: fe, despesasVeiculo: dv, taxasPool: tp, motoristas: mo, contas: ct, trocasOleo: to, servicosVeiculo: sv, semParar: sp, seguro: sg, semPararOutros: spo, financeiroPessoal: fp, config: cfg } = await loadFromSheets(tokenRef.current);
