...
const APP_VERSION = "2026.07.05-141";
...
const emptyGasto = () => ({ id: uid(), data: "", valor: "", descricao: "", categoria: "Despesas Estrada", posto: "", numeroCupom: "", paraComissao: false });
  ...
  const relatorioGastosExtras = useMemo(() => {
    ...
    tripsNoPeriodo.forEach((t) => {
      (t.gastosExtras || []).forEach((g) => {
        ...
        const chave = t.caminhaoId || "sem-placa";
        if (!porCaminhao[chave]) porCaminhao[chave] = { caminhaoId: t.caminhaoId, placa: truckLabel(t.caminhaoId), gastos: [], total: 0 };
        porCaminhao[chave].gastos.push({ ...g, data: g.data || t.data, trip: t });
        porCaminhao[chave].total += Number(g.valor) || 0;
      });
    });
    ...
    despesasVeiculo
      ...
      .forEach((d) => {
        ...
        const chave = d.caminhaoId || "sem-placa";
        if (!porCaminhao[chave]) porCaminhao[chave] = { caminhaoId: d.caminhaoId, placa: truckLabel(d.caminhaoId), gastos: [], total: 0 };
        porCaminhao[chave].gastos.push({ id: d.id, data: d.data, descricao: d.descricao, categoria: "Taxa de Viagem", valor: d.valor });
        porCaminhao[chave].total += Number(d.valor) || 0;
      });
    const rows = Object.values(porCaminhao).map((r) => ({ ...r, gastos: r.gastos.sort((a, b) => (a.data || "").localeCompare(b.data || "")) }));
    rows.sort((a, b) => b.total - a.total);
    const totalGeral = rows.reduce((s, r) => s + r.total, 0);
    return { rows, totalGeral };
  }, [trips, despesasVeiculo, gastosPeriodStart, gastosPeriodEnd]);
                      ...
                      <tbody>
                        {r.gastos.map((g, i) => (
                          <tr key={g.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                            ...
                            <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{g.descricao || ""}</td>
