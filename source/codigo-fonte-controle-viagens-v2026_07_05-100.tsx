...
const APP_VERSION = "2026.07.05-100";
...
const CAMINHOES_HEADER = ["id", "placa", "semAlertaOleo", "intervaloOleoKm", "tipo", "modelo", "foto", "renavam", "chassi", "anoFabricacao", "anoModelo", "cor", "combustivel", "categoria", "municipio", "uf", "vencimentoDocumento", "documentosPorAnoJSON", "crv"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Caminhoes!A1:S1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Caminhoes!A1:S1", values: [CAMINHOES_HEADER] }),
  });
...
function rowToTruck(row) {
  ...
  return {
    ...
    vencimentoDocumento: row[16] || "", documentosPorAno, crv: row[18] || "",
  };
}
function truckToRow(t) {
  return [
    ...
    JSON.stringify(t.documentosPorAno || {}), t.crv || "",
  ];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    sheetsFetch(`/values/Caminhoes!A2:S`, token),
...
const SHEET_DEFS = {
  trucks: { nome: "Caminhoes", header: CAMINHOES_HEADER, colunas: "S", toRow: truckToRow },
...
function carregarZXing() {
  ...
  zxingCarregando = new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error("Não consegui carregar o leitor de código de barras. Confere sua internet."));
...
export default function App() {
  ...
  const totalAlertas = alertasTrocaOleo.length + viagensSemComissaoPreenchida.length + viagensSemValorFrete.length + caminhoesDocumentoAtrasado.length;
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              style={{
                background: totalAlertas > 0 ? "#FFF6E2" : "#F7F8FA",
                border: `1px solid ${totalAlertas > 0 ? "#D9A419" : "#E4E7EB"}`,
                borderRadius: 12, padding: "8px 14px", cursor: "pointer",
                display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700,
                color: totalAlertas > 0 ? "#8A5A00" : "#9AA5B1",
              }}
            >
              🔔 {totalAlertas > 0 ? `${totalAlertas} ${totalAlertas === 1 ? "alerta" : "alertas"}` : "Sem alertas"}
            </button>
            {alertasAbertos && (
              <div
                className="painel-modal"
      ...
      {view === "dashboard" && (
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 28px" }}>
          ...
          {(() => {
            ...
            const maiorFaturamento = rowsComViagem.reduce((melhor, r) => (r.receita > melhor.receita ? r : melhor), rowsComViagem[0]);
              ...
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
                <div style={{ flex: "1 1 340px", minWidth: 300, display: "flex", alignItems: "center", gap: 16, background: "linear-gradient(160deg, #fff 0%, #E9F5F1 100%)", border: "1px solid #EDEFF2", borderRadius: 14, boxShadow: "0 2px 8px rgba(27,36,48,0.06)", padding: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 700, color: "#12503F", fontVariantNumeric: "tabular-nums" }}>{BRL(maiorFaturamento.receita)}</div>
                  </div>
                </div>
                    ...
                    <div style={{ padding: "0 10px 12px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div>
                        <Field label="Vencimento do licenciamento/documento (opcional, só referência)">
                          <input type="date" value={tr.vencimentoDocumento || ""} onChange={(e) => updateTruckInfo(tr.id, { vencimentoDocumento: e.target.value })} style={{ ...inputStyle, fontSize: 12, padding: "6px 8px" }} />
                        </Field>
                      </div>
                          ...
                          return (
                            <>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, background: temAnoAtual ? "#E9F5F1" : "#FBEBE8", borderRadius: 8, padding: "8px 10px" }}>
                                ...
                                <label style={{ fontSize: 11, color: "#2451A6", cursor: "pointer", border: "1px solid #D7DBE0", borderRadius: 8, padding: "5px 10px", background: "#fff", whiteSpace: "nowrap" }}>
                                  {temAnoAtual ? "Trocar" : "+ anexar"}
                                  <input
                                    type="file"
                                    ...
                                    onChange={(e) => {
                                      const file = e.target.files && e.target.files[0];
                                      if (!file) return;
                                      redimensionarFotoDocumento(file, (base64) => {
                                        updateTruckInfo(tr.id, { documentosPorAno: { ...(tr.documentosPorAno || {}), [anoAtual]: base64 } });
                                      });
                                      e.target.value = "";
                                    }}
                                  />
