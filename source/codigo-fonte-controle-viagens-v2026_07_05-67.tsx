...
const APP_VERSION = "2026.07.05-67";
...
const CAMINHOES_HEADER = ["id", "placa", "semAlertaOleo", "intervaloOleoKm", "tipo", "modelo", "foto"];
...
const BOLETOS_HEADER = ["id", "empresa", "descricao", "notaFiscal", "valor", "dataVencimento", "contaBancaria", "dataPagamento", "observacao", "desconto", "descontoDescricao", "juros", "baixasJSON"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Caminhoes!A1:G1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Caminhoes!A1:G1", values: [CAMINHOES_HEADER] }),
  });
  ...
  await sheetsFetch(`/values/Boletos!A1:M1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Boletos!A1:M1", values: [BOLETOS_HEADER] }),
  });
...
function rowToTruck(row) {
  return { id: row[0] || "", placa: row[1] || "", semAlertaOleo: row[2] === "sim", intervaloOleoKm: row[3] || "", tipo: row[4] === "carreta" ? "carreta" : "cavalo", modelo: row[5] || "", foto: row[6] || "" };
}
function truckToRow(t) {
  return [t.id, t.placa, t.semAlertaOleo ? "sim" : "", t.intervaloOleoKm || "", t.tipo === "carreta" ? "carreta" : "cavalo", t.modelo || "", t.foto || ""];
}
...
function boletoToRow(b) {
  return [b.id, b.empresa, b.descricao || "", b.notaFiscal, b.valor, b.dataVencimento, b.contaBancaria, b.dataPagamento, b.observacao || "", b.desconto || "", b.descontoDescricao || "", b.juros || "", JSON.stringify(b.baixas || [])];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    sheetsFetch(`/values/Caminhoes!A2:G`, token),
    sheetsFetch(`/values/Viagens!A2:X`, token),
    sheetsFetch(`/values/Vales!A2:H`, token),
    sheetsFetch(`/values/Boletos!A2:M`, token),
...
const SHEET_DEFS = {
  trucks: { nome: "Caminhoes", header: CAMINHOES_HEADER, colunas: "G", toRow: truckToRow },
  ...
  boletos: { nome: "Boletos", header: BOLETOS_HEADER, colunas: "M", toRow: boletoToRow },
...
function EmpresasChecklist({ titulo, empresasList, empresas, empresasSelecionadas, onToggle, onMarcarTodas, onDesmarcarTodas, onResetPadrao }) {
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", marginBottom: 20, maxWidth: 340 }}>
      <button
        onClick={() => setAberto((v) => !v)}
        style={{
          width: "100%", textAlign: "left", background: "#fff", border: "1px solid #D7DBE0", borderRadius: 6,
          padding: "9px 12px", fontSize: 13, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center",
        }}
      >
        <span>{titulo} ({empresasSelecionadas.length} de {empresasList.length})</span>
        <span style={{ transform: aberto ? "rotate(180deg)" : "none", transition: "transform 0.15s", color: "#5A6472" }}>▾</span>
      </button>
      ...
      {aberto && (
        <div
          ...
          style={{
            ...
            background: "#fff", border: "1px solid #D7DBE0", borderRadius: 8, boxShadow: "0 8px 24px rgba(0,0,0,0.12)",
        ...
        >
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
            <button onClick={onMarcarTodas} style={{ background: "none", border: "none", color: "#2451A6", fontSize: 12, textDecoration: "underline", cursor: "pointer" }}>marcar todas</button>
            <button onClick={onDesmarcarTodas} style={{ background: "none", border: "none", color: "#2451A6", fontSize: 12, textDecoration: "underline", cursor: "pointer" }}>desmarcar todas</button>
            <button onClick={onResetPadrao} style={{ background: "none", border: "none", color: "#5A6472", fontSize: 12, textDecoration: "underline", cursor: "pointer" }}>usar padrão</button>
          </div>
            ...
            return (
              <div key={categoria} style={{ marginBottom: 6 }}>
                <button
                  ...
                  style={{ background: "none", border: "none", padding: "4px 0", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 13, color: "#1B2430", width: "100%" }}
                >
                  <span style={{ display: "inline-block", transform: fechada ? "rotate(-90deg)" : "none", transition: "transform 0.15s" }}>▾</span>
                  {categoria}
                  <span style={{ fontSize: 11, color: "#5A6472", fontWeight: 400 }}>({marcadasDaCategoria}/{nomesDaCategoria.length})</span>
                </button>
                {!fechada && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 2, marginTop: 4, paddingLeft: 20 }}>
                    {nomesDaCategoria.map((nome) => {
                      ...
                      return (
                        <label
                          key={nome}
                          style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, cursor: "pointer", padding: "4px 6px", borderRadius: 4, color: marcada ? "#12503F" : "#5A6472" }}
                        >
                          <input type="checkbox" checked={marcada} onChange={() => onToggle(nome)} />
                          {nome}
                        </label>
                      );
                    })}
...
function BoletosView({
  ...
  empresas, onAddEmpresa, onRemoveEmpresa, onRenameEmpresa, onSetCategoriaEmpresa, onSetIncluirRelatorioEmpresa,
  ...
  const visibleBoletos = useMemo(() => {
    return report.items
      .filter((b) => {
        if (!boletoBateBusca(b, listFilter)) return false;
  ...
  // são eles que mostram a divisão por status
  // valor de um boleto já descontando desconto/juros e qualquer baixa parcial
  const valorFinalBoleto = (b) => {
    const base = (Number(b.valor) || 0) - (Number(b.desconto) || 0) + (Number(b.juros) || 0);
    const totalBaixas = (b.baixas || []).reduce((s, bx) => s + (Number(bx.valor) || 0), 0);
    return base - totalBaixas;
  };

  const statsCards = useMemo(() => {
    const base = report.items.filter((b) => boletoBateBusca(b, listFilter));
    return base.reduce(
      (acc, b) => {
        const valor = valorFinalBoleto(b);
        if (b.dataPagamento) acc.pago += valor;
        else if (b.dataVencimento < hoje) acc.vencido += valor;
        else acc.pendente += valor;
        return acc;
      },
      { count: base.length, pendente: 0, vencido: 0, pago: 0 }
    );
  }, [report.items, listFilter, hoje]);
  ...
  const openEdit = (b) => {
    setEditing({ ...b, baixas: b.baixas || [] });
    setBoletoTravado(!!b.dataPagamento);
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      <div className="boletos-flex-row" style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
      {/* empresas cadastradas, agrupadas por categoria — agora como coluna lateral */}
      <div className="sidebar-empresas" style={{ background: "#fff", borderRadius: 8, padding: 14, width: 280, flexShrink: 0, position: "sticky", top: 20 }}>
            ...
            {empresas.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {empresas.slice().sort((a, b) => a.nome.localeCompare(b.nome)).map((e) => (
                  <div key={e.id} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", background: "#F7F8F9", borderRadius: 6, padding: 8 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, marginBottom: 20 }}>
        <input
          ...
          placeholder="Buscar por empresa, descrição, NF ou observação..."
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
        <div onClick={() => setStatusFilter("all")} style={{ cursor: "pointer" }}>
          <MileSign label="Boletos" value={statsCards.count} tone="amber" />
        </div>
        <div onClick={() => setStatusFilter("pendente")} style={{ cursor: "pointer" }}>
          <MileSign label="Pendente" value={BRL(statsCards.pendente)} tone="amber" />
        </div>
        <div onClick={() => setStatusFilter("vencido")} style={{ cursor: "pointer" }}>
          <MileSign label="Vencido" value={BRL(statsCards.vencido)} tone="red" />
        </div>
        <div onClick={() => setStatusFilter("pago")} style={{ cursor: "pointer" }}>
          <MileSign label="Pago" value={BRL(statsCards.pago)} tone="green" />
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>
                  {BRL((Number(b.valor) || 0) - (Number(b.desconto) || 0) + (Number(b.juros) || 0))}
                  ...
                  {(b.baixas || []).length > 0 && (
                    <div style={{ fontSize: 10, fontWeight: 700, color: "#8A5A00" }}>
                      restam {BRL(
                        (Number(b.valor) || 0) - (Number(b.desconto) || 0) + (Number(b.juros) || 0) -
                        b.baixas.reduce((s, bx) => s + (Number(bx.valor) || 0), 0)
                      )}
                    </div>
                  )}
                </div>
      ...
      {panelOpen && editing && (
        <>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(480px, 100vw)", background: "#fff", zIndex: 21, overflowY: "auto", boxShadow: "-8px 0 24px rgba(0,0,0,0.15)", padding: "24px 24px 100px" }}>
            ...
            {boletoTravado && (
              <div style={{ background: "#FFF6E2", border: "1px solid #D9A419", borderRadius: 8, padding: 12, marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <button
                  ...
                  style={{ background: "#D9A419", color: "#1B2430", border: "none", borderRadius: 6, padding: "7px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}
                >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
              ...
              <Field label="Data de vencimento">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.dataVencimento} onChange={(e) => setEditing({ ...editing, dataVencimento: e.target.value })} />
              </Field>
              <Field label="Data de pagamento (baixa)">
                <input disabled={boletoTravado} type="date" style={inputStyle} value={editing.dataPagamento} onChange={(e) => setEditing({ ...editing, dataPagamento: e.target.value })} />
              </Field>
            ...
            {(Number(editing.desconto) > 0 || Number(editing.juros) > 0) && (
              <div style={{ background: "#F7F8F9", borderRadius: 8, padding: 10, marginBottom: 16, fontSize: 13 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#5A6472", textTransform: "uppercase", marginBottom: 8 }}>
                Baixas parciais (ex: vale adiantado)
              </div>
              {(editing.baixas || []).length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                  {editing.baixas.map((bx) => (
                    <div key={bx.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#F7F8F9", borderRadius: 6, padding: "6px 10px", fontSize: 12 }}>
                      <span>{fmtDate(bx.data)} {bx.observacao && `— ${bx.observacao}`}</span>
              ...
              {!boletoTravado && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
                  <Field label="Data">
                    <input type="date" style={inputStyle} value={novaBaixaData} onChange={(e) => setNovaBaixaData(e.target.value)} />
                  </Field>
                  <Field label="Valor (R$)">
                  ...
                  <button
                    onClick={() => {
                      ...
                      setEditing({
                        ...editing,
                        baixas: [...(editing.baixas || []), { id: uid(), data: novaBaixaData, valor: novaBaixaValor, observacao: novaBaixaObs.trim() }],
                      });
                      setNovaBaixaValor("");
                      setNovaBaixaObs("");
                    }}
              ...
              {(editing.baixas || []).length > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, marginTop: 10, background: "#1B2430", color: "#fff", borderRadius: 6, padding: "8px 12px" }}>
                  <span>Saldo restante</span>
                  <span>
                    {BRL(
                      (Number(editing.valor) || 0) - (Number(editing.desconto) || 0) + (Number(editing.juros) || 0) -
                      editing.baixas.reduce((s, bx) => s + (Number(bx.valor) || 0), 0)
                    )}
                  </span>
                </div>
              )}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button disabled={boletoTravado} onClick={save} style={{ flex: 1, background: boletoTravado ? "#EEF0F2" : "#D9A419", color: boletoTravado ? "#9AA5B1" : "#1B2430", border: "none", borderRadius: 6, padding: "12px", fontWeight: 700, cursor: boletoTravado ? "not-allowed" : "pointer" }}>
                Salvar boleto
              </button>
              {boletos.some((b) => b.id === editing.id) && (
                <button disabled={boletoTravado} onClick={remove} style={{ background: boletoTravado ? "#EEF0F2" : "#FBEBE8", color: boletoTravado ? "#9AA5B1" : "#B0402E", border: boletoTravado ? "1px solid #D7DBE0" : "1px solid #B0402E33", borderRadius: 6, padding: "12px 16px", fontWeight: 700, cursor: boletoTravado ? "not-allowed" : "pointer" }}>
                  Excluir
                </button>
              )}
            </div>
                    ...
                    {report.items.map((b, i) => {
                      ...
                      return (
                        <tr key={b.id} style={{ borderBottom: "1px solid #EEF0F2", background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                          ...
                          <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700 }}>{BRL(valorFinalBoleto(b))}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
export default function App() {
  ...
  const [recebimentoPeriodStart, setRecebimentoPeriodStart] = useState(() => todayISO());
  ...
  const confirmAddServico = () => {
    ...
    if (editingServicoId) {
      updateServicoVeiculo(editingServicoId, servicoCaminhaoId, servicoData, servicoKm, servicoTipo.trim(), servicoObs.trim());
    } else {
  ...
  const updateServicoVeiculo = (id, caminhaoId, data, km, tipoServico, observacao) => {
    const nextServicos = servicosVeiculo.map((s) => (s.id === id ? { ...s, caminhaoId, data, km, tipoServico, observacao } : s));
    setServicosVeiculo(nextServicos);
    persist(trucks, trips, vales, boletos, empresas, fechamentos, despesasVeiculo, taxasPool, motoristas, contas, trocasOleo, nextServicos);
  };
  ...
  const tripsParaExibir = useMemo(() => {
    ...
    return list;
  }, [tripsNoPeriodo, tripListStatusFilter, tripListSortBy]);
  ...
  // total geral: soma a comissão de TODAS as viagens de todos os caminhões, pagas
  // ou não — igual ao resto do cartão de Comissão, não filtra por placa
  const totalComissaoGeral = useMemo(
    () => trips.reduce((s, t) => s + comissao(t), 0),
    [trips]
  );
  ...
  // perfil de abastecimento de uma placa específica: histórico ordenado por km,
  // consumo (km/l) calculado entre abastecimentos consecutivos de diesel, média
  ...
  const perfilAbastecimentoCaminhao = useMemo(() => {
    ...
    const itensDiesel = abastecimentosFlat
      .filter((a) => a.caminhaoId === abastecPlacaFilter && a.tipo === "diesel" && Number(a.km) > 0)
      .sort((a, b) => (Number(a.km) || 0) - (Number(b.km) || 0));
    const comConsumo = itensDiesel.map((a, i) => {
      if (i === 0) return { ...a, consumo: null };
      const kmRodado = (Number(a.km) || 0) - (Number(itensDiesel[i - 1].km) || 0);
  ...
  // valor de um boleto já descontando desconto/juros e qualquer baixa parcial
  // já registrada — é esse valor que deve valer em todo lugar que soma boleto
  const valorFinalBoleto = (b) => {
    const base = (Number(b.valor) || 0) - (Number(b.desconto) || 0) + (Number(b.juros) || 0);
    const totalBaixas = (b.baixas || []).reduce((s, bx) => s + (Number(bx.valor) || 0), 0);
    return base - totalBaixas;
  ...
  const boletosReport = useMemo(() => {
    ...
    const filtered = boletos
      ...
      .filter((b) => boletosEmpresasSelecionadas.includes(b.empresa))
      .filter((b) => {
  ...
  const exportBoletosCSV = () => {
    const header = ["Empresa", "Descricao", "Nota Fiscal", "Valor Original", "Saldo Restante", "Vencimento", "Conta Bancaria", "Data Pagamento", "Status", "Observacao"];
    const hoje = todayISO();
    const lines = [header.join(";")];
    boletosReport.items.forEach((b) => {
      const status = b.dataPagamento ? "Pago" : b.dataVencimento < hoje ? "Vencido" : "Pendente";
      lines.push(
        [b.empresa, b.descricao || "", b.notaFiscal, (Number(b.valor) || 0).toFixed(2), valorFinalBoleto(b).toFixed(2), fmtDate(b.dataVencimento), b.contaBancaria, b.dataPagamento ? fmtDate(b.dataPagamento) : "", status, b.observacao || ""]
          .join(";")
      );
    });
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    ...
    >
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 640px) {
          ...
          table thead, table tbody, table tfoot, table tr { display: table !important; width: 100% !important; }
      ...
      <div style={{ background: "#fff", padding: "18px 28px", position: "sticky", top: 0, zIndex: 10, borderBottom: "1px solid #E4E7EB", boxShadow: "0 1px 3px rgba(27,36,48,0.04)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ background: "#F7F8FA", border: "1px solid #E4E7EB", borderRadius: 8, padding: "6px 10px", display: "flex", alignItems: "center" }}>
            ...
            >
              CONTROLE DE VIAGENS
              <span style={{ fontSize: 11, color: "#9AA5B1", fontFamily: "'JetBrains Mono', monospace", marginLeft: 10, verticalAlign: "middle" }}>
                v{APP_VERSION}
              </span>
            </div>
          ...
          <div className="header-botoes" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <button
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 16, justifyContent: "space-between", flexWrap: "wrap" }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "viagens" ? "#2451A6" : "transparent",
              color: view === "viagens" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "boletos" ? "#2451A6" : "transparent",
              color: view === "boletos" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "abastecimentos" ? "#2451A6" : "transparent",
              color: view === "abastecimentos" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "trocaoleo" ? "#2451A6" : "transparent",
              color: view === "trocaoleo" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "semparar" ? "#2451A6" : "transparent",
              color: view === "semparar" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "20px",
              border: "none",
              background: view === "seguro" ? "#2451A6" : "transparent",
              color: view === "seguro" ? "#fff" : "#5A6472",
              fontWeight: 700,
              fontSize: 14,
              cursor: "pointer",
            }}
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <button
              onClick={() => setRelatorioRecebimentoOpen(true)}
              title="Ver contratos recebidos por período"
              style={{
                background: "transparent",
                color: "#3A4351",
                border: "1px solid #D7DBE0",
      ...
      {viagensSemComissaoPreenchida.length > 0 && (
        <div style={{ background: "#FFF6E2", borderBottom: "1px solid #D9A419", padding: "10px 28px", fontSize: 13, color: "#8A5A00" }}>
          ...
          {viagensSemComissaoPreenchida.map((t) => `${truckLabel(t.caminhaoId)} ${fmtDate(t.data)} (${t.motorista})`).join(" · ")}
        </div>
      )}
      ...
      {viagensSemValorFrete.length > 0 && (
        <div style={{ background: "#FBEBE8", borderBottom: "1px solid #B0402E", padding: "10px 28px", fontSize: 13, color: "#7A2A1D" }}>
          ...
          {viagensSemValorFrete.map((t) => `${truckLabel(t.caminhaoId)} ${fmtDate(t.data)}`).join(" · ")}
        </div>
      )}

      {view === "viagens" && (
      <div className="boletos-flex-row" style={{ display: "flex", gap: 20, alignItems: "flex-start", padding: "24px 28px", maxWidth: 1400, margin: "0 auto" }}>
        {/* barra lateral de placas */}
        <div className="sidebar-empresas" style={{ background: "#fff", borderRadius: 8, padding: 14, width: 260, flexShrink: 0, position: "sticky", top: 20, border: "1px solid #E4E7EB" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#5A6472", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>
            Placas
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: "none",
                background: filterTruck === "all" ? "#EEF2FA" : "transparent",
                color: filterTruck === "all" ? "#2451A6" : "#3A4351",
                fontWeight: filterTruck === "all" ? 700 : 500,
                fontSize: 13,
                cursor: "pointer",
              }}
            >
            ...
            {trucksCavalos.map((tr) => (
              <div key={tr.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <button
                  onClick={() => setFilterTruck(tr.id)}
                  style={{
                    flex: 1,
                    textAlign: "left",
                    padding: "8px 10px",
                    borderRadius: 6,
                    border: "none",
                    background: filterTruck === tr.id ? "#EEF2FA" : "transparent",
                    color: filterTruck === tr.id ? "#2451A6" : "#3A4351",
                    fontWeight: filterTruck === tr.id ? 700 : 500,
                    fontSize: 13,
                    fontFamily: "'JetBrains Mono', monospace",
                    cursor: "pointer",
                  }}
                ...
                <button
                  onClick={() => { if (window.confirm(`Marcar ${tr.placa} como CARRETA? Ela vai deixar de aparecer em lançamento de viagem, troca de óleo e abastecimento.`)) alternarTipoPlaca(tr.id); }}
                  title="Marcar como carreta (ela não deveria aparecer aqui)"
                  style={{ background: "none", color: "#9AA5B1", border: "none", fontSize: 12, cursor: "pointer", padding: 4 }}
                >
                  🚚
                </button>
                <button
                  ...
                  style={{ background: "none", color: "#B0402E", border: "none", fontSize: 14, cursor: "pointer", padding: 4 }}
                >
                  ×
                </button>
              </div>
        ...
        {filterTruck !== "all" && (() => {
          const trSel = trucksCavalos.find((tr) => tr.id === filterTruck);
          if (!trSel) return null;
          return (
            <div style={{ display: "flex", gap: 16, alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 10, padding: 16, marginBottom: 20 }}>
              <div style={{ width: 72, height: 72, borderRadius: 10, overflow: "hidden", background: "#F7F8F9", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {trSel.foto ? (
                  <img src={trSel.foto} alt={trSel.placa} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <TruckIcon size={48} />
                )}
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 22, fontFamily: "'JetBrains Mono', monospace" }}>{trSel.placa}</div>
        ...
        {/* filtro e ordenacao da lista de viagens */}
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 12, color: "#5A6472", fontWeight: 600 }}>Período:</span>
            {[
              { valor: "all", rotulo: "Tudo" },
              { valor: "mes", rotulo: "Este mês" },
              { valor: "hoje", rotulo: "Hoje" },
              { valor: "escolher", rotulo: "Escolher mês" },
            ].map((op) => (
              <button
                key={op.valor}
                onClick={() => setTripListPeriodFilter(op.valor)}
                style={{
                  padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
                  border: tripListPeriodFilter === op.valor ? "2px solid #D9A419" : "1px solid #D7DBE0",
                  background: tripListPeriodFilter === op.valor ? "#FFF6E2" : "#fff",
                }}
              >
                {op.rotulo}
              </button>
            ...
            {tripListPeriodFilter === "escolher" && (
              <input
                ...
                style={{ padding: "5px 10px", borderRadius: 6, border: "1px solid #D7DBE0", fontSize: 12 }}
              />
            )}
          </div>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "#5A6472", fontWeight: 600 }}>Status:</span>
            {[
              ...
              <button
                key={op.valor}
                onClick={() => setTripListStatusFilter(op.valor)}
                style={{
                  padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
                  border: tripListStatusFilter === op.valor ? "2px solid #D9A419" : "1px solid #D7DBE0",
                  background: tripListStatusFilter === op.valor ? "#FFF6E2" : "#fff",
                }}
              >
                {op.rotulo}
              </button>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "#5A6472", fontWeight: 600 }}>Ordenar por:</span>
            {[
              { valor: "data", rotulo: "Data da viagem" },
              { valor: "lancamento", rotulo: "Ordem de lançamento" },
            ].map((op) => (
              <button
                key={op.valor}
                onClick={() => setTripListSortBy(op.valor)}
                style={{
                  padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
                  border: tripListSortBy === op.valor ? "2px solid #D9A419" : "1px solid #D7DBE0",
                  background: tripListSortBy === op.valor ? "#FFF6E2" : "#fff",
                }}
              >
                {op.rotulo}
              </button>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {pendingList.map((r) => (
                    <div
                      ...
                      style={{ background: "#FBEBE8", border: r.semValorPreenchido ? "1px solid #B0402E" : "none", borderRadius: 8, padding: "10px 14px", cursor: "pointer" }}
                    >
                  ...
                  <div key={g.motorista}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                      {g.trips.map(({ trip, valor, semValorPreenchido }) => (
                        <div
                          ...
                          style={{ background: semValorPreenchido ? "#FBEBE8" : "#FFF6E2", border: semValorPreenchido ? "1px solid #B0402E" : "none", borderRadius: 8, padding: "8px 12px", cursor: "pointer", fontSize: 12 }}
                        >
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    {addingValeFor === g.motorista ? (
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 8 }}>
                        ...
                        <button onClick={() => { setAddingValeFor(null); setEditingValeId(null); }} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "8px 14px", cursor: "pointer" }}>
                          Cancelar
                        </button>
                      </div>
                    ) : (
            ...
            <div style={{ marginBottom: 14 }}>
              {addingDespesaFor !== null ? (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end", background: "#F7F8F9", padding: 10, borderRadius: 8 }}>
                  ...
                  <button onClick={() => { setAddingDespesaFor(null); setEditingDespesaId(null); }} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "8px 14px", cursor: "pointer" }}>
                    Cancelar
                  </button>
                </div>
              ) : (
            ...
            <div style={{ marginBottom: 18, background: "#FFF6E2", borderRadius: 8, padding: 12 }}>
              ...
              {addingTaxa ? (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <button onClick={() => { setAddingTaxa(false); setEditingTaxaId(null); }} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "8px 14px", cursor: "pointer" }}>
                    Cancelar
                  </button>
                </div>
              ) : (
                  ...
                  <tbody>
                    {monthlyReport.rows.map((r) => (
                      <tr key={r.id} style={{ borderBottom: "1px solid #EEF0F2" }}>
                        <td style={{ padding: "8px 10px", textAlign: "left", fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>
      ...
      {relatorioRecebimentoOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
              <Field label="De">
                <input type="date" style={inputStyle} value={recebimentoPeriodStart} onChange={(e) => setRecebimentoPeriodStart(e.target.value)} />
              </Field>
              <Field label="Até">
                <input type="date" style={inputStyle} value={recebimentoPeriodEnd} onChange={(e) => setRecebimentoPeriodEnd(e.target.value)} />
              </Field>
              <Field label="Caminhão">
      ...
      {view === "abastecimentos" && (
        <div className="boletos-flex-row" style={{ display: "flex", gap: 20, alignItems: "flex-start", padding: "24px 28px", maxWidth: 1400, margin: "0 auto" }}>
          ...
          <div className="sidebar-empresas" style={{ background: "#fff", borderRadius: 8, padding: 14, width: 260, flexShrink: 0, position: "sticky", top: 20, border: "1px solid #E4E7EB" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <button
                onClick={() => setAbastecPlacaFilter("all")}
                style={{
                  textAlign: "left", padding: "8px 10px", borderRadius: 6, border: "none",
                  background: abastecPlacaFilter === "all" ? "#EEF2FA" : "transparent",
                  color: abastecPlacaFilter === "all" ? "#2451A6" : "#3A4351",
                  fontWeight: abastecPlacaFilter === "all" ? 700 : 500, fontSize: 13, cursor: "pointer",
                }}
              >
                Todos os caminhões
              </button>
              {trucksCavalos.map((tr) => (
                <button
                  key={tr.id}
                  onClick={() => setAbastecPlacaFilter(tr.id)}
                  style={{
                    display: "flex", alignItems: "center", gap: 8, textAlign: "left", padding: "8px 10px", borderRadius: 6, border: "none",
                    background: abastecPlacaFilter === tr.id ? "#EEF2FA" : "transparent",
                    color: abastecPlacaFilter === tr.id ? "#2451A6" : "#3A4351",
                    fontWeight: abastecPlacaFilter === tr.id ? 700 : 500, fontSize: 13, cursor: "pointer",
                  }}
                >
                  <div style={{ width: 24, height: 24, borderRadius: 5, overflow: "hidden", background: "#F7F8F9", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {tr.foto ? <img src={tr.foto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <TruckIcon size={16} />}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace" }}>{tr.placa}</div>
                    {tr.modelo && <div style={{ fontSize: 10, color: "#9AA5B1", fontWeight: 400, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tr.modelo}</div>}
                  </div>
                </button>
              ))}
            </div>
          ...
          {abastecPlacaFilter !== "all" && perfilAbastecimentoCaminhao && (() => {
            ...
            return (
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", gap: 16, alignItems: "center", background: "#fff", border: "1px solid #E4E7EB", borderRadius: 10, padding: 16, marginBottom: 14, flexWrap: "wrap" }}>
                  <div style={{ width: 72, height: 72, borderRadius: 10, overflow: "hidden", background: "#F7F8F9", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                {perfilAbastecimentoCaminhao.chartPoints.length > 0 && (
                  <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 10, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 10, padding: 16, marginBottom: 20 }}>
                    ...
                    <tbody>
                      {perfilAbastecimentoCaminhao.historico.map((a) => (
                        <tr key={a.id} style={{ borderBottom: "1px solid #EEF0F2" }}>
                          ...
                          <td style={{ padding: "5px 8px", textAlign: "right" }}>{Number(a.km).toLocaleString("pt-BR")}</td>
          ...
          <button
            ...
            style={{ background: "none", border: "1px solid #B0402E", color: "#B0402E", borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", marginBottom: 12 }}
          >
        ...
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          {addingTrocaOleo && (
            ...
            >
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingTrocaOleoId ? "Editar troca de óleo" : "Lançar troca de óleo"}</div>
            ...
            <div style={{ marginBottom: 12 }}>
              {addingServico && (
                ...
                >
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingServicoId ? "Editar serviço" : "Registrar outro serviço"}</div>
        ...
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "flex-end" }}>
              ...
              {editingSemPararOutroId && (
                <button
                  ...
                  style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "8px 14px", fontSize: 12, cursor: "pointer" }}
                >
                  Cancelar
                </button>
              )}
          ...
          {semPararReport.porData.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {semPararReport.porData.map((remessa) => (
                <div key={remessa.dataVencimento} style={{ border: "1px solid #D7DBE0", borderRadius: 8, overflow: "hidden" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: remessa.temNaoConfirmado ? "#FFF6E2" : "#E9F5F1", padding: "8px 14px" }}>
                    <span style={{ fontWeight: 700, fontSize: 14 }}>
                      ...
                      {remessa.temNaoConfirmado ? (
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#8A5A00", background: "#fff", borderRadius: 4, padding: "2px 6px", marginLeft: 8 }}>pendente de confirmar</span>
                      ) : (
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#12503F", background: "#fff", borderRadius: 4, padding: "2px 6px", marginLeft: 8 }}>confirmado</span>
                      )}
                    </span>
              ...
              <div style={{ background: "#1B2430", color: "#fff", borderRadius: 8, padding: 14 }}>
          ...
          {addingSemParar && (
            ...
            >
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingSemPararId ? "Editar lançamento Sem Parar" : "Lançar vencimento Sem Parar"}</div>
      ...
      {unificarPostosOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ width: "min(480px, 94vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 10, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 22 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 20 }}>Unificar postos</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 260, overflowY: "auto", marginBottom: 14, border: "1px solid #EEF0F2", borderRadius: 8, padding: 8 }}>
              {postosList.length === 0 ? (
                <div style={{ fontSize: 13, color: "#5A6472", padding: 8 }}>Nenhum posto lançado ainda.</div>
              ) : (
                postosList.map((nome) => (
                  <label key={nome} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, cursor: "pointer", padding: "5px 6px", borderRadius: 4 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button onClick={confirmUnificarPostos} style={{ flex: 1, background: "#1F6F5C", color: "#fff", border: "none", borderRadius: 6, padding: "10px", fontWeight: 700, cursor: "pointer" }}>
                Unificar
              </button>
              <button onClick={() => setUnificarPostosOpen(false)} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "10px 16px", cursor: "pointer" }}>
                Cancelar
              </button>
            </div>
          </div>
      ...
      {view === "seguro" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          {trucksCarretas.length > 0 && (
            <div style={{ background: "#fff", borderRadius: 8, padding: 12, marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#5A6472", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>
                Carretas cadastradas
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {trucksCarretas.map((tr) => (
                  <div key={tr.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "#F7F8F9", borderRadius: 20, padding: "6px 12px", fontSize: 12 }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>{tr.placa}</span>
                    <button
                      onClick={() => { if (window.confirm(`Marcar ${tr.placa} de volta como CAVALO?`)) alternarTipoPlaca(tr.id); }}
                      title="Marcar como cavalo (caso tenha sido marcada como carreta por engano)"
                      style={{ background: "none", border: "none", color: "#2451A6", cursor: "pointer", fontSize: 11, textDecoration: "underline" }}
                    >
                      é cavalo?
                    </button>
              ...
              {Object.keys(seguroReport.porSeguradora).length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 12 }}>
                  {Object.entries(seguroReport.porSeguradora).map(([nome, valor]) => (
                    <div key={nome} style={{ background: "#EEF0FF", border: "1px solid #C7CFFA", borderRadius: 8, padding: "8px 14px", fontSize: 13 }}>
          ...
          {addingSeguro && (
            ...
            >
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 20 }}>{editingSeguroId ? "Editar seguro" : "Lançar seguro"} — {seguroMesFiltro}</div>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                  ...
                  <button onClick={() => { setAddingSeguro(false); setEditingSeguroId(null); }} style={{ background: "none", border: "1px solid #D7DBE0", borderRadius: 6, padding: "10px 16px", cursor: "pointer" }}>
                    Cancelar
                  </button>
                </div>
              </div>
      ...
      {configOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: "#1B2430", marginBottom: 6 }}>
              Vencimento por seguradora (Seguro)
            </div>
            ...
            <button
              ...
              style={{ border: "1px dashed #B7BFC8", background: "transparent", borderRadius: 6, padding: "6px 12px", fontSize: 12, color: "#5A6472", cursor: "pointer", marginBottom: 16 }}
            >
              + adicionar seguradora
            </button>

            <div style={{ fontSize: 13, fontWeight: 700, color: "#1B2430", marginBottom: 6 }}>
              Caminhões (foto e modelo)
            </div>
            <div style={{ fontSize: 11, color: "#5A6472", marginBottom: 10 }}>
              Aparece no topo quando você seleciona a placa em Viagens/Abastecimentos.
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
              {trucksCavalos.map((tr) => (
                <div key={tr.id} style={{ display: "flex", gap: 10, alignItems: "center", background: "#F7F8F9", borderRadius: 8, padding: 10 }}>
                  <div style={{ width: 52, height: 52, borderRadius: 8, overflow: "hidden", background: "#EEF0F2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {tr.foto ? (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, fontFamily: "'JetBrains Mono', monospace" }}>{tr.placa}</div>
                    <input
                      value={tr.modelo || ""}
                      onChange={(e) => updateTruckInfo(tr.id, { modelo: e.target.value })}
                      placeholder="ex: Mercedes Benz 2546"
                      style={{ ...inputStyle, marginTop: 4, fontSize: 12, padding: "5px 8px" }}
                    />
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <label style={{ fontSize: 11, color: "#2451A6", cursor: "pointer", textAlign: "center", border: "1px solid #D7DBE0", borderRadius: 6, padding: "4px 8px" }}>
          ...
          >
            ...
            <Section title="Financeiro">
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#5A6472", gridColumn: "1 / -1", marginTop: 4 }}>
                ...
                />
                Comissão já foi paga fora do sistema (viagem antiga — entra no relatório mensal, mas não aparece como "a receber" do motorista)
              </label>
            </Section>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#2451A6", color: "#fff" }}>
                      <td style={{ padding: "4px 6px", fontWeight: 700 }}>{fmtDate(t.data)}</td>
                      ...
                      <td style={{ padding: "4px 6px", textAlign: "right" }}>{t.destino}</td>
                    </tr>
                  </thead>
                  <tbody>
                    {t.expenses.map((e, i) => (
          ...
          <thead>
            <tr style={{ background: "#2451A6" }}>
              {["Data da viagem", "Data recebido", "Caminhão", "Contrato", "Empresa", "Tipo", "Valor"].map((h) => (
                <th key={h} style={{ textAlign: h === "Valor" ? "right" : "left", padding: "8px", color: "#fff", fontFamily: "'Barlow Condensed', sans-serif", fontSize: 13 }}>
                  {h}
                </th>
              ))}
            </tr>
      ...
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        ...
        <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 16 }}>
          ...
          {boletosEmpresasSelecionadas.length < boletosEmpresas.length && (
            <> · Empresas: {boletosEmpresasSelecionadas.length === 0 ? "nenhuma selecionada" : boletosEmpresasSelecionadas.join(", ")}</>
          )}
            ...
            {boletosReport.items.map((b, i) => {
              ...
              return (
                <tr key={b.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                  ...
                  <td style={{ padding: "6px 8px", textAlign: "right", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{BRL(valorFinalBoleto(b))}</td>
                </tr>
              );
            })}
          </tbody>
