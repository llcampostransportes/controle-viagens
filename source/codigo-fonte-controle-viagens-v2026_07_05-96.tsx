...
const APP_VERSION = "2026.07.05-96";
...
const CAMINHOES_HEADER = ["id", "placa", "semAlertaOleo", "intervaloOleoKm", "tipo", "modelo", "foto", "renavam", "chassi", "anoFabricacao", "anoModelo", "cor", "combustivel", "categoria", "municipio", "uf"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Caminhoes!A1:P1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Caminhoes!A1:P1", values: [CAMINHOES_HEADER] }),
  });
...
function truckToRow(t) {
  return [
    ...
    t.cor || "", t.combustivel || "", t.categoria || "", t.municipio || "", t.uf || "",
  ];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    sheetsFetch(`/values/Caminhoes!A2:P`, token),
...
const SHEET_DEFS = {
  trucks: { nome: "Caminhoes", header: CAMINHOES_HEADER, colunas: "P", toRow: truckToRow },
...
export default function App() {
  ...
  const totalAlertas = alertasTrocaOleo.length + viagensSemComissaoPreenchida.length + viagensSemValorFrete.length;
