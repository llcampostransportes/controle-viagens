...
const APP_VERSION = "2026.07.30-255";
...
async function ensureSheetsExist(token) {
  ...
  const missing = ["Caminhoes", "Viagens", "Vales", "Boletos", "Empresas", "Fechamentos", "DespesasVeiculo", "TaxasPool", "Motoristas", "Contas", "TrocasOleo", "ServicosVeiculo", "SemParar", "Seguro", "SemPararOutros", "FinanceiroPessoal", "Multas", "Config"].filter((t) => !titles.includes(t));
  if (missing.length > 0) {
...
export default function App() {
  ...
  const relatorioServicos = useMemo(() => {
    ...
    return { items: linhas };
  }, [trips, servicosVeiculo, trocasOleo, servicosPeriodStart, servicosPeriodEnd, servicosEmpresaFiltro]);
    ...
    >
        ...
        >
          ...
          >
            ...
            {relatorioServicos.items.length === 0 ? (
              ...
              <>
                <div style={{ fontSize: 12, color: "#5A6472", marginBottom: 8 }}>
                  {relatorioServicos.items.length} serviço{relatorioServicos.items.length !== 1 ? "s" : ""}
                </div>
                <div style={{ background: "#fff", border: "1px solid #E4E7EB", borderRadius: 12, overflow: "hidden" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                    ...
                    <tfoot>
                      <tr style={{ background: "#1B2430" }}>
                        <td colSpan={servicosEmpresaFiltro === "all" ? 4 : 3} style={{ padding: "10px 12px", fontWeight: 700, color: "#fff" }}>TOTAL</td>
                        {servicosEmpresaFiltro === "all" && <td />}
                        <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 700, color: "#fff", fontSize: 15 }}>
                          {BRL(relatorioServicos.items.reduce((s, l) => s + l.valor, 0))}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
      ...
      {relatorioContadorOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="modal-relatorio"
            style={{ width: "min(700px, 96vw)", maxHeight: "88vh", overflowY: "auto", background: "#fff", borderRadius: 14, zIndex: 21, boxShadow: "0 12px 40px rgba(0,0,0,0.25)", padding: 24 }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 22 }}>
                ...
                return todasAsPlacas.map((placa) => {
                  ...
                  return (
                    <div key={placa} style={{ marginBottom: 24 }}>
                      <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8, color: "#1B2430" }}>
                        🚚 {placa}
                      </div>
                      {itensTroca.length > 0 && (
      ...
      {view === "financeiropessoal" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
            <div style={{ flex: "1 1 200px", background: "#FBEBE8", border: "1px solid #B0402E33", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#8A5A5A", textTransform: "uppercase", fontWeight: 700 }}>A pagar (pendente)</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#7A2A1D" }}>{BRL(financeiroReport.totais.pendentePagar)}</div>
              {financeiroReport.totais.vencidoPagar > 0 && <div style={{ fontSize: 11, color: "#B0402E", fontWeight: 700 }}>⚠️ {BRL(financeiroReport.totais.vencidoPagar)} vencido</div>}
            </div>
            <div style={{ flex: "1 1 200px", background: "#E9F5F1", border: "1px solid #1F6F5C33", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#3E7D6C", textTransform: "uppercase", fontWeight: 700 }}>A receber (pendente)</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#12503F" }}>{BRL(financeiroReport.totais.pendenteReceber)}</div>
              {financeiroReport.totais.vencidoReceber > 0 && <div style={{ fontSize: 11, color: "#8A5A00", fontWeight: 700 }}>⚠️ {BRL(financeiroReport.totais.vencidoReceber)} vencido</div>}
            </div>
            <div style={{ flex: "1 1 200px", background: "#F7F8F9", border: "1px solid #E4E7EB", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#5A6472", textTransform: "uppercase", fontWeight: 700 }}>Já pago</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#1B2430" }}>{BRL(financeiroReport.totais.pagoPagar)}</div>
            </div>
            <div style={{ flex: "1 1 200px", background: "#F7F8F9", border: "1px solid #E4E7EB", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#5A6472", textTransform: "uppercase", fontWeight: 700 }}>Já recebido</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#1B2430" }}>{BRL(financeiroReport.totais.pagoReceber)}</div>
            </div>
          </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#F7F8F9" }}>
                    ...
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Pessoa/empresa</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>De quem é</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Vencimento</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Pago/recebido em</th>
                  ...
                  {financeiroReport.itens.map((f, i) => {
                    ...
                    return (
                      <tr key={f.id} style={{ borderTop: "1px solid #EEF0F2", background: vencido ? "#FFF6E2" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        <td style={{ padding: "8px 10px", fontWeight: 700, color: cor }}>{f.tipo === "pagar" ? "A pagar" : "A receber"}</td>
                        ...
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{fmtDate(f.dataVencimento)}{vencido && <span style={{ color: "#B0402E", fontWeight: 700 }}> ⚠️</span>}</td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{f.dataPagamento ? fmtDate(f.dataPagamento) : "—"}</td>
      ...
      {view === "multas" && (
        <div style={{ padding: "24px 28px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
            <div style={{ flex: "1 1 200px", background: "#FBEBE8", border: "1px solid #B0402E33", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#8A5A5A", textTransform: "uppercase", fontWeight: 700 }}>Pendente</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#7A2A1D" }}>{BRL(multaReport.totalPendente)}</div>
              {multaReport.totalVencido > 0 && <div style={{ fontSize: 11, color: "#B0402E", fontWeight: 700 }}>⚠️ {BRL(multaReport.totalVencido)} vencido</div>}
            </div>
            <div style={{ flex: "1 1 200px", background: "#F7F8F9", border: "1px solid #E4E7EB", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: "#5A6472", textTransform: "uppercase", fontWeight: 700 }}>Já pago</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#1B2430" }}>{BRL(multaReport.totalPago)}</div>
            </div>
          </div>
          ...
          {multaReport.itens.length === 0 ? (
            ...
            <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden", border: "1px solid #E4E7EB" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#F7F8F9" }}>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Data</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Placa</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Motorista</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Descrição</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Vencimento</th>
                    <th style={{ textAlign: "left", padding: "8px 10px" }}>Pago em</th>
                    <th style={{ textAlign: "right", padding: "8px 10px" }}>Valor</th>
                    <th></th>
                  </tr>
                </thead>
                  ...
                  {multaReport.itens.map((m, i) => {
                    ...
                    return (
                      <tr key={m.id} style={{ borderTop: "1px solid #EEF0F2", background: vencida ? "#FFF6E2" : i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                        ...
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{m.dataVencimento ? fmtDate(m.dataVencimento) : "—"}{vencida && <span style={{ color: "#B0402E", fontWeight: 700 }}> ⚠️</span>}</td>
                        <td style={{ padding: "8px 10px", color: "#5A6472" }}>{pago ? fmtDate(m.dataPagamento) : "—"}</td>
    ...
    {view === "financeiropessoal" && (
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1B2430" }}>
              {["Tipo", "Descrição", "Pessoa/empresa", "De quem é", "Vencimento", "Pago/recebido em", "Onde comprou / Nota fiscal", "Valor"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {financeiroReport.itens.map((f, i) => (
              <tr key={f.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{f.tipo === "pagar" ? "A pagar" : "A receber"}</td>
                ...
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{f.dataPagamento ? fmtDate(f.dataPagamento) : ""}</td>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>
                  {f.ondeComprou || ""}{f.notaFiscalNumero ? ` · NF ${f.notaFiscalNumero}${f.notaFiscalData ? ` (${fmtDate(f.notaFiscalData)})` : ""}` : ""}
                </td>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{BRL(Number(f.valor) || 0)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ background: "#FBEBE8" }}>
              <td colSpan={7} style={{ padding: "8px", fontWeight: 700 }}>A PAGAR — pendente</td>
              <td style={{ padding: "8px", fontWeight: 700 }}>{BRL(financeiroReport.totais.pendentePagar)}</td>
            </tr>
            <tr style={{ background: "#E9F5F1" }}>
              <td colSpan={7} style={{ padding: "8px", fontWeight: 700 }}>A RECEBER — pendente</td>
              <td style={{ padding: "8px", fontWeight: 700 }}>{BRL(financeiroReport.totais.pendenteReceber)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
    ...
    {view === "multas" && (
      <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1B2430" }}>
              {["Data", "Placa", "Motorista", "Descrição", "Vencimento", "Pago em", "Valor"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "7px 8px", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {multaReport.itens.map((m, i) => (
              <tr key={m.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                ...
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2" }}>{m.dataPagamento ? fmtDate(m.dataPagamento) : ""}</td>
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{BRL(Number(m.valor) || 0)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ background: "#FBEBE8" }}>
              <td colSpan={6} style={{ padding: "8px", fontWeight: 700 }}>PENDENTE</td>
              <td style={{ padding: "8px", fontWeight: 700 }}>{BRL(multaReport.totalPendente)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
    ...
    <div className="print-only" style={{ display: "none", padding: 24, fontFamily: "'Inter', sans-serif", color: "#1B2430", fontSize: 12 }}>
      ...
      {relatorioGastosExtras.rows.length === 0 ? (
        ...
        <>
          {relatorioGastosExtras.rows.map((r, ri) => (
            <div key={r.caminhaoId} style={{ marginBottom: 18, breakInside: ri === 0 ? "auto" : "avoid" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
                ...
                <thead>
                  <tr style={{ background: "#1B2430" }}>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Data</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Descrição</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Categoria</th>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Setor</th>
                    <th style={{ textAlign: "right", padding: "6px 8px", color: "#fff" }}>Valor</th>
                  </tr>
                </thead>
                <tbody>
                  {r.gastos.map((g, i) => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 15, background: "#1B2430", color: "#fff", borderRadius: 8, padding: "10px 14px" }}>
            <span>TOTAL GERAL</span>
            <span>{BRL(relatorioGastosExtras.totalGeral)}</span>
          </div>
        </>
          ...
          <div key={g.placa} style={{ marginBottom: 18, breakInside: gi === 0 ? "auto" : "avoid" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
              ...
              <thead>
                <tr style={{ background: "#1B2430" }}>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Data</th>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Posto anterior</th>
                  <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Posto atual</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#fff" }}>Km rodado</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#fff" }}>Litros</th>
                  <th style={{ textAlign: "right", padding: "6px 8px", color: "#fff" }}>Km/L</th>
                </tr>
              </thead>
              <tbody>
                {g.itens.map((l, i) => (
        ...
        <>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
            <colgroup>
              ...
              <col style={{ width: "12%" }} />
              <col style={{ width: "10%" }} />
              <col style={{ width: servicosEmpresaFiltro === "all" ? "32%" : "44%" }} />
              {servicosEmpresaFiltro === "all" && <col style={{ width: "18%" }} />}
              <col style={{ width: "16%" }} />
            </colgroup>
            <thead>
              <tr style={{ background: "#1B2430" }}>
                <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Data</th>
                <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Placa</th>
                <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>KM</th>
                <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Serviço</th>
                {servicosEmpresaFiltro === "all" && <th style={{ textAlign: "left", padding: "6px 8px", color: "#fff" }}>Empresa</th>}
                <th style={{ textAlign: "right", padding: "6px 8px", color: "#fff" }}>Valor</th>
              </tr>
            </thead>
            <tbody>
              {relatorioServicos.items.map((l, i) => (
                <tr key={l.id} style={{ background: i % 2 === 0 ? "#fff" : "#F7F8F9" }}>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", color: "#5A6472" }}>{fmtDate(l.data)}</td>
                  <td style={{ padding: "5px 8px", borderBottom: "1px solid #EEF0F2", fontWeight: 700 }}>{l.placa}</td>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 15, background: "#1B2430", color: "#fff", borderRadius: 8, padding: "10px 14px", marginTop: 10 }}>
            <span>TOTAL</span>
            <span>{BRL(relatorioServicos.items.reduce((s, l) => s + l.valor, 0))}</span>
          </div>
        </>
      )}
    </div>
