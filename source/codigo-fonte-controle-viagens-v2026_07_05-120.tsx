...
const APP_VERSION = "2026.07.05-120";
...
const BOLETOS_HEADER = ["id", "empresa", "descricao", "notaFiscal", "valor", "dataVencimento", "contaBancaria", "dataPagamento", "observacao", "desconto", "descontoDescricao", "juros", "baixasJSON", "codigoBarras", "linhaDigitavel"];
...
async function ensureSheetsExist(token) {
  ...
  await sheetsFetch(`/values/Boletos!A1:O1?valueInputOption=RAW`, token, {
    method: "PUT",
    body: JSON.stringify({ range: "Boletos!A1:O1", values: [BOLETOS_HEADER] }),
  });
...
function boletoToRow(b) {
  return [b.id, b.empresa, b.descricao || "", b.notaFiscal, b.valor, b.dataVencimento, b.contaBancaria, b.dataPagamento, b.observacao || "", b.desconto || "", b.descontoDescricao || "", b.juros || "", JSON.stringify(b.baixas || []), b.codigoBarras || "", b.linhaDigitavel || ""];
}
...
async function loadFromSheets(token) {
  ...
  const [caminhoesRes, viagensRes, valesRes, boletosRes, empresasRes, fechamentosRes, despesasVeiculoRes, taxasPoolRes, motoristasRes, contasRes, trocasOleoRes, servicosVeiculoRes, semPararRes, seguroRes, semPararOutrosRes, configRes] = await Promise.all([
    ...
    sheetsFetch(`/values/Boletos!A2:O`, token),
...
const SHEET_DEFS = {
  ...
  boletos: { nome: "Boletos", header: BOLETOS_HEADER, colunas: "O", toRow: boletoToRow },
...
function LeitorCodigoBarras({ onDetectado, onFechar }) {
  ...
  const confirmarLinhaManual = () => {
    ...
    if (limpo.length !== 47) {
      setErroManual(`A linha digitável tem que ter 47 números — você digitou ${limpo.length}. Confere se não faltou nenhum.`);
      return;
    }
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 60, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ width: "min(420px, 92vw)", background: "#1B2430", borderRadius: 14, padding: 16 }}>
          <div style={{ color: "#fff", fontWeight: 700, marginBottom: 10 }}>✏️ Digite a linha digitável</div>
          <div style={{ color: "#9AA5B1", fontSize: 12, marginBottom: 10 }}>
            São os 47 números impressos no topo do boleto (com os pontinhos) — pode digitar só os números, sem os pontos.
          </div>
          <input
            value={linhaDigitada}
      ...
      {view === "dashboard" && (
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 28px" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
            ...
            <div style={{ flex: "1 1 0", minWidth: 160 }}><MileSign label="Receita" value={BRL(dashboardReport.totals.receita)} tone="green" icon="💵" full /></div>
              ...
              {filterTruck !== "all" && (
                <>
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", color: "#2451A6", textDecoration: "underline", cursor: "pointer", fontSize: 12, padding: 0 }}
                  >
                    ver todos os caminhões
                  </button>
                </>
